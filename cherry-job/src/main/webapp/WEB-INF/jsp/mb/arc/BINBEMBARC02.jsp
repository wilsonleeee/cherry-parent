...
function doBatch(url, flag) {
    ...
    if(confirm("您确定要执行吗？")) {
        ...
        var param = $('#arc02ExecForm').serialize();
        if (flag) {
        	param += "&zflag=1";
        }
        $.ajax({
...
<body>
	...
	<div class="main container clearfix">
		...
		<div class="panel-content">
		    ...
		    <div class="section">
			  <form id="arc02ExecForm">
			      <s:hidden name="brandInfoId"></s:hidden>
	  			  <s:hidden name="brandCode"></s:hidden>
				  <div class="clearfix" style="margin-top:10px;">
				 	<span class="left">需要导入的积分ID范围：<s:textfield name="mptIdStart"></s:textfield>-<s:textfield name="mptIdEnd"></s:textfield>
				 	<br/>最后更新时间在&nbsp;<s:textfield name="uptDay" cssStyle="width:30px;"></s:textfield>&nbsp;天内的记录需要做更新处理
				 	<br/><input type="checkbox" name="cgptFlag" value="1" >总积分和可兑换积分别导入 </span>
				  	<s:url action="BINBEMBARC02_arcExec" id="arc02ExecUrl"></s:url>
				  	<a class="add" onclick="doBatch('${arc02ExecUrl }');return false;">
					  <span class="ui-icon icon-enable"></span><span class="button-text">执行</span>
				    </a>	
				    <a class="add" onclick="doBatch('${arc02ExecUrl }', '1');return false;">
