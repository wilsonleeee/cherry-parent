...
function doBatch(url,flg) {
    ...
    if(confirm("您确定要执行吗？")) {
        ...
        var param = $('#pro01Form').serialize();
        if(flg == 2) {
        	param = $('#pro01RecalForm').serialize();
        }
        $.ajax({
            url :url, 
            type:'post',
		...
		<div class="panel-content">
		    ...
		    <div class="section">
		      ...
		      <div class="clearfix" style="margin-top:10px;">
		      	<span class="left">产品月度库存统计（以业务日期为计算时间点）：</span>
		        <s:url action="ss/BINBESSPRO01_pro01Exec" id="pro01ExecUrl"></s:url>
		        <a class="add" onclick="doBatch('${pro01ExecUrl }',1);return false;">
				  <span class="ui-icon icon-enable"></span><span class="button-text">执行</span>
			    </a>
			  </div>
			  <form id="pro01RecalForm">
			      <s:hidden name="brandInfoId"></s:hidden>
	  			  <s:hidden name="brandCode"></s:hidden>
				  <div class="clearfix" style="margin-top:10px;">
				 	<span class="left">重算产品月度库存统计的时间范围：<s:textfield name="controlDateStart"></s:textfield>-<s:textfield name="controlDateEnd"></s:textfield></span> 
				  	<s:url action="ss/BINBESSPRO01_pro01RecalExec" id="pro01RecalExecUrl"></s:url>
				  	<a class="add" onclick="doBatch('${pro01RecalExecUrl }',2);return false;">
