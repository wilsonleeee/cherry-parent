...
function doBatch(url) {
    ...
    if(confirm("您确定要执行吗？")) {
        ...
        var param = $('#han04ExecForm').serialize();
        $.ajax({
            url :url, 
            type:'post',
            dataType:'html',
...
<body>
	...
	<div class="main container clearfix">
		...
		<div class="panel-content">
		    ...
		    <div class="section">
			  <form id="han04ExecForm">
			      <s:hidden name="brandInfoId"></s:hidden>
	  			  <s:hidden name="brandCode"></s:hidden>
				  <div class="clearfix" style="margin-top:10px;">
				 	<span class="left">需要积分清零处理的会员ID范围：<s:textfield name="mebIdStart"></s:textfield>-<s:textfield name="mebIdEnd"></s:textfield></span> 
				  	<s:url action="BINBEDRHAN04_han04Exec" id="han04ExecUrl"></s:url>
				  	<a class="add" onclick="doBatch('${han04ExecUrl }');return false;">
					  <span class="ui-icon icon-enable"></span><span class="button-text">执行</span>
				    </a>	
			      </div>
		      </form>
		      <div class="clearfix" style="margin-top:10px;">
				 	<span class="left"><label style="color:red;">包含起始和结束ID，两者都不输入时，处理全部会员</label></span> 
		    </div>
		</div>
	</div>
