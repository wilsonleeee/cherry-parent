var BINBESSPRM05 = function () {};

BINBESSPRM05.prototype = {
	/*
	 * 柜台ID数组
	 */
	...
	"getDiff":function(curCounterRow){
		...
		var counterCode = BINBESSPRM05.counterArr[curCounterRow];
        var logicInventoryCode = BINBESSPRM05.logicInventoryInfoArr[curCounterRow];
	        ...
	        success:function(msg){
	            ...
	            if(msg.indexOf('id="actionResultBody"')>-1){
	                ...
	                for(var i=0;i<jsons.length;i++){
						...
						if(row-1 >= BINBESSPRM05.showMaxCount){
							stopShowFlag = true;
							break;
						}else{
		                    appendhtml += '<tr>';
		                    ...
		                    appendhtml += '<td>'+escapeHTMLHandle(jsons[i].promotionName)+'</td>';
                    ...
                    if(nextRow < BINBESSPRM05.counterArr.length){
                    	if(stopShowFlag){
                    		$("#dataTable_processing").addClass("hide");
                    		requestEnd();
                    		//显示超过最大提示
                    		$("#maxShow").html("<ul><li><span>由于促销品库存差异过多，只显示前"+BINBESSPRM05.showMaxCount+"条差异。</span></li></ul>");
                    		$("#errorMessage").show();
                    	}else{
                    		setTimeout(function(){BINBESSPRM05.getDiff(nextRow);},50);
                    	}
					}else{
					   $("#dataTable_processing").addClass("hide");
					   requestEnd();
	...
	"search":function(){
		//清空数据
		$("#aaData").empty();
		BINBESSPRM05.counterArr = new Array();
		BINBESSPRM05.logicInventoryInfoArr = new Array();
		//显示差异数据
	        ...
	        success:function(msg){
				if(msg.length>0){
		            for(var i=0;i<msg.length;i++){
		            	BINBESSPRM05.counterArr.push(msg[i].counterCode);
		            	BINBESSPRM05.logicInventoryInfoArr.push(msg[i].logicInventoryCode);
					}
					//取第一个柜台的差异库存
					BINBESSPRM05.getDiff(0);
				}else{
					$("#dataTable_processing").addClass("hide");
					requestEnd();
				}
...
var BINBESSPRM05 = new BINBESSPRM05();
