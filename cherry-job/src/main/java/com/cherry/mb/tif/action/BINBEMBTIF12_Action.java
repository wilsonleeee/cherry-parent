 ...
 */
public class BINBEMBTIF12_Action extends BaseAction{

	private static final long serialVersionUID = -5995242242541425115L;

	private static Logger logger = LoggerFactory.getLogger(BINBEMBTIF12_Action.class.getName());
	...
	public String binbembarcExec() throws Exception {
		...
		try {
			...
			flg = binBEMBTIF12_BL.tran_addMixMobile(map);
			...
			if(flg == CherryBatchConstants.BATCH_SUCCESS) {
				this.addActionMessage("添加会员天猫加密手机号（巧迪会员通）处理正常终了");
				logger.info("******************************添加会员天猫加密手机号（巧迪会员通）处理正常终了***************************");
			} else if(flg == CherryBatchConstants.BATCH_WARNING) {
				this.addActionError("添加会员天猫加密手机号（巧迪会员通）处理警告终了");
				logger.info("******************************添加会员天猫加密手机号（巧迪会员通）处理警告终了***************************");
			} else if(flg == CherryBatchConstants.BATCH_ERROR) {
				this.addActionError("添加会员天猫加密手机号（巧迪会员通）处理异常终了");
				logger.info("******************************添加会员天猫加密手机号（巧迪会员通）处理异常终了***************************");
			}
		}
		return "DOBATCHRESULT";
	}
