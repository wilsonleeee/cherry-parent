/*	
 * @(#)BINBEMBTIF04_Action.java     1.0 2015/06/24		
 * 		
 ...
 */
public class BINBEMBTIF04_Action extends BaseAction{

	private static final long serialVersionUID = 5575014953894910536L;
...
private static Logger logger = LoggerFactory.getLogger(BINBEMBTIF04_Action.class.getName());
	...
	public String binbembarcExec() throws Exception {
		...
		try {
			...
			flg = binBEMBTIF04_BL.tran_PointChange(map);
			...
			if(flg == CherryBatchConstants.BATCH_SUCCESS) {
				this.addActionMessage("天猫积分兑换同步处理正常终了");
				logger.info("******************************天猫积分兑换同步处理正常终了***************************");
			} else if(flg == CherryBatchConstants.BATCH_WARNING) {
				this.addActionError("天猫积分兑换同步处理警告终了");
				logger.info("******************************天猫积分兑换同步处理警告终了***************************");
			} else if(flg == CherryBatchConstants.BATCH_ERROR) {
				this.addActionError("天猫积分兑换同步处理异常终了");
				logger.info("******************************天猫积分兑换同步处理异常终了***************************");
			}
		}
		return "DOBATCHRESULT";
	}
