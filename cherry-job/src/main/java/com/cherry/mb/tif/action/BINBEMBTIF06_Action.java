/*	
 * @(#)BINBEMBTIF06_Action.java     1.0 2015/06/24		
 * 		
...
/**
 * 天猫历史注册会员转假登录会员处理Action
 * 
 * @author hub
 * @version 1.0 2015/06/24
 */
public class BINBEMBTIF06_Action extends BaseAction{

	private static final long serialVersionUID = -4026926546772535853L;
	
	private static Logger logger = LoggerFactory.getLogger(BINBEMBTIF04_Action.class.getName());
	...
	public String binbembarcExec() throws Exception {
		...
		try {
			...
			flg = binBEMBTIF06_BL.tran_HisRegister(map);
			...
			if(flg == CherryBatchConstants.BATCH_SUCCESS) {
				this.addActionMessage("天猫历史注册会员转假登录会员处理正常终了");
				logger.info("******************************天猫历史注册会员转假登录会员处理正常终了***************************");
			} else if(flg == CherryBatchConstants.BATCH_WARNING) {
				this.addActionError("天猫历史注册会员转假登录会员处理警告终了");
				logger.info("******************************天猫历史注册会员转假登录会员处理警告终了***************************");
			} else if(flg == CherryBatchConstants.BATCH_ERROR) {
				this.addActionError("天猫历史注册会员转假登录会员处理异常终了");
				logger.info("******************************天猫历史注册会员转假登录会员处理异常终了***************************");
			}
		}
		return "DOBATCHRESULT";
	}
