...
//			}
	 ...
	 */
	public List<String> getTicketNumberList(Map<String, Object> map,int count) {
		String orgInfoId = CherryBatchUtil.getString(map.get(CherryBatchConstants.ORGANIZATIONINFOID));
		String brandInfoId = CherryBatchUtil.getString(map.get(CherryBatchConstants.BRANDINFOID));
		Map<String, Object> ticketNumbers = getActivityCodeList(orgInfoId, brandInfoId, "", "D", count);
		if (null != ticketNumbers && !ticketNumbers.isEmpty()) {
			List<String> ticketNumberList = new ArrayList<String>();
			// 最小数
			int minnum = Integer.parseInt(ticketNumbers.get("minnum").toString());
			// 最大数
			int maxnum = Integer.parseInt(ticketNumbers.get("maxnum").toString());
	...
	public List<String> getTicketNumberList(Map<String, Object> map,String type,int count) {
		...
		String businessDate = ConvertUtil.getString(map.get(CherryBatchConstants.BUSINESS_DATE));
