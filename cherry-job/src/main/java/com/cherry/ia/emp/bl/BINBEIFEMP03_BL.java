/*
 * @(#)BINBEIFEMP03_BL.java     1.0 2010/11/12
 * 
...
import com.cherry.ia.emp.service.BINBEIFEMP03_Service;
...
public class BINBEIFEMP03_BL {
	...
	/** 营业员列表导入Service（雅漾用） */
	@Resource
	private BINBEIFEMP03_Service binBEIFEMP03_Service;
	...
	public int tran_batchBaInfo(Map<String, Object> map) throws CherryBatchException {
		...
		while (true) {
			...
			// 从接口数据库中取得营业员信息List
			List<Map<String, Object>> baInfoList = binBEIFEMP03_Service.getBaInfoList(map);
			// 营业员数据不为空
	...
	public void updateBaInfo(List<Map<String, Object>> baInfoList, Map<String, Object> map)
		...
		for (Map<String, Object> baInfoMap : baInfoList) {
			...
			// 校验模式的场合，使用NEC_bainfo表中的BA名称更新新后台的数据库
			if(configValue != null && "1".equals(configValue)) {
				// 取得营业员姓名
				String banameNEC = (String)baInfoMap.get("banameNEC");
				if(banameNEC != null && !"".equals(banameNEC)) {
					baInfoMap.put("baname", banameNEC);
				}
			}
			// 取得营业员姓名
...
//				}
				...
				Object flag = baInfoMap.get("flag");
				if(flag != null && "-1".equals(flag.toString())) {
					baInfoMap.put("validFlagVal", "0");
				} else {
					baInfoMap.put("validFlagVal", "1");
				}
				...
				if(baInfo != null) {
					Object positionCategoryId = baInfo.get("positionCategoryId");
					if(positionCategoryId != null && (posId == null || !positionCategoryId.toString().equals(posId.toString()))) {
						BatchExceptionDTO batchExceptionDTO = new BatchExceptionDTO();
						batchExceptionDTO.setBatchName(this.getClass());
						batchExceptionDTO.setErrorCode("EIF04007");
						batchExceptionDTO.setErrorLevel(CherryBatchConstants.LOGGER_ERROR);
						// 营业员代号
						batchExceptionDTO.addErrorParam(bacode);
						// 营业员姓名
					...
					try {
						...
						String oldHigher = (String)baInfo.get("higher");
						if(oldHigher == null) {
							oldHigher = "/";
						}
						// 营业员上级发生变化的场合更新上级
						if(!oldHigher.equals(higherPath)) {
					...
					try {
						baInfoMap.remove("validFlagVal");
						if(flag != null && "-1".equals(flag.toString())) {
							// 更新员工入退职信息
							binBEIFEMP02_Service.delInvalidEmployeeQuit(baInfoMap);
						} else {
							// 插入信息到员工入退职信息表
