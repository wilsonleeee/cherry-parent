...
import com.cherry.ss.prm.service.BINBESSPRM02_Service;
 ...
 */
public class BINBESSPRM02_BL {

	/** 促销产品月度库存重算SERVICE */
	@Resource
	private BINBESSPRM02_Service binbessprm02Service;
	...
	public int tran_batchHistory(Map<String, Object> map) throws CherryBatchException {
		...
		// 取得业务日期
		String bussinessDate = binbessprm02Service.getBussinessDate(map);
		...
		// 查询离业务日期最近的一个截止日期
		Map<String, Object> lastProStockCloseDay = binbessprm02Service.getLastPrmStockCloseDay(map);
		if(lastProStockCloseDay != null && !lastProStockCloseDay.isEmpty()) {
			map.putAll(lastProStockCloseDay);
			// 判断指定截止日期是否存在月度库存统计信息
			int result = binbessprm02Service.getPrmStockHistoryCount(map);
			if(result == 0) {
				// 月度库存统计处理
				try {
					if(!binbessprm01bl.stockHistoryHandle(map)) {
						BatchLoggerDTO batchLoggerDTO1 = new BatchLoggerDTO();
		...
		// 补录的促销产品入出库日期
		List<Map<String, Object>> stockInOutDateList = binbessprm02Service.getStockInOutDateList(map);
		if(stockInOutDateList != null && !stockInOutDateList.isEmpty()) {
			...
			for(int i = 0; i < stockInOutDateList.size(); i++) {
				...
				try {
					...
					// 更新程序名
					map.put(CherryBatchConstants.UPDATEPGM, "BINBESSPRM02");
					// 把截止计算区分从未反映到历史库存表中状态更新为待处理状态
					binbessprm02Service.updateCloseFlagWait(map);
				} catch (Exception e) {
					failCount++;
					flag = CherryBatchConstants.BATCH_WARNING;
					BatchLoggerDTO batchLoggerDTO1 = new BatchLoggerDTO();
					batchLoggerDTO1.setCode("ESS00007");
				...
				// 查询要更新的截止日期LIST
				List<String> endDateList = binbessprm02Service.getEndDateList(map);
				if (CherryBatchUtil.isBlankList(endDateList)) {
					failCount++;
					flag = CherryBatchConstants.BATCH_WARNING;
					BatchLoggerDTO batchLoggerDTO1 = new BatchLoggerDTO();
					batchLoggerDTO1.setCode("ISS00001");
					batchLoggerDTO1.setLevel(CherryBatchConstants.LOGGER_ERROR);
					// 所属组织
				...
				while (true) {
					...
					// 补录的促销产品入出库数据
					List<Map<String, Object>> inOutList = binbessprm02Service.getInOutList(map);
				...
				if(isTrue) {
					try {
						...
						binbessprm02Service.updateCloseFlagEnd(map);
						// 提交数据
						binbessprm02Service.manualCommit();
					} catch (Exception e) {
						try {
							// 回滚数据
							binbessprm02Service.manualRollback();
						} catch (Exception ex) {
	...
	private void updateStockHistoty(List<Map<String, Object>> inOutList, List<String> endDateList, Map<String, Object> map)
		...
		for (Map<String, Object> inOutMap : inOutList) {
			...
			// 作成程序名
			inOutMap.put(CherryBatchConstants.CREATEPGM, "BINBESSPRM02");
			// 更新者
			inOutMap.put(CherryBatchConstants.UPDATEDBY, CherryBatchConstants.UPDATE_NAME);
			// 更新程序名
			inOutMap.put(CherryBatchConstants.UPDATEPGM, "BINBESSPRM02");
			...
			for (int i = 0; i < endDateList.size(); i++) {
				try {
					...
					re = binbessprm02Service.updateHistory(inOutMap);
				} catch (Exception e) {
					isTrue = false;
					BatchLoggerDTO batchLoggerDTO1 = new BatchLoggerDTO();
					batchLoggerDTO1.setCode("ESS00006");
					batchLoggerDTO1.setLevel(CherryBatchConstants.LOGGER_ERROR);
					// 所属组织
...
//					if(i == 0) {
//						// 查询最近一个截止日期的月度库存信息
//						Map<String, Object> lastHistoryInfo = binbessprm02Service.getLastHistoryInfo(inOutMap);
...
//					}
					try {
						// 插入新纪录到促销产品月度库存表
						binbessprm02Service.insertHistory(inOutMap);
					} catch (Exception e) {
						isTrue = false;
						BatchLoggerDTO batchLoggerDTO1 = new BatchLoggerDTO();
