/*
 * @(#)BINBESSPRO03_Action.java     1.0 2011/09/01
 * 
 ...
 */
public class BINBESSPRO03_Action extends BaseAction {
    ...
    private static Logger logger = LoggerFactory.getLogger(BINBESSPRO03_Action.class.getName());
    ...
    public void getCounter() throws Exception{
        ...
        List<Map<String,Object>> list = binBESSPRO03_BL.getOldCounterInfoList(param);
    ...
    public String getDiff() throws Exception{
        ...
        List<Map<String,Object>> newCounterDepotInfoList = binBESSPRO03_BL.getNewCounterDepotInfoList(param);
        ...
        List<Map<String,Object>> newCounterLogicInfoList = binBESSPRO03_BL.getNewCounterLogicInfoList(param);
        ...
        try{
            List<Map<String,Object>> list = binBESSPRO03_BL.getDiffProductList(map);
            ConvertUtil.setResponseByAjax(response, list);
        }catch(Exception e){
            this.addActionError("获取产品库存失败");
            return "error";
        }
     ...
     * @throws Exception
     */
    public String binBESSPRO03Exec() throws Exception {
        logger.info("******************************BINBESSPRO03处理开始***************************");
        // 设置batch处理标志
        ...
        try {
            ...
            //新后台库存不存在
            map.put("filterStockNotExist", filterStockNotExist);
            flg = binBESSPRO03_BL.tran_sync(map);
            ...
            if (flg == CherryBatchConstants.BATCH_SUCCESS) {
                this.addActionMessage("产品库存同步处理正常终了");
                logger.info("******************************BINBESSPRO03处理正常终了***************************");
            } else if (flg == CherryBatchConstants.BATCH_WARNING) {
                this.addActionError("产品库存同步处理警告终了");
                logger.info("******************************BINBESSPRO03处理警告终了***************************");
            } else if (flg == CherryBatchConstants.BATCH_ERROR) {
                this.addActionError("产品库存同步处理异常终了");
                logger.info("******************************BINBESSPRO03处理异常终了***************************");
            }
        }
    ...
    public void export(){
        ...
        try {
            ...
            String exportName = binBESSPRO04_BL.getResourceValue("BINBESSPRO03", language, "downloadFileName");
            response.setHeader("Content-Disposition", "attachment;filename="
                    + new String(exportName.getBytes("GBK"),"ISO8859-1"));
            byte[] excel = binBESSPRO03_BL.exportExcel(map);
