/*	
 * @(#)BINBEMQSYN01_BL.java     1.0 2010/12/01		
 * 		
...
import com.cherry.mq.syn.service.BINBEMQSYN01_Service;
import com.mongodb.BasicDBList;
import com.mongodb.BasicDBObject;
import com.mongodb.DBObject;

/**
 * MQ同步batch处理BL ，已经废弃不再使用！！！
 * 
 * @author WangCT
 * @version 1.0 2010.11.04
 */
public class BINBEMQSYN01_BL {
	...
	/** MQ同步batch处理Service */
	@Resource
	private BINBEMQSYN01_Service binBEMQSYN01_Service;
	...
	public int tran_receFailMQLogHandle(Map<String, Object> map) throws Exception {
		...
		try {
			...
			binBEMQSYN01_Service.updateMQLogStatusCherry(map);
			// 把POS品牌的MQ发送日志更新成待处理状态
			binBEMQSYN01_Service.updateMQLogStatusPOS(map);
			binBEMQSYN01_Service.manualCommit();
			binBEMQSYN01_Service.witManualCommit();
		} catch (Exception e) {
			flag = CherryBatchConstants.BATCH_WARNING;
			BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
			// 把MQ日志更新成待处理状态时发生错误
		...
		while (true) {
			// 查询POS品牌的MQ发送日志
			List<Map<String, Object>> witposMQLogList = binBEMQSYN01_Service.getWitposMQLogPagingList(map);
			// 查询Cherry的MQ接收日志
			List<Map<String, Object>> cherryMQLogList = binBEMQSYN01_Service.getCherryMQLogPagintList(map);
			// POS品牌的MQ日志存在的场合
			if(witposMQLogList != null && !witposMQLogList.isEmpty()) {
				// POS品牌需要删除的日志
				List<Map<String, Object>> delWitMqLogList = new ArrayList<Map<String,Object>>();
				// Cherry需要删除的日志
				List<Map<String, Object>> delCherryMqLogList = new ArrayList<Map<String,Object>>();
				// POS品牌需要重新发送的日志
				...
				for(int i = 0; i < witposMQLogList.size(); i++) {
					...
					// 业务类型
					String billType = (String)wMQLogMap.get("billType");
					// 修改回数
					int modifyCounts = 0;
					Object modifyCountsObj = wMQLogMap.get("modifyCounts");
					if(modifyCountsObj != null && !"".equals(modifyCountsObj.toString())) {
						modifyCounts = (Integer)modifyCountsObj;
					}
					
					boolean isContain = false;
					// 判断POS品牌的MQ发送日志是否存在Cherry的MQ接收日志中处理
					if(cherryMQLogList != null && !cherryMQLogList.isEmpty()) {
						Map<String, Object> cMqLogMap = cherryMQLogList.get(0);
						String _billCode = (String)cMqLogMap.get("billCode");
						String _billType = (String)cMqLogMap.get("billType");
						int _modifyCounts = 0;
						Object _modifyCountsObj = cMqLogMap.get("modifyCounts");
						if(_modifyCountsObj != null && !"".equals(_modifyCountsObj.toString())) {
							_modifyCounts = (Integer)_modifyCountsObj;
						}
						if(billType.equals(_billType) && billCode.equals(_billCode) && modifyCounts == _modifyCounts) {
							delWitMqLogList.add(wMQLogMap);
							delCherryMqLogList.add(cMqLogMap);
							cherryMQLogList.remove(0);
							isContain = true;
						}
					}
					// 记录接收失败的MQ日志
					if(!isContain) {
						wMQLogMap.put("receiveFlag", CherryBatchConstants.RECEIVEFLAG2);
						updWitMqLogList.add(wMQLogMap);
					}
				}
				...
				if(updWitMqLogList.size() > 0) {
					try {
						// 批量更新POS品牌的MQ日志状态
						binBEMQSYN01_Service.updateWitposMQLog(updWitMqLogList);
						binBEMQSYN01_Service.witManualCommit();
					} catch (Exception e) {
						flag = CherryBatchConstants.BATCH_WARNING;
						BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
						// 把接收失败的数据批量更新成对方接受失败状态时发生错误
						batchLoggerDTO.setCode("EMQ00027");
						batchLoggerDTO.setLevel(CherryBatchConstants.LOGGER_ERROR);
						cherryBatchLogger.BatchLogger(batchLoggerDTO,e);
						break;
					}
				...
				// 删除接收成功的日志处理
				if(delWitMqLogList.size() > 0 && delCherryMqLogList.size() > 0) {
					try {
						// 批量删除cherry数据库MQ收发日志表
						binBEMQSYN01_Service.deleteCherryMQLog(delCherryMqLogList);
						binBEMQSYN01_Service.manualCommit();
					} catch (Exception e) {
						flag = CherryBatchConstants.BATCH_WARNING;
						BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
					...
					try {
						// 批量删除POS品牌数据库MQ日志表
						binBEMQSYN01_Service.deleteWitposMQLog(delWitMqLogList);
						binBEMQSYN01_Service.witManualCommit();
					} catch (Exception e) {
						flag = CherryBatchConstants.BATCH_WARNING;
						BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
		...
		try {
			// 删除cherry数据库多余的接收日志
			binBEMQSYN01_Service.deleteLeftCherryMQLog(map);
			binBEMQSYN01_Service.manualCommit();
		} catch (Exception e) {
			flag = CherryBatchConstants.BATCH_WARNING;
			BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
		...
		// 判断JMSXGroupID字段是否存在
		boolean hasJMSXGroupID = binBEMQSYN01_Service.getJMSXGroupIDCol(map);
		...
		int endNum = 0;
		// 排序字段
		map.put(CherryBatchConstants.SORT_ID, "Txddate ASC, Txdtime ASC, InsertTime ASC");
		while (true) {
			// 删除日志件数
			int delCount = 0;
			// 查询开始位置
			startNum = keepCount + 1;
			// 查询结束位置
			...
			if(hasJMSXGroupID) {
				...
				witposMQLogList = binBEMQSYN01_Service.getWitposMQLogSendGroupIDList(map);
			} else {
				// 查询POS品牌需要重新发送的MQ日志List
				witposMQLogList = binBEMQSYN01_Service.getWitposMQLogSendList(map);
			}
			...
			if(witposMQLogList != null && !witposMQLogList.isEmpty()) {
					...
					if(originalMsg != null && !"".equals(originalMsg)) {
						try {
							...
							if(binBEMQMES99_BL.judgeIfIsRepeatData(mainDataMap)) {
								...
								wMQLogMap.put("organizationInfoId", map.get("organizationInfoId"));
								wMQLogMap.put("brandInfoId", map.get("brandInfoId"));
								delWitMqLogList.add(wMQLogMap);
								BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
								// 重复的MQ接收数据被忽略
				...
				if(delWitMqLogList.size() > 0) {
					try {
						// 删除重复接收的MQ收发日志
						binBEMQSYN01_Service.deleteCherryMQLogRe(delWitMqLogList);
						binBEMQSYN01_Service.manualCommit();
					} catch (Exception e) {
						flag = CherryBatchConstants.BATCH_WARNING;
						BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
				...
				// 未删除日志件数统计
				keepCount = keepCount + dataSize - delCount;
			} else {
				break;
			}
		}
		...
		try {
			...
			binBEMQSYN01_Service.updateWitReceiveFlag(map);
		} catch (Exception e) {
			flag = CherryBatchConstants.BATCH_WARNING;
			BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
	...
	public int tran_sendFailMQLogHandle(Map<String, Object> map) throws Exception {
		...
		while (true) {
			...
			List<Map<String, Object>> cherryMQLogList = binBEMQSYN01_Service.getCherryMQLogPagintList(map);
			// 查询POS品牌数据库MQ日志List
			List<Map<String, Object>> witMQLogList = binBEMQSYN01_Service.getWitposMQLogPagingList(map);
			// cherry的MQ日志存在的场合
			if(cherryMQLogList != null && !cherryMQLogList.isEmpty()) {
				// POS品牌需要删除的日志
				List<Map<String, Object>> delWitMqLogList = new ArrayList<Map<String,Object>>();
				...
				List<Map<String, Object>> delCherryMqLogList = new ArrayList<Map<String,Object>>();
				// Cherry需要重新发送的日志
				List<Map<String, Object>> updCherryMqLogList = new ArrayList<Map<String,Object>>();
				for(int i = 0; i < cherryMQLogList.size(); i++) {
					...
					// 业务类型
					String billType = (String)cMQLogMap.get("billType");
					// 修改回数
					int modifyCounts = 0;
					Object modifyCountsObj = cMQLogMap.get("modifyCounts");
					if(modifyCountsObj != null && !"".equals(modifyCountsObj.toString())) {
						modifyCounts = (Integer)modifyCountsObj;
					}
					boolean isContain = false;
					// 判断Cherry的MQ发送日志是否存在POS品牌的MQ接收日志中处理
					if(witMQLogList != null && !witMQLogList.isEmpty()) {
						Map<String, Object> wMqLogMap = witMQLogList.get(0);
						String _billCode = (String)wMqLogMap.get("billCode");
						String _billType = (String)wMqLogMap.get("billType");
						int _modifyCounts = 0;
						Object _modifyCountsObj = wMqLogMap.get("modifyCounts");
						if(_modifyCountsObj != null && !"".equals(_modifyCountsObj.toString())) {
							_modifyCounts = (Integer)_modifyCountsObj;
						}
						if(billType.equals(_billType) && billCode.equals(_billCode) && modifyCounts == _modifyCounts) {
							delWitMqLogList.add(wMqLogMap);
							delCherryMqLogList.add(cMQLogMap);
							witMQLogList.remove(0);
							isContain = true;
						}
					}
					// 记录接收失败的MQ日志
					if(!isContain) {
						cMQLogMap.put("receiveFlag", CherryBatchConstants.RECEIVEFLAG2);
						updCherryMqLogList.add(cMQLogMap);
					}
				}
				
				if(updCherryMqLogList.size() > 0) {
					try {
						// 把接收失败的数据批量更新成对方接受失败状态
						binBEMQSYN01_Service.updateCherryMQLog(updCherryMqLogList);
						binBEMQSYN01_Service.manualCommit();
						sendMqCount += updCherryMqLogList.size();
					} catch (Exception e) {
						flag = CherryBatchConstants.BATCH_WARNING;
						BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
						// 把接收失败的数据批量更新成对方接受失败状态时发生错误
				...
				// 删除接收成功的日志处理
				if(delCherryMqLogList.size() > 0 && delWitMqLogList.size() > 0) {
					try {
						// 批量删除POS品牌数据库MQ日志表
						binBEMQSYN01_Service.deleteWitposMQLog(delWitMqLogList);
						binBEMQSYN01_Service.witManualCommit();
					...
					successCount += delCherryMqLogList.size();
				}
		...
		try {
			// 删除POS品牌数据库多余的接收日志
			binBEMQSYN01_Service.deleteLeftWitposMQLog(map);
			binBEMQSYN01_Service.witManualCommit();
		} catch (Exception e) {
			flag = CherryBatchConstants.BATCH_WARNING;
			BatchLoggerDTO batchLoggerDTO = new BatchLoggerDTO();
		...
		// 需要重新发送的MQ消息件数
		BatchLoggerDTO batchLoggerDTO2 = new BatchLoggerDTO();
		batchLoggerDTO2.setCode("EMQ00019");
		batchLoggerDTO2.setLevel(CherryBatchConstants.LOGGER_INFO);
		batchLoggerDTO2.addParam(String.valueOf(sendMqCount));
		cherryBatchLogger.BatchLogger(batchLoggerDTO1);
		cherryBatchLogger.BatchLogger(batchLoggerDTO2);
	 ...
	 * @return BATCH处理标志
	 */
	public int tran_sendFailMQLog(Map<String, Object> map) throws Exception {
		...
		int endNum = 0;
		// 排序字段
		map.put(CherryBatchConstants.SORT_ID, "mqLogId");
		while (true) {
			// 查询开始位置
			...
			List<Map<String, Object>> cherryMQLogList = binBEMQSYN01_Service.getCherryMQLogSendList(map);
			// 需要重新发送的MQ日志存在的场合
			if(cherryMQLogList != null && !cherryMQLogList.isEmpty()) {
				for(int i = 0; i < cherryMQLogList.size(); i++) {
					Map<String, Object> cMQLogMap = cherryMQLogList.get(i);
					...
					String billCode = (String)cMQLogMap.get("billCode");
					// Cherry的日志ID
					String cherryId = cMQLogMap.get("mqLogId").toString();
		...
		try {
			...
			binBEMQSYN01_Service.updateCherryReceiveFlag(map);
		} catch (Exception e) {
			BatchExceptionDTO batchExceptionDTO = new BatchExceptionDTO();
			batchExceptionDTO.setBatchName(this.getClass());
			// 更新cherry的MQ收发日志表时发生错误。
			batchExceptionDTO.setErrorCode("EMQ00024");
		...
		// 重新发送接收失败的MQ消息成功件数
		BatchLoggerDTO batchLoggerDTO1 = new BatchLoggerDTO();
		batchLoggerDTO1.setCode("EMQ00018");
		batchLoggerDTO1.setLevel(CherryBatchConstants.LOGGER_INFO);
		batchLoggerDTO1.addParam(String.valueOf(successCount));
		// 重新发送接收失败的MQ消息失败件数
		BatchLoggerDTO batchLoggerDTO2 = new BatchLoggerDTO();
		batchLoggerDTO2.setCode("EMQ00034");
		batchLoggerDTO2.setLevel(CherryBatchConstants.LOGGER_INFO);
		...
		// 处理新后台内部发送失败的MQ消息
		int result = this.sendFailMQLogCherry(map);
