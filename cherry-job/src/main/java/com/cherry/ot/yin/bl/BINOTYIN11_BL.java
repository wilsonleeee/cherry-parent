/*	
 * @(#)BINOTYIN11_BL.java     1.0 @2013-5-02
 * 		
...
import com.cherry.ot.yin.service.BINOTYIN11_Service;
 ...
 * @version  2013-5-02
 */
public class BINOTYIN11_BL {
	
	private static CherryBatchLogger logger = new CherryBatchLogger(BINOTYIN11_BL.class);	

	/** 颖通柜台导入Service */
	@Resource
	private BINOTYIN11_Service binOTYIN11_Service;
	...
	public int tran_batchOTYIN11(Map<String, Object> map)
			throws CherryBatchException {
		...
		while (true) {
			...
			List<Map<String, Object>> counterList = binOTYIN11_Service.getCounterListForOT(paraMap);
	...
	private Map<String, Object> getCounterRegionId() {
		...
		List<Map<String, Object>> HKCounterIDList = binOTYIN11_Service
	...
	private void updateBackEnd(List<Map<String, Object>> counterList,Map<String, Object> map) {
		...
		for (Map<String, Object> counterMap : counterList) {
			...
			try {
				...
				binOTYIN11_Service.manualCommit();
			} catch (CherryBatchException e) {
				binOTYIN11_Service.manualRollback();
				// 记载导入新后台失败的U_SAPCode
	...
	private void updateCounterRef(Map<String, Object> counterMap,Map<String, Object> paramMap)
		...
		// 查询组织结构中的柜台信息
		Map<String, Object> organizationInfo = binOTYIN11_Service.getOrganizationId(counterMap);
		...
		if(organizationInfo == null) {
			try {
				...
				// 查询柜台在组织结构表中的插入位置
				counterMap.put("counterNodeId", binOTYIN11_Service.getCounterNodeId(counterMap));
				// 在组织结构表中插入柜台节点
				organizationId = binOTYIN11_Service.insertCouOrg(counterMap);
				// 所属部门
			...
			try {
				...
				// 取得柜台主管所在部门节点
				String couHeadDepPath = binOTYIN11_Service.getCouHeadDepPath(organizationInfo);
				if(couHeadDepPath != null) {
					if(couHigherPath == null || !couHeadDepPath.equals(couHigherPath)) {
						...
						// 查询柜台在组织结构表中的插入位置
						counterMap.put("couNodeId", binOTYIN11_Service.getCounterNodeId(counterMap));
					}
				}

				// 取得柜台信息(新老后台交互时使用)【增加了柜台地址与柜台电话】
				Map<String, Object> counterInfo = binOTYIN11_Service.getCounterInfo(counterMap);
				...
				// 更新在组织结构中的柜台
				binOTYIN11_Service.updateCouOrg(counterMap);
			} catch (Exception e) {
				BatchExceptionDTO batchExceptionDTO = new BatchExceptionDTO();
				batchExceptionDTO.setBatchName(this.getClass());
		...
		// 查询柜台ID
		Object counterId = binOTYIN11_Service.getCounterId(counterMap);
		// 柜台数据不存在时，插入柜台信息
		if (null == counterId) {
			try {
				// 插入柜台信息
				counterId = binOTYIN11_Service.insertCounterInfo(counterMap);
				// 插入件数加一
				insertCount++;
			} catch (Exception e) {
				BatchExceptionDTO batchExceptionDTO = new BatchExceptionDTO();
			...
			try {
				// 更新柜台信息表
				binOTYIN11_Service.updateCounterInfo(counterMap);
				// 更新件数加一
				updateCount++;
			} catch (Exception e) {
				BatchExceptionDTO batchExceptionDTO = new BatchExceptionDTO();
	...
	public Map<String,Object> getCounterWSMap(Map<String,Object> map) throws Exception{
		...
		// 取得柜台信息(新老后台交互时使用)
		Map<String, Object> counterInfo = binOTYIN11_Service.getCounterInfo(map);
		if(counterInfo != null && !counterInfo.isEmpty()){
			//品牌编码
	...
	private void init(Map<String, Object> map){
		...
		// 系统时间
		String sysDate = binOTYIN11_Service.getSYSDate();
		// 作成日时
	...
	private Map<String, Object> getComMap(Map<String, Object> map) {
		...
		// 更新程序名
		baseMap.put(CherryBatchConstants.UPDATEPGM, "BINOTYIN11");
		// 作成程序名
		baseMap.put(CherryBatchConstants.CREATEPGM, "BINOTYIN11");
		// 作成者
