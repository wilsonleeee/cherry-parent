/*	
 * @(#)BINOTYIN04_BL.java     1.0 @2013-03-18		
 * 		
...
import com.cherry.ot.yin.service.BINOTYIN04_Service;
 ...
 * 
 */
public class BINOTYIN04_BL {

	/** BATCH LOGGER */
	private static CherryBatchLogger logger = new CherryBatchLogger(
			BINOTYIN04_BL.class);

	@Resource(name = "binOTYIN04_Service")
	private BINOTYIN04_Service binOTYIN04_Service;
	 ...
	 */
	public int tran_batchExportPrtReceive(Map<String, Object> map)
			throws CherryBatchException {
		// 查询参数初始化
		...
		while (true) {
			...
			try {
				...
				try {
					...
					preFailAmount = getPrtReceiveAmountBySynch();
				} catch (CherryBatchException cbx1) {
					// 新后台数据源回滚【synchFlag由2->3出现异常的回滚】
					try {
						binOTYIN04_Service.manualRollback();
					} catch (Exception e) {
					...
					batchLoggerDTO1.addParam(PropertiesUtil.getMessage("OTY00003",null));
				...
				// 入库单SynchFlag：1->2【出现异常在catch中进行数据源回滚】
				int expAmount = updatePrtReceiveFlag(synchUpMap, 1);
				// 新后台数据源事务提交
				binOTYIN04_Service.manualCommit();
	...
	private void exportData2OTYin(int preFailAmount)
		...
		// 取得SynchFlag=2[导出处理中]的入库单,SQL中直接查出
		List<Map<String, Object>> receiveList = binOTYIN04_Service
				.getPrtReceiveDetail(searchMap);
		try {
			// step2 : 查询到的入库单导出到颖通接口表中
			if (null != receiveList && !receiveList.isEmpty()) {
				binOTYIN04_Service.insertOTYINIFDbBatch(receiveList);
				binOTYIN04_Service.tpifManualCommit();
			}
			try {
				// step3 : 入库单SynchFlag：2->3
				updatePrtReceiveFlag(comMap, 2);
				// 新后台数据源提交
				binOTYIN04_Service.manualCommit();
			} catch (CherryBatchException cbx1) {
				// 入库单SynchFlag：2->3异常则回滚,此时出现导出成功但flag依旧为2的情况
				try {
					binOTYIN04_Service.manualRollback();
				} catch (Exception e1) {
			...
			try {
				binOTYIN04_Service.tpifManualRollback();
			} catch (Exception e1) {
	 ...
	 * 
	 * @param map
	 * @return int: 剔除已成功导出但synchFlag=2的入库单单据后synchFlag=2的入库单数量
	 * @throws CherryException
	 */
	private int getPrtReceiveAmountBySynch() throws CherryBatchException {
		Map<String, Object> receiveMap = new HashMap<String, Object>();
		receiveMap.putAll(comMap);
		receiveMap.put("synchFlag", SYNCH_FLAG_2);
		// 取得入库单主表中synchFlag=2的入库单List
		List<String> prtReceiveListBySynch = binOTYIN04_Service
				.getPrtReceiveListBySynch(receiveMap);
		// 存在synchFlag=2的单据
		if (null != prtReceiveListBySynch && !prtReceiveListBySynch.isEmpty()) {
			receiveMap.put("receiveNoIFList", prtReceiveListBySynch);
			// 根据新后台的单据号->查询颖通产品入库单接口表的单据号List
			List<String> oTIFListFromPrtReceive = binOTYIN04_Service
					.getOTIFListFromPrtReceive(receiveMap);
			// 颖通接口表中存在synchFlag=2的数据，即导出成功，状态更新不成功
			if (null != oTIFListFromPrtReceive
					&& !oTIFListFromPrtReceive.isEmpty()) {
				Map<String, Object> otyingMap = new HashMap<String, Object>();
				otyingMap.putAll(comMap);
				otyingMap.put("receiveNoIFList", oTIFListFromPrtReceive);
				// 将已经成功导出到颖通接口表中的对应新后台的入库单单据的synchFlag由2置为3（剔除成功导出的入库单数据）
				// 出现异常时将会在下次处理中进行此方法中相同的处理
				updatePrtReceiveFlag(otyingMap, 2);
				// 新后台数据源提交
				binOTYIN04_Service.manualCommit();
			}
			// 去除已经成功导出到颖通接口中的入库单号list
			prtReceiveListBySynch.removeAll(oTIFListFromPrtReceive);
		}
		// 返回synchFlag=2且还未导出到颖通接口中的数据量
		return prtReceiveListBySynch.size();
	}
	 ...
	 * @throws CherryBatchException
	 */
	private int updatePrtReceiveFlag(Map<String, Object> map, int identifyId)
			throws CherryBatchException {
		...
		if (identifyId == 1) {
			...
			return binOTYIN04_Service.updateSynchFlag(updateMap);
	...
	private Map<String, Object> getComMap(Map<String, Object> map) {
		...
		// 更新程序名
		baseMap.put(CherryBatchConstants.UPDATEPGM, "BINOTYIN04");
		// 作成程序名
		baseMap.put(CherryBatchConstants.CREATEPGM, "BINOTYIN04");
		// 作成者
