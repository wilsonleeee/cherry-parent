/*	
 * @(#)BINOTBAT120_BL.java     1.0 @2015-8-13		
 * 		
...
import com.cherry.ot.yin.service.BINBAT120_Service;
import com.cherry.ot.yin.service.BINOTYIN08_Service;
import com.mqhelper.interfaces.MQHelper_IF;

/**
 *
 * 颖通接口：发货单退库单(订货/退货废除)导入BL
 *
 * @author jijw
 *
 * @version  2015-8-13
 */
public class BINBAT120_BL {
	
	private static CherryBatchLogger logger = new CherryBatchLogger(BINBAT120_BL.class);	
	@Resource
	private BINBAT120_Service binOTBAT120_Service;
	 ...
	 */
	public int tran_batchExec(Map<String, Object> map) throws CherryBatchException ,Exception{
    	// 初始化
    	try{
	        init(map);
    	}catch(Exception e){
		...
		while (true) {
			...
			List<String> docEntryList = binOTBAT120_Service.getDocEntryListForOT(trxStatusNullMap);
	...
	private void updExpTrans(List<String> docEntryList) throws CherryBatchException {
		...
		for(String docEntry : docEntryList){
			try {
				...
				binOTBAT120_Service.updTrxStatusForOT(paraMap);
				
				// Step2: 取得颖通发货单接口表物理数据(及根据接口表ItemCode取得新后台产品的相关属性)
				paraMap.put("trxStatus", trxStatus_1);
				List<Map<String,Object>> exportTransListForOT = binOTBAT120_Service.getExportTransListForOT(paraMap);
				...
				for(Map<String,Object> exportTransMapForOT : exportTransListForOT){
					...
					Map<String,Object> priceMap = binOTBAT120_Service.getProPrmPriceByUB(ppPriceMap);
				...
				binOTBAT120_Service.updTrxStatusForOT(t1ToT2Map);
				
				//Step5: 调用MQHelper接口进行数据发送
				mqHelperImpl.sendData(propSendMQMap, "posToCherryMsgQueue");
				
				//事务提交
				binOTBAT120_Service.witManualCommit();
				binOTBAT120_Service.tpifManualCommit();
				
			} catch(Exception ex){
				try{
					// 颖通数据源回滚
					binOTBAT120_Service.tpifManualRollback();
					//MQ_Log表中事物回滚
					binOTBAT120_Service.witManualRollback();
				} catch(Exception exx){
	...
	private Map<String, Object> getComMap(Map<String, Object> map) {
		...
		// 更新程序名
		baseMap.put(CherryBatchConstants.UPDATEPGM, "BINBAT120");
		// 作成程序名
		baseMap.put(CherryBatchConstants.CREATEPGM, "BINBAT120");
		// 作成者
		...
		// 品牌Code
		String branCode = binOTBAT120_Service.getBrandCode(map);
		// 品牌Code
		baseMap.put(CherryConstants.BRAND_CODE, branCode);
		
		// 业务日期，日结标志
		Map<String, Object> bussDateMap = binOTBAT120_Service.getBussinessDateMap(map);
		// 业务日期
	...
	private void outMessage() throws CherryBatchException {
		...
		if(!CherryBatchUtil.isBlankList(falidDocEntryList)){
			...
			fReason = "发货单退库单(订货/退货废除)导入部分数据处理失败，具体见log日志";
		}else{
	...
	public int tran_batchOTYIN01_Old(Map<String, Object> map)
		...
		while (true) {
			...
			try {
				...
				int upCountSucc = binOTBAT120_Service.updTrxStatusNullTo1ForOT(tNullToT1Map);
	...
	private List<Map<String,Object>> getExportTransListForOT(Map<String, Object> map) throws CherryBatchException{
		List<Map<String,Object>> exportTransListForOT = binOTBAT120_Service.getExportTransListForOT(map);
...
//			paraMap.put("barCode", barCode);
			Map<String,Object> priceMap = binOTBAT120_Service.getProPrmPriceByUB(paraMap);
