/*	
 * @(#)BINBAT122_BL.java     1.0 @2016-09-26
 * 		
...
public class BINBAT122_BL {
	...
	private static Logger logger = LoggerFactory.getLogger(BINBAT122_BL.class.getName());
...
//	}
	...
	private void updateESOrderDetail(Map<String, Object> orderMainMap, List<Map<String, Object>> orderDetailList) {
		if (null != orderDetailList && !orderDetailList.isEmpty()) {
			// 查找订单明细表
			List<Map<String, Object>> preDetailList = binbat122_Service.getESOrderDetail(orderMainMap);
			if (null != preDetailList && !preDetailList.isEmpty()) {
				...
				// 更新订单明细表
				if (!upList.isEmpty()) {
					binbat122_Service.updateESOrderDetail(upList);
				}
			}
		}
	}
	...
	private int addESOrderMain(Map<String, Object> orderMainMap, Map<String, Object> ordersMap, Map<String, Object> params, Map<String, Object> searchMap) {
		...
		//单据交易状态
		orderMainMap.put("BillState", binbat122_Service.convertOrderStatus(orderStatus));
		// 业务类型
        ...
        //收货人地址
		orderMainMap.put("ConsigneeAddress", binbat122_Service.subStringByLimit(
				(String) ordersMap.get("ReceiverAddress"), 200));
        //买家留言
		orderMainMap.put("BuyerMessage", binbat122_Service.subStringByLimit(
				(String) ordersMap.get("BuyerMessage"), 300));
        //卖家备注
		orderMainMap.put("SellerMemo", binbat122_Service.subStringByLimit(
