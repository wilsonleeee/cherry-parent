...
public class OSWorkflowJob implements ApplicationContextAware{
	
	private static Logger logger = LoggerFactory
	.getLogger(OSWorkflowJob.class.getName());
	...
	public void work() {
		...
		try {
			...
			KnowledgeEngine knowledgeEngine = (KnowledgeEngine) applicationContext.getBean("knowledgeEngine");
			int freshFlag = 0;
			try{
				// 刷新工作流文件
			...
			if (null != confBrandList) {
				for (Map<String, Object> brandInfo : confBrandList) {
					...
					try {
						...
						checkMap.put("fileCode", "CherryBatchFlow");
					...
					try {
						...
						// 获取业务日期
						brandInfo.put("BUSSINESS_DATE", binOLCM06_BL.getBussinessDate(brandInfo));
						// 组织ID
						...
						// 刷新工作流文件标识
						brandInfo.put("freshFlag", freshFlag);
						String wf_name = ConvertUtil.getWfName(orgCode, brandCode, "CherryBatchFlow");
