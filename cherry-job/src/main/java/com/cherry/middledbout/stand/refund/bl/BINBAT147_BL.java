/*	
 * @(#)BINBAT147_BL.java     1.0 @2015-12-22		
 * 		
...
import com.cherry.middledbout.stand.refund.service.BINBAT147_Service;
 ...
 * 
 */
public class BINBAT147_BL {

	/** BATCH LOGGER */
	private static CherryBatchLogger logger = new CherryBatchLogger(BINBAT147_BL.class);

	@Resource(name = "binbat147_Service")
	private BINBAT147_Service binbat147_Service;
	 ...
	 */
	public int tran_binbat147(Map<String, Object> map) throws Exception {
    	try{
    		//参数初始化
	        init(map);
    	}catch(Exception e){
		...
		while (true) {
			...
			try {
				...
				// 新后台数据源提交
				binbat147_Service.manualCommit();
				...
				try {
					//新后台数据源回滚
					binbat147_Service.manualRollback();
				} catch (Exception ex) {
				...
				batchLoggerDTO1.addParam(PropertiesUtil.getMessage("BAT147",null));
				batchLoggerDTO1.addParam(PropertiesUtil.getMessage("BAT147",null));
				logger.BatchLogger(batchLoggerDTO1);
				CherryBatchLogger cherryBatchLogger = new CherryBatchLogger(this.getClass());
				cherryBatchLogger.BatchLogger(batchLoggerDTO1, e);
				fReason = String.format("退库确认单数据同步状态从【SynchFlag=null--->SynchFlag=1】时更新失败,详细信息查看Log日志", e.getMessage());
				break;
			}
		}
		programEnd(map);
	...
	private void exportDataToIFOutStorage(int preFailAmount,Map<String,Object> map) throws CherryBatchException {
		...
		List<Map<String, Object>> proReturnReqList = binbat147_Service.getProReturnReqList(map);
		if(!CherryBatchUtil.isBlankList(proReturnReqList)){
			try {
				...
				//插入标准退库确认主单接口表
				binbat147_Service.insertIFOutStorage(proReturnReqList);
				//查询新后台退库确认单据同步状态为【SynchFlag=1】的明细数据，SQL中直接查出
				List<Map<String,Object>> proReturnReqDetailList = binbat147_Service.getProReturnReqDetailList(map);
				if(!CherryBatchUtil.isBlankList(proReturnReqDetailList)){
					//插入标准退库确认明细接口表
					binbat147_Service.insertIFOutStorageDetail(proReturnReqDetailList);
				}
				//第三方数据源提交
				binbat147_Service.tpifManualCommit();
				...
				try {
					//第三方数据源回滚
					binbat147_Service.tpifManualRollback();
					//新后台数据源回滚
					binbat147_Service.manualRollback();
				} catch (Exception ex) {
				...
				CherryBatchLogger cherryBatchLogger = new CherryBatchLogger(this.getClass());
				cherryBatchLogger.BatchLogger(batchLoggerDTO1, e);
				fReason = String.format("新后台退库确认单据导出至标准接口表时失败，详情信息查看Log日志", e.getMessage());
			}
		}
	}
	...
	private int getProReturnReqCountBySynch(Map<String,Object> map) throws CherryBatchException {
		...
		List<String> proReturnReqList = binbat147_Service.getProReturnReqListByBillNoIF(map);
		if (!CherryBatchUtil.isBlankList(proReturnReqList)) {
			...
			List<String> existsProReturnReqList = binbat147_Service.getExistsProReturnReqList(updateSynchMap);
			...
			if (!CherryBatchUtil.isBlankList(existsProReturnReqList)) {
				try {
					...
					CherryBatchLogger cherryBatchLogger = new CherryBatchLogger(this.getClass());
					cherryBatchLogger.BatchLogger(batchLoggerDTO1, e);
					fReason = String.format("退库确认单据更新同步状态【SynchFalg=1----->SynchFlag=2】时更新失败,详细信息查看Log日志", e.getMessage());
				}
			}
	...
	private int updateReturnReqFlag(Map<String, Object> map, int identifyId) throws CherryBatchException {
		...
		if (identifyId == 1) {
			...
			return binbat147_Service.updateSynchFlag(updateMap);
	...
	private void init(Map<String, Object> map) {
		...
		map.put("JobCode", "BAT147");

		// 程序【开始运行时间】
		String runStartTime = binbat147_Service.getSYSDateTime();
		// 作成日时
		map.put("RunStartTime", runStartTime);
		// 更新程序名
		map.put(CherryBatchConstants.UPDATEPGM, "BINBAT147");
		// 作成程序名
		map.put(CherryBatchConstants.CREATEPGM, "BINBAT147");
		// 作成者
