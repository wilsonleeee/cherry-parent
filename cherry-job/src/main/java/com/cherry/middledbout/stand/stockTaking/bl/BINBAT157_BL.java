/*	
 * @(#)BINBAT157_BL.java     1.0 @2015-12-16		
 * 		
...
import com.cherry.middledbout.stand.stockTaking.service.BINBAT157_Service;
 ...
 * @version  2015-12-16
 */
public class BINBAT157_BL {
	
	private static CherryBatchLogger logger = new CherryBatchLogger(BINBAT157_BL.class);	
	@Resource
	private BINBAT157_Service binBAT157_Service;
	...
	public int tran_batchExec(Map<String, Object> map)
		...
		map.put("JobCode", "BAT157");
		comMap.put("JobCode", "BAT157");
		// 程序【开始运行时间】
		String runStartTime = binBAT157_Service.getSYSDateTime();
		// 作成日时
		...
		while (true) {
			...
			int prepFailCount = 0;
			try {
				// Step1:查看新后台盘点申请数据中是否存在处于同步中且未导入到标准盘点申请接口表的数据（synchFlag=2)
				int srCountBySync = getStockTakingCountBySynch();
				
				// Step1:查看新后台产品盘点申请主数据中是否存在处于同步中（synchFlag=2）或可同步（synchFlag=1）的数据
				// 更新新后台产品盘点申请主数据从[synchFlag=1或者sycnFlag = null]更新为[synchFlag=2]
				...
				int upCountSucc = binBAT157_Service.updProductStockTakingRequestBySync(s1ToS2Map);
				
				// 新后台数据源commit
				binBAT157_Service.manualCommit();
				...
				// (Step2、Step3)取得新后台同步状态为"同步处理中"[synchFlag=2]的产品盘点申请主数据插入到标准盘点申请单接口表
				...
				// 新后台数据源回滚
				binBAT157_Service.manualRollback();
				// 失败件数
	...
	/**
	 * 查看新后台盘点申请数据中是否存在处于同步中（synchFlag=2）
	 * 若存在，则查询其是否已导出到标准盘点申请接口表（是：将新后台的同步状态由同步中[synchFlag=2]改为已完成[synchFlag=3]）
	 * @return
	 * @throws CherryBatchException 
	 */
	...
	private int getStockTakingCountBySynch() throws CherryBatchException{
		...
		// 取得新后台[synchFlag=2]的盘点申请单据号List
		Map<String, Object> synch2Map = new HashMap<String, Object>(); // [同步中]的盘点申请单据数据
		synch2Map.putAll(comMap);
		synch2Map.put("synchFlag", SYNCH_FLAG_2);
		List<String> stNoIFOfBackENDList = binBAT157_Service.getSTNoIFOfPrtStockTakingRequestListBySync(synch2Map);
		
		if(null != stNoIFOfBackENDList && !stNoIFOfBackENDList.isEmpty()){
			// 根据新后台[synchFlag=2]的产品盘点申请单据号List查询标准盘点申请接口表是否存在（存在则说明新后台相应盘点申请单数据已导出到标准盘点盘点申请单接口表）
			Map<String,Object> bcMap = new HashMap<String, Object>();
			bcMap.put("stNoIFList", stNoIFOfBackENDList);
			bcMap.putAll(comMap);
			List<String> stNoIFListOfOT = binBAT157_Service.getPstrqListBySTNoIFForOT(bcMap);
			
			// 将已导出到标准盘点申请接口的单据号作为条件更新新后台的同步状态([synchFlag=2]更新为[synchFlag=3])
			if(null != stNoIFListOfOT && !stNoIFListOfOT.isEmpty()){
				...
				binBAT157_Service.updProductStockTakingRequestBySync(upTo3Map);
				// 新后台数据源Commit
				binBAT157_Service.manualCommit();
			}
			
			// 计算去除掉在标准接口上存在并更新成[synchFlag=3]的数据后还剩余为[synchFlag=2]的盘点申请单数据
	...
	/**
	 * →产品盘点申请明细表的导入到标准盘点申请接口表
	 * →更新之前进行的盘点申请数据从[synchFlag=2]更新为[synchFlag=3]
	 * 
	 * @throws CherryBatchException
	 */
	private void insertOTStkake(int prepFailCount) throws CherryBatchException{
		...
		try {
			// Step2:
			// 取得新后台同步状态为"同步处理中"[synchFlag=2]产品盘点申请数据（主数据、明细数据）导出到标准盘点申请单接口表
			Map<String, Object> map = new HashMap<String, Object>();
			map.putAll(comMap);
			map.put("synchFlag", SYNCH_FLAG_2);
			// 盘点申请单主表
			List<Map<String,Object>> stkakeListBySynchFlagList = binBAT157_Service.getProductStockRequestListBySyncFlag(map);
			if(null != stkakeListBySynchFlagList && !stkakeListBySynchFlagList.isEmpty()){
				...
				// 盘点申请明细
				List<Map<String,Object>> stkakeDetailListBySynchFlagList = binBAT157_Service.getProductStockRequestDetailListBySyncFlag(map);
				// 插入盘点申请单接口表
				binBAT157_Service.insertIF_StockTaking(stkakeListBySynchFlagList);
				binBAT157_Service.insertIF_StockTakingDetail(stkakeDetailListBySynchFlagList);
				// 标准数据源commit
				binBAT157_Service.tpifManualCommit();
			}			
			try {
				// Step3 盘点申请数据从[synchFlag=2]更新为[synchFlag=3]
				...
				binBAT157_Service.updProductStockTakingRequestBySync(s2ToS3Map);
			...
			try{
				// 标准数据源回滚
				binBAT157_Service.tpifManualRollback();
			} catch(Exception e2){
	...
	private Map<String, Object> getComMap(Map<String, Object> map) {
		...
		// 更新程序名
		baseMap.put(CherryBatchConstants.UPDATEPGM, "BINBAT157_BL");
		// 作成程序名
		baseMap.put(CherryBatchConstants.CREATEPGM, "BINBAT157_BL");
		// 作成者
		...
		// 业务日期，日结标志
		Map<String, Object> bussDateMap = binBAT157_Service.getBussinessDateMap(map);
		// 业务日期
	...
	private void outMessage() throws CherryBatchException {
		...
		if(totalCount!=0){
			if(failCount!=0){
				fReason="柜台盘点申请单据导出到标准接口失败，详情请查看log日志！";
			}
		}
	}
