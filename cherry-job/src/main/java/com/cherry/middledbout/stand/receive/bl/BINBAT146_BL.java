...
package com.cherry.middledbout.stand.receive.bl;
...
import com.cherry.middledbout.stand.receive.service.BINBAT146_Service;
 ...
 * 
 */
public class BINBAT146_BL {

	/** BATCH LOGGER */
	private static CherryBatchLogger logger = new CherryBatchLogger(BINBAT146_BL.class);

	
	@Resource(name = "binBAT146_Service")
	private BINBAT146_Service binBAT146_Service;
	 ...
	 */
	public int tran_binBAT146(Map<String, Object> map) throws Exception {
    	try{
    		//参数初始化
	        init(map);
    	}catch(Exception e){
		...
		while (true) {
			...
			try {
				...
				// 检索新后台收货单据同步状态为【SynchFlag=2】的数据量
				preFailAmount = getProductReceiveBySynch(map);
				// 查看新后台中是否存在可导出【synchFlag=1】或导出处理中【synchFlag=2】的数据
				map.put("batchSize", BATCH_SIZE);
				
				// 更改新后台收货单据同步状态【SynchFlag=1或SynchFlag=null---->SynchFlag=2】
				int expAmount = updateProReceiveFlag(map, 1);
				// 新后台数据源提交
				binBAT146_Service.manualCommit();
				...
				// 将收货单据数据导出到标准接口表中并将同步状态更改为【SynchFlag=3】
				...
				try {
					//新后台数据源回滚
					binBAT146_Service.manualRollback();
				} catch (Exception ex) {
				...
				batchLoggerDTO1.addParam(PropertiesUtil.getMessage("BAT146",null));
				batchLoggerDTO1.addParam(PropertiesUtil.getMessage("BAT146",null));
				logger.BatchLogger(batchLoggerDTO1);
				CherryBatchLogger cherryBatchLogger = new CherryBatchLogger(this.getClass());
				cherryBatchLogger.BatchLogger(batchLoggerDTO1, e);
				fReason = String.format("收货单据数据同步状态从【SynchFlag=1--->SynchFlag=2】时更新失败,详细信息查看Log日志", e.getMessage());
				break;
			}
		}
		programEnd(map);
	...
	private void exportDataToIFInStorage(int preFailAmount,Map<String,Object> map) throws CherryBatchException {
		// 查询新后台收货单据同步状态为【SynchFlag=2】的主单数据，SQL中直接查出
		List<Map<String,Object>> proReceiveList = binBAT146_Service.getProductReceiveList(map);
		if(!CherryBatchUtil.isBlankList(proReceiveList)){
			try {
				String[] proReceiveIDArr = new String[proReceiveList.size()];
				for(int i =0;i<proReceiveList.size();i++){
					proReceiveIDArr[i]=ConvertUtil.getString(proReceiveList.get(i).get("proReceiveId"));
				}
				map.put("proReceiveIDArr", proReceiveIDArr);
				//插入标准入库主单接口表
				binBAT146_Service.insertIFInStorage(proReceiveList);
				//查询新后台收货单据同步状态为【SynchFlag=2】的明细数据，SQL中直接查出
				List<Map<String,Object>> proReceiveDetailList = binBAT146_Service.getProductReceiveDetailList(map);
				if(!CherryBatchUtil.isBlankList(proReceiveDetailList)){
					//插入标准收货明细接口表
					binBAT146_Service.insertIFInStorageDetail(proReceiveDetailList);
				}
				//第三方数据源提交
				binBAT146_Service.tpifManualCommit();
				
				updateProReceiveFlag(map, 2);
				//新后台数据源提交
				binBAT146_Service.manualCommit();
				...
				try {
					//第三方数据源回滚
					binBAT146_Service.tpifManualRollback();
					//新后台数据源回滚
					binBAT146_Service.manualRollback();
				} catch (Exception ex) {
					logger.outExceptionLog(ex);
				...
				CherryBatchLogger cherryBatchLogger = new CherryBatchLogger(this.getClass());
				cherryBatchLogger.BatchLogger(batchLoggerDTO1, e);
				fReason = String.format("新后台收货单据导出至标准接口表时失败，详情信息查看Log日志", e.getMessage());
			}
		}
	}
	 ...
	 * 
	 * @param map
	 * @return int: 未成功导出至标准接口表的收货单据同步状态为【SynchFlag=2】的数据量
	 * @throws CherryException
	 */
	private int getProductReceiveBySynch(Map<String,Object> map) throws CherryBatchException {
		// 取得收货单据主表中同步状态为【SynchFlag=2】的收货单据
		map.put("synchFlag", SYNCH_FLAG_2);
		List<String> proReceiveList =  binBAT146_Service.getProductReceiveListByBillNoIF(map);
		if (!CherryBatchUtil.isBlankList(proReceiveList)) {
			Map<String,Object> updateSynchMap = new HashMap<String, Object>();
			updateSynchMap.putAll(map);
			updateSynchMap.put("billNoIFList", proReceiveList);
			//根据新后台已存在的单号查询单号数据在标准接口表中是否存在
			List<String> existsReceiveList = binBAT146_Service.getExistsProductReceiveList(updateSynchMap);
			// 标准接口表中存在synchFlag=2的数据，即导出成功，状态更新不成功
			if (!CherryBatchUtil.isBlankList(existsReceiveList)) {
				try {
					updateSynchMap.put("billNoIFList", existsReceiveList);
					//更新已在标准接口表存在的收货单据的同步状态【SynchFlag=3】
					updateProReceiveFlag(updateSynchMap, 2);
					// 新后台数据源提交
					binBAT146_Service.manualCommit();
				} catch (Exception e) {
					try {
					...
					batchLoggerDTO1.setLevel(CherryBatchConstants.LOGGER_ERROR);
					batchLoggerDTO1.addParam(PropertiesUtil.getMessage("BINBAT146",null));
					batchLoggerDTO1.addParam(PropertiesUtil.getMessage("BINBAT146",null));
					logger.BatchLogger(batchLoggerDTO1);
					CherryBatchLogger cherryBatchLogger = new CherryBatchLogger(this.getClass());
					cherryBatchLogger.BatchLogger(batchLoggerDTO1, e);
					fReason = String.format("收货单据更新同步状态【SynchFalg=2----->SynchFlag=3】时更新失败,详细信息查看Log日志", e.getMessage());
				}
			}
			//去除已经成功导出到标准接口表中的收货申请单号list
			proReceiveList.removeAll(existsReceiveList);
		}
		// 返回synchFlag=2且还未导出到标准接口表中的数据量
		return proReceiveList.size();
	}
	 ...
	 * @throws CherryBatchException
	 */
	private int updateProReceiveFlag(Map<String, Object> map, int identifyId) throws CherryBatchException {
		Map<String, Object> updateMap = new HashMap<String, Object>();
		updateMap.putAll(map);
		if (identifyId == 1) {
			// 导出状态由可导出（SynchFlag：1或SynchFlag: NULL）设置为导出处理中（SynchFlag：2）
			updateMap.put("synchFlag_Old", SYNCH_FLAG_1);
			updateMap.put("synchFlag_New", SYNCH_FLAG_2);
			updateMap.put("identitySynch", 1);
			return binBAT146_Service.updateSynchFlag(updateMap);
	...
	private void init(Map<String, Object> map) {
		...
		map.put("JobCode", "BAT146");

		// 程序【开始运行时间】
		String runStartTime = binBAT146_Service.getSYSDateTime();
		// 作成日时
		map.put("RunStartTime", runStartTime);
		// 更新程序名
		map.put(CherryBatchConstants.UPDATEPGM, "BINBAT146");
		// 作成程序名
		map.put(CherryBatchConstants.CREATEPGM, "BINBAT146");
		// 作成者
