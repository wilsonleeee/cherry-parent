/*	
 * @(#)BINOLBSFAC03_Action.java     1.0 2011/02/22	
 * 		
...
import com.cherry.bs.fac.bl.BINOLBSFAC03_BL;
import com.cherry.bs.fac.form.BINOLBSFAC03_Form;
...
public class BINOLBSFAC03_Action extends BaseAction implements
		ModelDriven<BINOLBSFAC03_Form> {

	private static final long serialVersionUID = 5248665143372071946L;

	/** 参数FORM */
	private BINOLBSFAC03_Form form = new BINOLBSFAC03_Form();
	...
	@Resource
	private BINOLBSFAC03_BL binolbsfac03BL;
	...
	@Override
	public BINOLBSFAC03_Form getModel() {
		return form;
	}
	...
	public String init() throws Exception {
		...
		// 取得厂商基本信息
		factoryInfo = binolbsfac03BL.getFacInfo(map);
		// 取得厂商地址List
		addList = binolbsfac03BL.getAddList(map);
		return SUCCESS;
	}
	...
	public String update() throws Exception {
		...
		// 更新厂商数据
		binolbsfac03BL.tran_updateFactory(map);
		// 处理成功
	...
	private boolean validateSave(Map<String, Object> map) throws Exception {
		...
		if (CherryChecker.isNullOrEmpty(manufacturerCode)) {
			...
			// 厂商ID取得
			String factoryId = binolbsfac03BL.getFactoryId(map);
			// 厂商ID
			String manufacturerInfoId = (String) map.get("manufacturerInfoId");
			if (!CherryChecker.isNullOrEmpty(factoryId)
					&& !factoryId.equals(manufacturerInfoId)) {
				// 厂商编号重复验证
		...
		if (!CherryChecker.isNullOrEmpty(legalPerson)
			...
			this.addFieldError("legalPerson", getText("ECM00020", new String[] {
					getText("PBS00043"), "30" }));
			validResult = false;
		}
		// 联系电话入力验证
