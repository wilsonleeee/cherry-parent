...
public class BINOLCM14_BL {
	 ...
	 * @return
	 */
	@Cacheable(value="CherrySysConfCache")
	public String getConfigValue(String code, String organizationInfoID,String brandInfoID){
		...
		if(list!=null&&list.size()>0){
			ret = ConvertUtil.getString(list.get(0).get("ConfigValue"));
			return ret;
		}
	...
	/**
	 * 根据分组ID取得有效的系统配置项，将查询出的结果ConfigCode的升序组装到List<Map>中，并且Map中将DetailNo作为key，将相应的ConfigValue做为value
	 * 
	 * 
	 * */
	@Cacheable(value="CherrySysConfCache")
	public List getConfigValueByGroupNo(int groupNo,String organizationInfoID,String brandInfoID){
		List retList = new ArrayList();
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("GroupNo", groupNo);
		map.put("BIN_OrganizationInfoID", organizationInfoID);
		map.put("BIN_BrandInfoID", brandInfoID);
		List<Map<String, Object>> list = binOLCM14_Service.getConfigValueByGroupNo(map);
		if(null == list || list.size() == 0){
			map.put("BIN_OrganizationInfoID", "-9999");
			map.put("BIN_BrandInfoID", "-9999");
			list = binOLCM14_Service.getConfigValueByGroupNo(map);
		}
		if(null != list && list.size() != 0){
			for(int j=0 ; j < list.size() ; j++){
				Map<String,Object> map1 = new HashMap<String,Object>();
				Map<String,Object> temp = list.get(j);
				String configCode = (String)temp.get("ConfigCode");
				map1.put("ConfigCode", configCode);
				for(int i=0 ; i < list.size() ; i++){
					Map<String,Object> temp1 = list.get(i);
					String configCode1 = (String)temp1.get("ConfigCode");
					if(configCode1.equals(configCode)){
						map1.put(String.valueOf(temp1.get("DetailNo")), temp1.get("ConfigValue"));
						list.remove(i);
						i--;
					}
				}
	...
	public List getConfigValByGroup(int groupNo,int organizationInfoID,int brandInfoID){
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("GroupNo", groupNo);
		map.put("BIN_OrganizationInfoID", organizationInfoID);
		map.put("BIN_BrandInfoID", brandInfoID);
	 ...
	 * @return
	 */
	@Cacheable(value="CherrySysConfCache2")
	 ...
	 */
	 public boolean isAuditOpen(String code,String value,String organizationInfoID,String brandInfoID){
		...
		if(list!=null&&list.size()>0){
			ret = ConvertUtil.getString(list.get(0).get("ConfigEfficient"));
	 ...
	 */
	@Cacheable(value="CherrySysConfCache")
	public String getWebposConfigValue(String code, String organizationInfoID,String brandInfoID){
		String ret="";
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("ConfigCode", code);
		map.put("BIN_OrganizationInfoID", organizationInfoID);
		map.put("BIN_BrandInfoID", brandInfoID);
		List<Map<String, Object>> list = binOLCM14_Service.getWebposConfigValue(map);
		...
		map.put("BIN_OrganizationInfoID", "-9999");
		map.put("BIN_BrandInfoID", "-9999");
		list = binOLCM14_Service.getWebposConfigValue(map);
		if(list!=null&&list.size()>0){
			ret = ConvertUtil.getString(list.get(0).get("ConfigValue"));			
		}
		return ret;
