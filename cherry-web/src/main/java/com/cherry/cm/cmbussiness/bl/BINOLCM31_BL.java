...
public class BINOLCM31_BL implements BINOLCM31_IF {
     ...
     * @param ticketDate  单据时间
     * @param memberLevel 当前会员等级
     * @param reCalcDate  重算日期（非重算调用设为null）
     * @param memberId    会员ID
     * @return Map
     ...
     * 取得系统默认等级
     *
     * @param map organizationInfoID: 所属组织ID
     *            brandInfoID: 所属品牌ID
     * @return int
     * 系统默认等级
     */
    ...
    public Map<String, Object> getComEmployeeInfoById(Map<String, Object> map) {
        // 取得员工信息
        return binOLCM31_Service.getComEmployeeInfoById(map);
    }
     ...
     * @param memberInfoId 会员信息ID
     * @param memCode      会员卡号
     * @param curPoint     当前总积分
     * @param memberClubId 会员俱乐部ID
     * @return double
     * 重算后的前卡积分
     */
    @Override
    ...
    public void sendMEMQMsg(Map<String, Object> map) throws Exception {
        ...
        // 微信账号
        detailDataMap.put("WechatAccount", ConvertUtil.getString(map.get("wechatAccount")));
        // 职业
        detailDataMap.put("Profession", ConvertUtil.getString(map.get("profession")));
        // 最佳联络时间
        detailDataMap.put("ConnectTime", ConvertUtil.getString(map.get("connectTime")));
        // 收入
     ...
     * 取得所有等级变化明细
     *
     * @param campBaseDTO     会员实体
     * @param levelDetailList 等级变化明细
     * @return List
     * 所有等级变化明细
     * @throws Exception
     */
     ...
     * @return
     */
    public Map<String, Object> getMemInfoByCompleteRule(String organizationInfoId, String brandInfoId, String memCode) {
        Map<String, Object> ruleInfo_map = getMemCompleteRule(organizationInfoId, brandInfoId);
        return getMemInfoByCompleteRule(organizationInfoId, brandInfoId, memCode, ruleInfo_map);
    ...
    public List<String> getMemCompleteRuleKey(String organizationInfoId, String brandInfoId) {
        Map<String, Object> ruleInfo_map = getMemCompleteRule(organizationInfoId, brandInfoId);
        return getMemCompleteRuleKey(organizationInfoId, brandInfoId, ruleInfo_map);
    }
    ...
    public int calMemCompletePercent(String organizationInfoId, String brandInfoId, Map<String, Object> memInfo) {
        Map<String, Object> ruleInfo_map = getMemCompleteRule(organizationInfoId, brandInfoId);
        return calMemCompletePercent(organizationInfoId, brandInfoId, memInfo, ruleInfo_map);
    }
    ...
    public Map<String, Object> calMemCompletePoint(String organizationInfoId, String brandInfoId, Map<String, Object> new_memInfo, String memberCode) {
        Map<String, Object> ruleInfo_map = getMemCompleteRule(organizationInfoId, brandInfoId);
        return calMemCompletePoint(organizationInfoId, brandInfoId, new_memInfo, memberCode, ruleInfo_map);
    }
    ...
    public void updateCompletePercentByMemcode(String organizationInfoId, String brandInfoId, String memberCode, int percent) {
        ...
        Map<String, Object> memInfo = binOLCM31_Service.getMemInfoId(param);
        if (!CherryChecker.isNullOrEmpty(memInfo)) {
            memInfo.putAll(param);
            binOLCM31_Service.updateCompleteByMemcode(memInfo);
        }
    }
    ...
    public void updateCompletePointByMemcode(String organizationInfoId, String brandInfoId, String memberCode, int point, String awardPoint) {
        ...
        param.put("awardPoint", awardPoint);
        Map<String, Object> memInfo = binOLCM31_Service.getMemInfoId(param);
        if (!CherryChecker.isNullOrEmpty(memInfo)) {
            memInfo.putAll(param);
            binOLCM31_Service.updateCompleteByMemcode(memInfo);
    ...
    public Map<String, Object> getMemCompleteRule(String organizationInfoId, String brandInfoId) {
        ...
        param.put("organizationInfoId", organizationInfoId);
        param.put("brandInfoId", brandInfoId);
        param.put("sysTime", CherryUtil.getSysDateTime(DateUtil.DATETIME_PATTERN));
        Map<String, Object> ruleInfo_map = binOLCM31_Service.getMemCompleteRule(param);
        return ruleInfo_map;
    }

    /**
     * 这个是根据规则返回对应会员属性的方法（内有查询会员信息逻辑）
     *
     * @param organizationInfoId
     * @param brandInfoId
     * @param memCode
    ...
    public Map<String, Object> getMemInfoByCompleteRule(String organizationInfoId, String brandInfoId, String memCode, Map<String, Object> ruleInfo_map) {
        ...
        List<String> memInfoKey_list = new ArrayList<String>();
        //获取当前生效的规则
        Map<String, Object> param = new HashMap<String, Object>();
        param.put("organizationInfoId", organizationInfoId);
        param.put("brandInfoId", brandInfoId);
        ...
        param.put("sysTime", CherryUtil.getSysDateTime(DateUtil.DATETIME_PATTERN));
        Map<String, Object> memInfo_map = binOLCM31_Service.getMemInfo(param);
        if (CherryChecker.isNullOrEmpty(memInfo_map)) {
            return null;
        } else {
            if (!CherryChecker.isNullOrEmpty(ruleInfo_map)) {
                String ruleConditionJson = ConvertUtil.getString(ruleInfo_map.get("RuleCondition"));
                Map<String, Object> param_map = ConvertUtil.json2Map(ruleConditionJson);
                ...
                for (Map<String, Object> pointGroupInfo : pointGroup) {
                    for (Map.Entry<String, Object> entry : pointGroupInfo.entrySet()) {
                        ...
                        String value = ConvertUtil.getString(memInfo_map.get(key));
                        if (!CherryChecker.isNullOrEmpty(value)) {
                            result_map.put(key, value);
                        }
                    }
                }
                for (Map<String, Object> unpointGroupInfo : noProintGroup) {
                    for (Map.Entry<String, Object> entry : unpointGroupInfo.entrySet()) {
                        String key = entry.getKey();
                        String value = ConvertUtil.getString(memInfo_map.get(key));
                        if (!CherryChecker.isNullOrEmpty(value)) {
                            result_map.put(key, value);
    ...
    public int calMemCompletePercent(String organizationInfoId, String brandInfoId, Map<String, Object> memInfo, Map<String, Object> ruleInfo_map) {
        Map<String, Object> memInfo_check = CherryUtil.removeEmptyVal(memInfo);

        if (CherryChecker.isNullOrEmpty(memInfo_check)) {
            return 0;
        }
        ...
        if (CherryChecker.isNullOrEmpty(ruleInfo_map)) {
            ...
            for (Map<String, Object> pointGroupInfo : pointGroup) {
                for (Map.Entry<String, Object> entry : pointGroupInfo.entrySet()) {
                    String key = entry.getKey();
                    String value = ConvertUtil.getString(memInfo_check.get(key));
                    Map<String, Object> convert_map = (Map<String, Object>) entry.getValue();
                    int percent = ConvertUtil.getInt(convert_map.get("percent"));
                    if (!"".equals(value)) {
     ...
     * @return
     */
    public Map<String, Object> calMemCompletePoint(String organizationInfoId, String brandInfoId, Map<String, Object> memInfo, String memberCode, Map<String, Object> ruleInfo_map) {
        int pointTotal = 0;
        Map<String, Object> result_map = new HashMap<String, Object>();
        Map<String,Object> new_memInfo=new HashMap<String, Object>();
        if (ruleInfo_map == null || ruleInfo_map.isEmpty()) {
            result_map.put("pointTotal", pointTotal);
            return result_map;
        }
        if (null != memInfo && !memInfo.isEmpty()){
            new_memInfo.putAll(memInfo);
            new_memInfo=CherryUtil.remEmptyVal(new_memInfo);
        }
        ...
        param.put("memCode", memberCode);
        //本次新发送积分的属性List
        Set<String> newSendPro = new HashSet<String>();
        //通过会员号获取当前会员下已经占位过的属性
        String awardPoint = ConvertUtil.getString(binOLCM31_Service.getCompleteDegreeOccu(param));
        String[] awardPointArr = awardPoint.split(",");

            int occuRuleCount = 0;
            int new_memInfo_ex = 0;
            int new_memInfo_remove = 0;
            if (null != new_memInfo && !new_memInfo.isEmpty()) {
                new_memInfo_ex = new_memInfo.size();
                for (String award : awardPointArr) {
                    new_memInfo.remove(award);
                }
                if (null != new_memInfo && !new_memInfo.isEmpty()) {
                    new_memInfo_remove = new_memInfo.size();
                }
            }

        if(new_memInfo.isEmpty()){
            result_map.put("pointTotal", pointTotal);
            result_map.put("awardPoint", awardPoint);
            return result_map;
        }


        occuRuleCount = new_memInfo_ex - new_memInfo_remove;
        String ruleConditionJson = ConvertUtil.getString(ruleInfo_map.get("RuleCondition"));
        int exceptPoint = ConvertUtil.getInt(ruleInfo_map.get("ExceptPoint"));
        Map<String, Object> param_map = ConvertUtil.json2Map(ruleConditionJson);
        List<Map<String, Object>> pointGroup = (List<Map<String, Object>>) param_map.get("pointGroup");
        List<Map<String, Object>> noPointGroup = (List<Map<String, Object>>) param_map.get("noPointGroup");
        //确定了point的列表和新增属性Map作比较,匹配到了直接送积分
        for (Map<String, Object> pointGroupInfo : pointGroup) {
            for (Map.Entry<String, Object> pointGroupInfo_entry : pointGroupInfo.entrySet()) {
                String key_pointGroupInfo = pointGroupInfo_entry.getKey();
                Map<String, Object> value_pointGroupInfo = (Map<String, Object>) pointGroupInfo_entry.getValue();
                if (new_memInfo.containsKey(key_pointGroupInfo)) {
                    int point = ConvertUtil.getInt(value_pointGroupInfo.get("point"));
                    pointTotal += point;
                    newSendPro.add(key_pointGroupInfo);
                }
            }
        }

        //没有确定point的列表，计算情况分为两种情况


        if (noPointGroup != null && !noPointGroup.isEmpty()) {
            //对new_memInfo进行一个数据格式的转换便于比较
            Set checkSet = new HashSet();
            int noPointOccuCount=0;
            for (Map<String, Object> unpointGroupInfo : noPointGroup) {
                boolean isExist=false;
                for(String awardStr:awardPointArr){
                    if(unpointGroupInfo.containsKey(awardStr)){
                        isExist=true;
                        noPointOccuCount++;
                        break;
                    }
                ...
                for (Map.Entry<String, Object> unPointGroupInfo_entry : unpointGroupInfo.entrySet()) {
                    String unpointKey = unPointGroupInfo_entry.getKey();
                    if (new_memInfo.containsKey(unpointKey)) {
                        checkSet.add(unpointKey);
                    }
                }
            }
                //(1)之前在占位表中不存在的情况--匹配规则中的key如果都匹配上了直接累加赠送ExceptPoint积分
            if (occuRuleCount == 0) {
                //原先没有占位的情况，先进行匹配打上标记，之后看是否全部被选中，选中则赠送ExceptPoint对应的积分
                if (checkSet.size() == noPointGroup.size()) {
                    //全部选中状态，赠送积分并且把相关字段加入newSendPro中
                    pointTotal += exceptPoint;
                    for (Map.Entry<String, Object> entry : new_memInfo.entrySet()) {
                        String key = entry.getKey();
                        newSendPro.add(key);
                    }
                }
                ...
                //(2)之前在占位表中存在的情况
                //	未分组的属性总和 UNGroupCount
                // 	规则关心的属性个数 RuleCount
                //	已经占位的属性与RuleCount匹配到的个数 OccuRuleCount
                //  赠送的积分=ExceptPoint*((RuleCount-OccuRuleCount)/UNGroupCount)
                int UNGroupCount = noPointGroup.size();
                if (checkSet.size() + noPointOccuCount == UNGroupCount) {
                    int point = (int)
                            DoubleUtil.roundDown(DoubleUtil.mul(exceptPoint, DoubleUtil.div((UNGroupCount - noPointOccuCount), UNGroupCount)), 0);
                    pointTotal += point;
                    newSendPro.addAll(checkSet);
                }
            }
        }
            //已经占位的还要写入表中
        if(awardPointArr != null && awardPointArr.length >0){
            for(String awardPoint_str:awardPointArr){
                newSendPro.add(awardPoint_str);
            }
        }
            StringBuilder sb = new StringBuilder();

            Iterator it = newSendPro.iterator();//先迭代出来

            while (it.hasNext()) {
                sb.append(it.next() + ",");
            }
            String awardPointStr = sb.toString();
            int endIndex = StringUtils.isEmpty(awardPointStr) ? 0 : awardPointStr.length() - 1;
            awardPointStr = awardPointStr.substring(0, endIndex);


            result_map.put("pointTotal", pointTotal);
            result_map.put("awardPoint", awardPointStr);
            return result_map;
