package com.cherry.cm.activemq.util;
...
public class MessageUtil {
	...
	public static String getMQMessage(Map<String,Object> MessageMap) throws CherryMQException{
		...
		try{
			version = ConvertUtil.getString(MessageMap.get("Version"));
			if(("").equals(version)){
				throw new Exception();
			}
		}catch(Exception e){
		...
		//如果没有设定消息数据类型抛出异常
		try{
			type = ConvertUtil.getString(MessageMap.get("Type"));
			if(("").equals(type)){
				throw new Exception();
			}else{
				message.append(type);
    ...
    public static String getMQMessageJSON(Map<String,Object> MessageMap) throws CherryMQException{
        ...
        //设定DataLine
        try{
            String dataLine = ConvertUtil.getString(MessageMap.get("DataLine"));
            if(dataLine.equals("")){
