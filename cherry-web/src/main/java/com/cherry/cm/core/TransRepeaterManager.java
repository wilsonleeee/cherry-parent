 ...
 */
public class TransRepeaterManager implements InitializingBean {
     ...
     * @return
     */
    private List<MQBaseDTO> createTransMq(String brandCode, String sourceTradeType, Object param) throws Exception{
        List<TransRepeaterConfigDTO> configList = allConfig.get(brandCode + "_" + sourceTradeType);
        ...
        if(null!=configList) {
            for (TransRepeaterConfigDTO config : configList) {
                ...
                MQBaseDTO mqdto = creater.createTransMqByTrade(param);
                if (null != mqdto) {
