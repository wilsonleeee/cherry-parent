/*
 * @(#)BINOLPTJCS47_Action.java v1.0 2014-8-7
 * 
...
import com.cherry.pt.jcs.form.BINOLPTJCS48_Form;
 ...
 */
public class BINOLPTJCS48_Action extends BaseAction implements
ModelDriven<BINOLPTJCS48_Form>{

	private static final long serialVersionUID = 1324480801173680548L;
	
	private static Logger logger = LoggerFactory.getLogger(BINOLPTJCS48_Action.class.getName());
	
	
	/** 参数FORM */
	private BINOLPTJCS48_Form form = new BINOLPTJCS48_Form();
	
	@Resource(name="binOLPTJCS48_IF")
	private BINOLPTJCS48_IF binOLPTJCS48_IF;
	...
	public String init() throws Exception {
		...
		try {
			...
			// 所有产品价格方案
			prtPriceSolutionList = binOLPTJCS48_IF.getPrtPriceSolutionList(map);
	...
	public String importSoluDeatil() {
		try {
			...
			//导入的数据
			Map<String, Object> importDataMap = binOLPTJCS48_IF.ResolveExcel(sessionMap);
			
			Map<String, Object> resultMap = binOLPTJCS48_IF.tran_excelHandle(importDataMap, sessionMap);
...
//			this.addActionMessage(getText("SSM00014"));
	...
	public String importSoluDeatilCnt() {
		try {
			...
			//导入的数据
			Map<String, Object> importDataMap = binOLPTJCS48_IF.ResolveExcelCnt(sessionMap);
			
			Map<String, Object> resultMap = binOLPTJCS48_IF.tran_excelHandleCnt(importDataMap, sessionMap);
			resultMap.put("solutionName", form.getSolutionName());
			// 导入成功
//			this.addActionMessage(getText("SSM00014"));
	...
	private Map getSessionInfo() throws Exception{
		...
		// 业务日期
		String businessDate = binOLPTJCS48_IF.getBussinessDate(map);
		map.put("businessDate", businessDate); 
		configMap.put("businessDate",businessDate);
		
		// 作成程序名
		map.put(CherryConstants.CREATEPGM, "BINOLPTJCS47");
		// 更新程序名
		map.put(CherryConstants.UPDATEPGM, "BINOLPTJCS47");
		// 作成者
	...
	@Override
	public BINOLPTJCS48_Form getModel() {
		return form;
	}
