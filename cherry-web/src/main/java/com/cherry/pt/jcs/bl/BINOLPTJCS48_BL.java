...
import com.cherry.pt.jcs.interfaces.BINOLPTJCS48_IF;
...
import com.cherry.pt.jcs.service.BINOLPTJCS48_Service;
 ...
 */
public class BINOLPTJCS48_BL implements BINOLPTJCS48_IF {
	
	@Resource(name="binOLPTJCS48_Service")
	private BINOLPTJCS48_Service binOLPTJCS48_Service;
	...
	private static final Logger logger = LoggerFactory.getLogger(BINOLPTJCS48_BL.class);
	...
	public Map<String, Object> ResolveExcel(Map<String, Object> map) throws Exception {
		...
		int sum = 0;
		// 是否更新产品表价格
		String isSynchProductPrice = ConvertUtil.getString(map.get("isSynchProductPrice"));
	...
	private boolean checkDataCnt(Map<String, Object> rowMap,Map<String,Object> map,List<Map<String,Object>> errorList ) throws Exception {
		...
		if (CherryConstants.BLANK.equals(unitCode)) {
			...
			rowMap.putAll(map);
			Map<String,Object> prtMap = binOLPTJCS48_Service.getProductInfo(rowMap);
...
//			prtMap.put("priceJson", priceJson);
			...
			Map<String, Object> mergeMap = binOLPTJCS48_Service.mergePrtPriceSoluDetail(prtMap);
//			binOLPTJCS48_Service.mergePrtPriceSoluDetail(prtMap);
...
//		infoMap.put(ProductConstants.ADDCOUNT, 0);
		...
		// Step1: 产品方案中原有数据删除
		// *Step2: 产品方案中原有数据删除
		binOLPTJCS48_Service.delPrtPriceSoluDetail(sessionMap);
...
//			prtMap.put("priceJson", priceJson);
			...
			// Step1.1: 新明细插入到方案明细表
			// *Step2.1: 新明细插入到方案明细表
			binOLPTJCS48_Service.insertPrtPriceSoluDetail(prtMap);
//			binOLPTJCS48_Service.mergePrtPriceSoluDetail(prtMap);
		...
		binOLPTJCS48_Service.updProductDepart(sessionMap);
		// Step2.1: 将方案原关联的柜台对应的部门产品价格删除。
		binOLPTJCS48_Service.delProductDepartPrice(sessionMap);
		*/
...
//		List<Map<String, Object>> newSoluDetailProductDepartPriceList = binOLPTJCS48_Service.getNewSoluDetailProductDepartPriceList(sessionMap);
//		if(null != newSoluDetailProductDepartPriceList && !newSoluDetailProductDepartPriceList.isEmpty()){
...
//				
//				// Step3.1: 插入产品部门表
//				binOLPTJCS48_Service.mergeProductDepartInfo(itemMap);
//				
//				// Step3.2: 插入柜台价格到产品价格表中
...
//			binOLPTJCS48_Service.insertPrtPriceSoluDetail(prtMap);
	...
	public List getPrtPriceSolutionList(Map<String, Object> map) {
		return binOLPTJCS48_Service.getPrtPriceSolutionList(map);
	}
			...
			if (null != priceInfoList) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (CherryConstants.BLANK.equals(option) || ProductConstants.OPTION_1.equals(option)) {
						// 插入产品销售价格
						binOLPTJCS48_Service.insertProductPrice(priceMap);
					} 
				}
			}
		}
