 ...
 */
public class BINOLPTJCS23_Action extends BaseAction implements
		ModelDriven<BINOLPTJCS23_Form> {

	private static final long serialVersionUID = -6676412305227501560L;
	
	private static final Logger logger = LoggerFactory.getLogger(BINOLPTJCS23_Action.class);
...
//		minPackTypeList = binOLCM05_BL.getMinPackageTypeList(map);
	...
	public void getUnitCode() throws Exception{
		...
		if(!(null == form.getBarCode() || form.getBarCode().trim().isEmpty())){
			unitCode = binOLPTJCS23_IF.getUnitCodeRightTree(map);
		}
...
//		Map<String,Object> resultMap = new HashMap<String, Object>();
		...
		try{
			...
			// barCode
			List<Map<String, Object>> barCode = (List<Map<String, Object>>) JSONUtil.deserialize(form.getBarCode());
			for (int i = 0; i < barCode.size(); i++) {
				String barCodei = ConvertUtil.getString(barCode.get(i).get(CherryConstants.BARCODE));
				map.put("barCode", barCodei);
				List<Map<String, Object>> existProductList = binOLCM05_BL.getProductListByBarCode(map);
				chkExistBarCodeMap = new HashMap();
				chkExistBarCodeMap.put("existProductList", existProductList);
				chkExistBarCodeMap.put("barCode", barCodei );
				chkExistBarCodeMap.put("existProductListIsEmpty", existProductList.size() !=0 ? "existProductListIsEmpty1" : "existProductListIsEmpty0" );
			}
	...
	public String queryCate() throws Exception {
		...
		// 产品分类List
		cateList = binOLPTJCS23_IF.getCategoryList(paramMap);
		return SUCCESS;
	}
	...
	public String queryCateList() throws Exception {
		...
		if (ProductConstants.TEMINALFLAG_1.equals(form.getTeminalFlag())) {
			// 取得大分类List
			cateList = binOLPTJCS23_IF.getPatCateList(map);
		} else if (ProductConstants.TEMINALFLAG_2.equals(form.getTeminalFlag())) {
			// 取得小分类List
			cateList = binOLPTJCS23_IF.getSubCateList(map);
		}
		return SUCCESS;
	}
...
//		minPackTypeList = binOLCM05_BL.getMinPackageTypeList(paramMap);
	...
	public String save() throws Exception {
		...
		try {
			...
			uploadImg(imagePath);
			// 产品添加插表处理
			binOLPTJCS23_IF.tran_addProduct(map);
...
//			String  str = strr[3];
	...
	private void doList(List<Map<String, Object>> cateList, String path) {
		...
		if(null != cateList && propValIds.length > 0){
			...
			List<Map<String, Object>> cateValList = binOLPTJCS23_IF.getCateValList(map);
