 ...
 * 
 */
public class BINOLPTJCS05_BL implements BINOLPTJCS05_IF {

	private static Logger logger = LoggerFactory.getLogger(BINOLPTJCS05_BL.class.getName());
	@Resource
	...
	/** 产品添加Service */
	@Resource
	private BINOLPTJCS03_Service binolptjcs03Service;
	...
	/** 产品详细Service */
	@Resource
	private BINOLPTJCS07_Service binolptjcs07Service;
	...
	public List<Map<String, Object>> parseFile(File file, UserInfo userInfo)
        ...
        if(!CherryConstants.PRODUCT_EXCEL_VERSION.equals(version)){
		...
		for (int r = 1; r < dateSheet.getRows(); r++) {
			...
			// 产品编码
			String unitCode = dateSheet.getCell(2, r).getContents().trim().toUpperCase();
			// 产品中文名
			String chineseName = dateSheet.getCell(3, r).getContents().trim().replace("|", "｜");
			// 分类1选项代码
			...
			// 会员价格
			String memPrice = dateSheet.getCell(13, r).getContents().trim();
			// 状态
			...
			// 品牌
			String originalBrand = dateSheet.getCell(17, r).getContents().trim();
			// 品类
			String itemType = dateSheet.getCell(18, r).getContents().trim();
			// 是否需要积分兑换
			String isExchanged = dateSheet.getCell(19, r).getContents().trim();
			// 色系
			String color = dateSheet.getCell(20, r).getContents().trim();
			// 利润
			String profit = dateSheet.getCell(21, r).getContents().trim();
			// 整行数据为空，程序认为sheet内有效行读取结束
			if ("".equals(brandCode) && "".equals(barCode)
					&& "".equals(unitCode) && "".equals(chineseName)
					&& "".equals(propValueCherry1) && "".equals(propName1)
					&& "".equals(propValueCherry2) && "".equals(propName2)
					&& "".equals(propValueCherry3) && "".equals(price)
					&& "".equals(standardCost) && "".equals(orderPrice)
					&& "".equals(memPrice)&& "".equals(status)&& "".equals(mode)&& "".equals(isStock)
					&& "".equals(originalBrand) && "".equals(itemType)&& "".equals(isExchanged)&&"".equals(color)&&"".equals(profit)) {
				sum++;
				if (sum >= 10) {
					break;
				} else {
...
//			} else if (!CherryChecker.isProCode(barCode)) {
			} else if (!binOLPTJCS03_IF.checkBarCode(userInfo, barCode)) {
				// 英数验证
			...
			if (CherryConstants.BLANK.equals(unitCode)) {
				...
				// 代码长度错误
				throw new CherryException("EBS00033", new String[] {
						ProductConstants.DATE_SHEET_NAME, "C" + (r + 1), "20" });
//			} else if (!CherryChecker.isProCode(unitCode)) {
			} else if (!binOLPTJCS03_IF.checkUnitCode(userInfo, unitCode)) {
				// 英数验证
				throw new CherryException("EBS00034", new String[] {
						ProductConstants.DATE_SHEET_NAME, "C" + (r + 1) });
			}else {
				...
				int productId = binolptjcs03Service.getProductId(prtMap);
				if (productId != 0) {
					// 更新情景
					// 已存在的unitCode数量
				...
				if(unitCodeCount != 0){
					...
					throw new CherryException("EBS00098", new String[] {
							ProductConstants.DATE_SHEET_NAME, "C" + (r + 1), unitCode });
				}
...
//			}
				...
				if(CherryUtil.mixStrLength(chineseName) > nameRuleLen){
					// 中文名称不能超过系统配置规则
					throw new CherryException("EBS00114", new String[] { "D" + (r + 1)});
				}
			}
...
//			}
			...
			if(isAddFlag && CherryConstants.BLANK.equals(memPrice)){
				...
				// 会员价不填写时， 默认使用销售价代替
//				memPrice = ProductConstants.DEF_PRICE;
				memPrice = price;
			...
			if (!CherryConstants.BLANK.equals(originalBrand)){
				...
				if(null == originalBrandKey){
					...
					throw new CherryException("EBS00112", new String[] {
							ProductConstants.DATE_SHEET_NAME, "R" + (r + 1)});
				}
			...
			if (!CherryConstants.BLANK.equals(itemType)){
				...
				if(null == itemTypeKey){
					...
					throw new CherryException("EBS00113", new String[] {
							ProductConstants.DATE_SHEET_NAME, "S" + (r + 1)});
				}
			}
			if(!CherryConstants.BLANK.equals(color)){
				if(color.length()>16){
					//色系长度错误
					throw new CherryException("EBS00033", new String[] {
							ProductConstants.DATE_SHEET_NAME, "U" + (r + 1),"16"});
				}
			}
			if(!CherryConstants.BLANK.equals(profit)){
				if (!CherryChecker.isFloatValid(profit, 12, 2)) {
					// 利润格式有误
					throw new CherryException("EBS00034", new String[] {
							ProductConstants.DATE_SHEET_NAME, "V" + (r + 1) });
				}
			}
			// map
			...
			map.put("color", color);
			map.put("profit", getPrice(profit));
			//是否管理库存 1：是 0：否，默认"是"
...
//			optPrice(prtMap);
	...
	private int optProduct(Map<String, Object> prtMap,
		...
		if (productId != 0) {
			prtMap.put(ProductConstants.PRODUCTID, productId);
			// 更新产品信息
			binolptjcs05Service.updProduct(prtMap);
			...
			binOLPTJCS04_Service.updPrtSolutionDetail(prtMap);
			
		} else {
//			prtMap.put("isStock", "1"); // 默认管理库存
			// 添加产品信息
			productId = binolptjcs03Service.insertProduct(prtMap);
...
//			// 添加产品厂商信息
//			binolptjcs03Service.insertProductVendor(prtMap);
//		} 
//		else if(prtMap.get("prtOptFlag").equals("U")){
//			// 编辑产品厂商信息
//			Map<String, Object> prtVendorMap = binolptjcs05Service.getProductVendorInfo(prtMap);
//			int proVendorId = (Integer)prtVendorMap.get("proVendorId");
//			prtMap.put("prtVendorId", proVendorId);
//			binolptjcs07Service.updPrtVendor(prtMap);
//			// 更新产品条码对应关系信息
//			binolptjcs07Service.updPrtBarCode(prtMap);
//		}
		
		//是否一品多码
		boolean isU2M = binOLCM14_BL.isConfigOpen("1077",ConvertUtil.getString(prtMap.get(CherryConstants.ORGANIZATIONINFOID)),ConvertUtil.getString(prtMap.get(CherryConstants.BRANDINFOID)));
		// 产品条码list
		List<Map<String, Object>> barList = binolptjcs05Service
				.getBarCodeList(prtMap);
		boolean addFlag = true;
		if (null != barList && !barList.isEmpty()) {
			// 伦理清除产品条码信息
//			binolptjcs05Service.updProductVendor(prtMap, "0");
			String barCode = ConvertUtil.getString(prtMap.get(CherryConstants.BARCODE));
...
//					if(map.get("validFlag").toString().equals("0")){
...
//					Map<String,Object> paramMap = new HashMap<String, Object>();
//					paramMap.put("productId", prtMap.get("productId"));
...
//					}
					...
					binolptjcs05Service.updProductVendor(map, "1");
//					}
					addFlag = false;
					break;
				}
			...
			if(addFlag){
				...
				if(!isU2M){
					...
					temp.put("prtVendorId",barList.get(0).get("proVendorId"));
					temp.put("validFlag", "1");
					binolptjcs07Service.updPrtVendor(temp);
					binolptjcs07Service.updPrtBarCode(temp);
				}
				// 一品多码
				else {
					// 添加产品条码信息
	...
	private void optPrice(Map<String, Object> prtMap) {
		// 清除产品价格信息
		binolptjcs05Service.delProductPrice(prtMap);
		if (!CherryChecker
				.isNullOrEmpty(prtMap.get(ProductConstants.SALEPRICE))) {
			prtMap.put(ProductConstants.PRICESTARTDATE, prtMap
					.get(CherryConstants.CREATE_TIME));
			prtMap.put(ProductConstants.PRICEENDDATE,
					CherryConstants.longLongAfter);
			// 添加产品价格信息
			binolptjcs03Service.insertProductPrice(prtMap);
		}
	}
	...
	private void optPriceNew(Map<String, Object> prtMap) {
		...
		if (!CherryChecker.isNullOrEmpty(imSalePrice)) {
			...
			if(prtMap.get("prtOptFlag").equals("A")){
				...
				// 查看当前业务日期落在价格记录的生效区间内
				Map<String,Object> priceInfo = binolptjcs05Service.getProductPrice(prtMap);
				// 如果没有，新增一条价格记录 起始日期为当前业务日期
				if(null == priceInfo || priceInfo.isEmpty()){
					
					// 查询是否存在未生效的价格记录
					priceInfo = binolptjcs05Service.getPreProductPrice(prtMap);
					if(null == priceInfo){
						prtMap.put(ProductConstants.PRICEENDDATE,CherryConstants.longLongAfter);
					} else {
				...
				// 如果有的，判断价格记录中的价格是否与导入的价格不一样，即发生修改
					...
					if (!CherryUtil.subZeroAndDot(salePrice).equals(CherryUtil.subZeroAndDot(imSalePrice))
						...
						if(businessDate.equals(startDate)){
							...
							binolptjcs05Service.upProPrices(pMap);
						}
						// 否，删除老价格记录，新增价格记录（开始日期为当前业务日期，结束日期需要查是否存在未生效的价格记录，不存在，取21000101,存在，取最早一条的开始日期减1）
						else {
							// 删除老价格
							pMap.put("prtPriceId", priceInfo.get("BIN_ProductPriceID"));
							binolptjcs07Service.delProductPrice(pMap);
	...
	private void optCategory(Map<String, Object> prtMap) throws Exception {
		// 删除产品分类信息
		binolptjcs05Service.delPrtCategory(prtMap);
		// 更新产品大分类
	...
	private void updPrtCategory(Map<String, Object> prtMap, int cateType) throws Exception {
		...
		if (!CherryConstants.BLANK.equals(propName)) {
			...
			if (!CherryConstants.BLANK.equals(propValueCherry)) {
				...
				if (propValId == 0) {
					...
					List result = binolptjcs05Service.getExistPvCN(tempMap);
					if(result.size() == 0){
		...
		if (propValId != 0) {
			tempMap.put(ProductConstants.PROPVALID, propValId);
			// 添加产品分类对应关系表
			binolptjcs03Service.insertPrtCategory(tempMap);
		}
	}
...
//						throw new Exception();
	...
	private Map<String, Object> getMap(Map<String, Object> map) {
		...
		// 系统时间
		String sysDate = binolptjcs05Service.getSYSDate();
		// 业务日期
		String bussinessDate = binolptjcs05Service.getBusDate(map);
		...
		// 作成程序名
		map.put(CherryConstants.CREATEPGM, "BINOLPTJCS05");
		// 更新程序名
		map.put(CherryConstants.UPDATEPGM, "BINOLPTJCS05");
		// 作成者
