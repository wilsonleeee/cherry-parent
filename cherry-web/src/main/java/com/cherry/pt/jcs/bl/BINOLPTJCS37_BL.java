...
import com.cherry.pt.jcs.interfaces.BINOLPTJCS37_IF;
...
import com.cherry.pt.jcs.service.BINOLPTJCS37_Service;
 ...
 */
public class BINOLPTJCS37_BL implements BINOLPTJCS37_IF {
	...
	@Resource(name="binOLPTJCS37_Service")
	private BINOLPTJCS37_Service binOLPTJCS37_Service;
	...
	public int tran_updProduct(Map<String, Object> map) throws Exception {
		...
		// 产品属性信息更新
		result = binOLPTJCS37_Service.updProduct(map);
	...
	private int updPrtVendor(Map<String, Object> map) throws Exception {
		...
		// 业务日期
		String bussDate = binOLPTJCS37_Service.getBusDate(map);
		
		String sysdateTime = binOLPTJCS37_Service.getSYSDateTime();
		...
		if (CherryConstants.VALIDFLAG_DISABLE.equals(map.get("validFlag"))) {
			for(Map<String, Object>closeTimeMap : allList){
				...
				// 更新产品厂商的validFlag为0
				binOLPTJCS37_Service.updPrtVendor(paramsMap);
				// 更新关系表的ClosingTime字段为当前时间（业务时间）
				binOLPTJCS37_Service.updateClosingTime(paramsMap);
			}
			return prtVendorId;
		}
		...
		for (Map<String, Object> code : addCodeList) {
			code.putAll(map);
			prtVendorId = binOLPTJCS33_Service.insertProductVendor(code);
		}
		// =========== 更新的产品条码 ============ //
		for (Map<String, Object> updMap : updCodeList) {
			...
			binOLPTJCS37_Service.updPrtVendor(paramMap);
			// 更新产品条码对应关系信息
		...
		for (Map<String, Object> delMap : delCodeList) {
			...
			binOLPTJCS37_Service.updPrtVendor(delMap);
			// 更新产品条码对应关系信息
			delMap.put(ProductConstants.CLOSING_TIME, closingTime);
			binOLPTJCS37_Service.updPrtBarCode(delMap);
		}
		// =========== 不变的产品条码 ============ //
		for (Map<String, Object> unMap : unChangeCodeList) {
			...
			if ("0".equals(validFlag)) {
				...
				binOLPTJCS37_Service.updPrtVendor(unMap);
				// 更新停用日时
...
//			}
			if (null != priceInfoList) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (CherryConstants.BLANK.equals(option)) {
						...
						// 更新产品销售价格
						binOLPTJCS37_Service.updProductPrice(priceMap);
					} else if (ProductConstants.OPTION_2.equals(option)) {
						// 删除产品销售价格
						binOLPTJCS37_Service.delProductPrice(priceMap);
					}
				}
			}
	...
	private void updPrtCategory(Map<String, Object> map) throws Exception {
		// 删除数据库中产品分类信息
		binOLPTJCS37_Service.delPrtCategory(map);
		// 产品分类信息
	...
	private void updPrtBOM(Map<String, Object> map,
		...
		if (mode.equals(ProductConstants.MODE_1)
			...
			for (Map<String, Object> barCode : barCodeList) {
				...
				// 删除产品BOM表
				binOLPTJCS37_Service.delProductBOM(prtMap);
				if (!ProductConstants.OPTION_2.equals(option)) {
	...
	private Map<String, Object> getUpdMap(Map<String, Object> map) {
		...
		// 系统时间
		String sysDate = binOLPTJCS37_Service.getSYSDate();
		// 作成日时
		map.put(CherryConstants.CREATE_TIME, sysDate);
		// 更新日时
		map.put(CherryConstants.UPDATE_TIME, sysDate);
		// 作成程序名
		map.put(CherryConstants.CREATEPGM, "BINOLPTJCS27");
		// 更新程序名
		map.put(CherryConstants.UPDATEPGM, "BINOLPTJCS27");
		// 作成者
	...
	public int getActUsePrtCount(Map<String, Object> map){
		return binOLPTJCS37_Service.getActUsePrtCount(map);
	}
	...
	public int getActPrtCount(Map<String, Object> map){
		String bussDate = binOLPTJCS37_Service.getBussinessDate(map);
		map.put("bussDate", bussDate); // 业务时间
		return binOLPTJCS37_Service.getActPrtCount(map);
	...
	public void updPrtBarCode(Map<String, Object> map){
		...
		Map<String, Object> prtBCMap = binOLPTJCS37_Service.getPrtBarCode(map);
		...
		binOLPTJCS37_Service.updPrtBarCode(map);
	}
}
