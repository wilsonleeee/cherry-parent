 ...
 */
public class BINOLPTJCS35_Action extends BaseAction {

	/** */
	private static final long serialVersionUID = -5515185790225353101L;
	
	private static Logger logger = LoggerFactory.getLogger(BINOLPTJCS35_Action.class.getName());

	@Resource(name="binOLCM05_BL")
	private BINOLCM05_BL binOLCM05_BL;

	@Resource(name="binOLPTJCS25_IF")
	private BINOLPTJCS25_IF binolptjcs25IF;
	...
	public String importPros() throws Exception {
		...
		try {
			...
			// 解析文件
			list = binolptjcs25IF.parseFile(upExcel, userInfo);

			// 参数MAP
			Map<String, Object> map = getSearchMap();
			map.put(ProductConstants.LIST, list);
			// 批量导入产品
			Map<String, Integer> infoMap = binolptjcs25IF.tran_import(map);
			// 导入成功
