 ...
 */
public class BINOLPTJCS07_Action extends BaseAction implements
		ModelDriven<BINOLPTJCS07_Form> {

	private static final long serialVersionUID = 7052727878284081396L;

	private static final Logger logger = LoggerFactory.getLogger(BINOLPTJCS07_Action.class);
	
	/** 参数FORM */
	private BINOLPTJCS07_Form form = new BINOLPTJCS07_Form();
	...
	@Override
	public BINOLPTJCS07_Form getModel() {
		return form;
	}
	...
	public String init() throws Exception {
		...
		// 取得产品信息
		proMap = binolptjcs06_IF.getDetail(map);
...
//			}
		...
		// 取得产品条码List
		barCodeList = binolptjcs06_IF.getBarCodeList(map);
		// 取得分类值List
		List<Map<String, Object>> cateValList = binolptjcs06_IF
				.getCateList(map);
		// 取得销售价格List
		sellPriceList = binolptjcs06_IF.getSellPriceList(map);
		...
		if (mode.equals(ProductConstants.MODE_1)
			...
			// 查询产品BOM表
			bomList = binolptjcs06_IF.getBOMList(map);
		}
		// 产品扩展属性个数
	...
	public String update() throws Exception {
		...
		try {
			...
			// 更新产品
			int result = binolptjcs07_IF.tran_updProduct(map);
			if (result == 0) {
