...
import com.cherry.pt.jcs.form.BINOLPTJCS23_Form;
 ...
 */
public class BINOLPTJCS33_Action extends BaseAction implements
		ModelDriven<BINOLPTJCS23_Form> {

	private static final long serialVersionUID = -6676412305227501560L;

	private static final Logger logger = LoggerFactory
			.getLogger(BINOLPTJCS33_Action.class);
	...
	/** barcode校验结果 */
	private Map chkExistBarCodeMap;

	/** 参数FORM */
	private BINOLPTJCS23_Form form = new BINOLPTJCS23_Form();
	...
	public BINOLPTJCS23_Form getModel() {
		return form;
	}
	...
	public Map getChkExistBarCodeMap() {
		return chkExistBarCodeMap;
	}

	public void setChkExistBarCodeMap(Map chkExistBarCodeMap) {
		this.chkExistBarCodeMap = chkExistBarCodeMap;
	}
	...
	public String init() throws Exception {
		...
		chkExistBarCodeMap = new HashMap();
		chkExistBarCodeMap.put("businessDate", businessDate);

		// 是否一品多码
		isU2M = binOLCM14_BL.isConfigOpen("1077", ConvertUtil.getString(map
				.get(CherryConstants.ORGANIZATIONINFOID)), ConvertUtil
				.getString(map.get(CherryConstants.BRANDINFOID)));
	...
	public void getUnitCode() throws Exception {
		...
		if (!(null == form.getBarCode() || form.getBarCode().trim().isEmpty())) {
			unitCode = binOLPTJCS33_IF.getUnitCodeRightTree(map);
		}

		ConvertUtil.setResponseByAjax(response, unitCode.toString());

	}

	/**
	 * 检查当前添加的产品条码是否在系统或方案（柜台对应 的默认方案）中存在
	 * 
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	public String chkExistBarCode() throws Exception {
		...
		try {
			...
			for (int i = 0; i < barCode.size(); i++) {
				...
				map.put("barCode", barCodei);
				List<Map<String, Object>> existProductList = binOLCM05_BL.getProductListByBarCodeForSolu(map);
				chkExistBarCodeMap = new HashMap();
				chkExistBarCodeMap.put("existProductList", existProductList);
				chkExistBarCodeMap.put("barCode", barCodei);
				chkExistBarCodeMap
						.put("existProductListIsEmpty",
								existProductList.size() != 0 ? "existProductListIsEmpty1"
										: "existProductListIsEmpty0");
			}

		} catch (Exception e) {
			chkExistBarCodeMap.put("existProductList", null);
	...
	public String queryCate() throws Exception {
		...
		// 产品分类List
		cateList = binOLPTJCS33_IF.getCategoryList(paramMap);
		return SUCCESS;
	}
	...
	public String queryCateList() throws Exception {
		...
		if (ProductConstants.TEMINALFLAG_1.equals(form.getTeminalFlag())) {
			// 取得大分类List
			cateList = binOLPTJCS33_IF.getPatCateList(map);
		} else if (ProductConstants.TEMINALFLAG_2.equals(form.getTeminalFlag())) {
			// 取得小分类List
			cateList = binOLPTJCS33_IF.getSubCateList(map);
		}
		return SUCCESS;
	}
	...
	public String save() throws Exception {
		...
		try {
			...
			// 扩展属性
			map.put(ProductConstants.EXTENDPROPERTYLIST,form.getExtendPropertyList());
			map.putAll(paramMap);
			if(form.getProductId() == 0 ){
				// 产品图片
			...
			// 产品添加插表处理
			binOLPTJCS33_IF.tran_addProduct(map);
			...
			// if(!CherryChecker.isNullOrEmpty(form.getProductPriceSolutionID())){
			// return "BINOLPTJCS23_7"; // 如果是方案明细画面添加产品，则保存后返回方案明细画面
			// }
	...
	private List<Map<String, Object>> queryBrandList(Map<String, Object> map) {
		...
		if (!CherryConstants.BLANK.equals(path)) {
			...
			if (paths.length >= 1) {
				...
				brandMap.put(CherryConstants.BRAND_NAME,
						binOLCM05_BL.getBrandName(brandMap));
			...
			if (userInfo.getBIN_BrandInfoID() != CherryConstants.BRAND_INFO_ID_VALUE) {
				Map<String, Object> brandMap = new HashMap<String, Object>();
				brandMap.put(CherryConstants.BRANDINFOID,
						userInfo.getBIN_BrandInfoID());
				brandMap.put(CherryConstants.BRAND_NAME,
						userInfo.getBrandName());
				list.add(brandMap);
			} else {
				// 取得所属品牌List
	...
	private void doList(List<Map<String, Object>> cateList, String path) {
		...
		if (null != cateList && propValIds.length > 0) {
			...
			List<Map<String, Object>> cateValList = binOLPTJCS33_IF
					.getCateValList(map);

			for (Map<String, Object> cate : cateList) {
				// 分类ID
				int proplId1 = CherryUtil.obj2int(cate
						.get(ProductConstants.PROPID));
				List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
				for (Map<String, Object> cateVal : cateValList) {
					// 分类ID
					int proplId2 = CherryUtil.obj2int(cateVal
							.get(ProductConstants.PROPID));
	...
	private boolean validateForm() throws Exception {
		...
		if (CherryChecker.isNullOrEmpty(form.getUnitCode())
				|| getText("ECM00090").equals(form.getUnitCode())) {
			this.addFieldError(CherryConstants.UNITCODE,
			...
			this.addFieldError(
					...
					getText("ECM00020", new String[] { getText("PSS00001"),
							"20" }));
			isCorrect = false;
		} else if (!binOLPTJCS33_IF.checkUnitCode(userInfo, form.getUnitCode())) {
			// 厂商编码验证
			this.addFieldError(CherryConstants.UNITCODE,
			...
			// 所属组织
			paramsMap.put(CherryConstants.ORGANIZATIONINFOID,
					userInfo.getBIN_OrganizationInfoID());
			...
			if(form.getProductId() == 0){
				int unitCodeCount = binOLCM05_BL
						.getExistUnitCodeForPrtAndProm(paramsMap);
				if (unitCodeCount != 0) {
					this.addFieldError(
							CherryConstants.UNITCODE,
							getText("ECM00032",
		...
		for (int i = 0; i < barCode.size(); i++) {
			String barCodei = ConvertUtil.getString(barCode.get(i).get(
					CherryConstants.BARCODE));
			if (CherryChecker.isNullOrEmpty(barCodei, true)
					|| getText("ECM00091").equals(barCodei)) {
				this.addFieldError(
						CherryConstants.BARCODE + CherryConstants.UNLINE + i,
						getText("ECM00009",
				...
				this.addFieldError(
						CherryConstants.BARCODE + CherryConstants.UNLINE + i,
						getText("ECM00020", new String[] { getText("PSS00047"),
								"13" }));
				isCorrect = false;
			} else if (!binOLPTJCS33_IF.checkBarCode(userInfo, barCodei.trim())) {
				// barCode英数验证
				this.addFieldError(
						CherryConstants.BARCODE + CherryConstants.UNLINE + i,
						getText("ECM00095",
		...
		for (int i = 0; i < barCode.size() - 1; i++) {
			...
			for (int j = i + 1; j < barCode.size(); j++) {
				...
				if (barCodei.equalsIgnoreCase(barCodej)) {
					this.addFieldError(
							CherryConstants.BARCODE + CherryConstants.UNLINE
									+ j,
							getText("ECM00032",
									new String[] { getText("PSS00047") }));
					isCorrect = false;
		...
		Map<String, Object> paramsMap = new HashMap<String, Object>();
		paramsMap
				.put(CherryConstants.BRANDINFOID, form.getBrandInfoId().trim());

		for (int i = 0; i < barCode.size(); i++) {
			paramsMap.put(
					CherryConstants.BARCODE,
					ConvertUtil.getString(barCode.get(i).get(
							CherryConstants.BARCODE)));
		...
		if (CherryChecker.isNullOrEmpty(form.getNameTotal())) {
			this.addFieldError(CherryConstants.NAMETOTAL,
		...
		// }
			...
			String nameRule = binOLCM14_BL
					.getConfigValue("1132", ConvertUtil.getString(userInfo
							.getBIN_OrganizationInfoID()), ConvertUtil
							.getString(userInfo.getBIN_BrandInfoID()));
			int nameRuleLen = Integer.parseInt(nameRule);
			if (CherryUtil.mixStrLength(form.getNameTotal()) > nameRuleLen) {

				// 中文名称不能超过系统配置规则
				this.addFieldError(
						CherryConstants.NAMETOTAL,
						getText("ECM00101", new String[] { getText("PSS00002")
								+ getText("PSS00054") }));
				isCorrect = false;
			}
		}
		...
		if (!CherryChecker.isNullOrEmpty(form.getStandardCost())) {
			if (!CherryChecker.isFloatValid(form.getStandardCost(), 12, 2)) {
				this.addFieldError(
						"standardCost",
						getText("ECM00024", new String[] { getText("PSS00012"),
								"12", "2" }));
				isCorrect = false;
			}
		}
		...
		if (!CherryChecker.isNullOrEmpty(form.getMinSalePrice())
			...
			if (max < min) {
				this.addFieldError(
						"maxSalePrice",
						getText("ECM00033", new String[] { getText("EPT00006"),
								getText("EPT00005") }));
				isCorrect = false;
			}
		}
		...
		if (!CherryChecker.isNullOrEmpty(form.getOrderPrice())) {
			if (!CherryChecker.isFloatValid(form.getOrderPrice(), 12, 2)) {
				this.addFieldError(
						"orderPrice",
						getText("ECM00024", new String[] { getText("PSS00048"),
								"12", "2" }));
				isCorrect = false;
			}
		}
		...
		if (!CherryChecker.isNullOrEmpty(form.getRecNumDay())
			...
			this.addFieldError("recNumDay",
					getText("ECM00021", new String[] { getText("PSS00040") }));
			isCorrect = false;
		}
		// 开始销售日期
			...
			if (null != priceInfoList) {
				...
				for (int i = 0; i < priceListSize; i++) {
					...
					if (!CherryConstants.BLANK.equals(salePrice)) {
						...
						if (!CherryChecker
								.isNullOrEmpty(form.getMinSalePrice())) {
							float price = ConvertUtil.getFloat(salePrice);
							float min = ConvertUtil.getFloat(form
									.getMinSalePrice());
							if (price < min) {
								this.addFieldError(
										"salePrice_" + i,
						...
						if (!CherryChecker
								.isNullOrEmpty(form.getMaxSalePrice())) {
							float price = ConvertUtil.getFloat(salePrice);
							float max = ConvertUtil.getFloat(form
									.getMaxSalePrice());
							if (price > max) {
								this.addFieldError(
										"salePrice_" + i,
										getText("ECM00052", new String[] {
												getText("PSS00017"),
												getText("EPT00006") }));
								isCorrect = false;
							}
