 ...
 */
public class BINOLPTJCS27_BL implements BINOLPTJCS27_IF {
	...
	@Resource(name="binOLPTJCS27_Service")
	private BINOLPTJCS27_Service binOLPTJCS27_Service;
	...
	public int tran_updProduct(Map<String, Object> map) throws Exception {
		...
		// 产品属性信息更新
		result = binOLPTJCS27_Service.updProduct(map);
	...
	private int updPrtVendor(Map<String, Object> map) throws Exception {
		...
		// 业务日期
		String bussDate = binOLPTJCS27_Service.getBusDate(map);
		
		String sysdateTime = binOLPTJCS27_Service.getSYSDateTime();
		...
		if (CherryConstants.VALIDFLAG_DISABLE.equals(map.get("validFlag"))) {
			for(Map<String, Object>closeTimeMap : allList){
				...
				// 更新产品厂商的validFlag为0
				binOLPTJCS27_Service.updPrtVendor(paramsMap);
				// 更新关系表的ClosingTime字段为当前时间（业务时间）
				binOLPTJCS27_Service.updateClosingTime(paramsMap);
			}
			return prtVendorId;
		}
		...
		for (Map<String, Object> code : addCodeList) {
			code.putAll(map);
			prtVendorId = binOLPTJCS23_Service.insertProductVendor(code);
		}
		// =========== 更新的产品条码 ============ //
		for (Map<String, Object> updMap : updCodeList) {
			...
			binOLPTJCS27_Service.updPrtVendor(paramMap);
			// 更新产品条码对应关系信息
		...
		for (Map<String, Object> delMap : delCodeList) {
			...
			binOLPTJCS27_Service.updPrtVendor(delMap);
			// 更新产品条码对应关系信息
			delMap.put(ProductConstants.CLOSING_TIME, closingTime);
			binOLPTJCS27_Service.updPrtBarCode(delMap);
		}
		// =========== 不变的产品条码 ============ //
		for (Map<String, Object> unMap : unChangeCodeList) {
			...
			if ("0".equals(validFlag)) {
				...
				binOLPTJCS27_Service.updPrtVendor(unMap);
				// 更新停用日时
...
//			}
			if (null != priceInfoList) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (CherryConstants.BLANK.equals(option)) {
						...
						// 更新产品销售价格
						binOLPTJCS27_Service.updProductPrice(priceMap);
					} else if (ProductConstants.OPTION_2.equals(option)) {
						// 删除产品销售价格
						binOLPTJCS27_Service.delProductPrice(priceMap);
					}
				}
			}
	...
	private void updPrtCategory(Map<String, Object> map) throws Exception {
		// 删除数据库中产品分类信息
		binOLPTJCS27_Service.delPrtCategory(map);
		// 产品分类信息
	...
	private void updPrtBOM(Map<String, Object> map,
		...
		if (mode.equals(ProductConstants.MODE_1)
			...
			for (Map<String, Object> barCode : barCodeList) {
				...
				// 删除产品BOM表
				binOLPTJCS27_Service.delProductBOM(prtMap);
				if (!ProductConstants.OPTION_2.equals(option)) {
	...
	private Map<String, Object> getUpdMap(Map<String, Object> map) {
		...
		// 系统时间
		String sysDate = binOLPTJCS27_Service.getSYSDate();
		// 作成日时
	...
	public int getActUsePrtCount(Map<String, Object> map){
		return binOLPTJCS27_Service.getActUsePrtCount(map);
	}
	...
	public int getActPrtCount(Map<String, Object> map){
		String bussDate = binOLPTJCS27_Service.getBussinessDate(map);
		map.put("bussDate", bussDate); // 业务时间
		return binOLPTJCS27_Service.getActPrtCount(map);
	...
	public void updPrtBarCode(Map<String, Object> map){
		...
		Map<String, Object> prtBCMap = binOLPTJCS27_Service.getPrtBarCode(map);
		...
		binOLPTJCS27_Service.updPrtBarCode(map);
	}
}
