...
import com.cherry.pt.rps.bl.BINOLPTRPS44_BL;
import com.cherry.pt.rps.form.BINOLPTRPS44_Form;
 ...
 */
public class BINOLPTRPS44_Action extends BaseAction implements ModelDriven<BINOLPTRPS44_Form> {
	...
	/** 统计销售信息IF **/
	@Resource
	private BINOLPTRPS44_BL binOLPTRPS44_BL;
	...
	public String search() throws Exception {
		...
		//产品ID
		String selPrtBarCodeStr = form.getSelPrtBarCodeArr();
		map.remove("selPrtBarCodeStr");
		if(!"null".equalsIgnoreCase(selPrtBarCodeStr) && null != selPrtBarCodeStr && !"".equals(selPrtBarCodeStr)) {
			List selPrtBarCodeList = ConvertUtil.json2List(selPrtBarCodeStr);
			map.put("selPrtBarCodeArr", selPrtBarCodeList);
		}
...
//					Map<String, Object> minMaxDateValueMap = binOLPTRPS15_Service.getMinMaxDateValue(map);
		...
		if(map.get("saleCountFlag") != null) {//此时saleCountHistoryID != null
			...
			int modifiedSaleCount = binOLPTRPS44_BL.getModifiedSaleCount(map);
		...
		// 查询销售统计信息0
		saleCountInfo = binOLPTRPS44_BL.getSaleCountInfo(map);
		if(saleCountInfo != null && !saleCountInfo.isEmpty()) {
			...
			if(count > 0) {
				// 查询销售统计信息List
				saleCountInfoList = binOLPTRPS44_BL.getSaleCountInfoList(map);
			}
		}
		return SUCCESS;
	}
...
//	    				Map<String, Object> minMaxDateValueMap = binOLPTRPS15_Service.getMinMaxDateValue(map);
            ...
            byte[] byteArray = binOLCM37_BL.exportExcel(map, binOLPTRPS44_BL);
	...
	/** 统计销售信息Form **/
	private BINOLPTRPS44_Form form = new BINOLPTRPS44_Form();

	@Override
	public BINOLPTRPS44_Form getModel() {
		return form;
	}
