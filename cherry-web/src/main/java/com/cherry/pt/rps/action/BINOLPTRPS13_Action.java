 ...
 * @version  2013-3-6
 */
public class BINOLPTRPS13_Action extends BaseAction implements
		ModelDriven<BINOLPTRPS13_Form> {

	private static final long serialVersionUID = 8947628908345198289L;
	
	private static Logger logger = LoggerFactory.getLogger(BINOLPTRPS13_Action.class.getName());

	// FROM
	private BINOLPTRPS13_Form form = new BINOLPTRPS13_Form();
...
//				.getSysDateTime(CherryConstants.DATE_PATTERN));
		...
		//是否显示产品入出库成本
		boolean config  = binOLCM14_BL.isConfigOpen("1367",ConvertUtil.getString(map.get(CherryConstants.ORGANIZATIONINFOID)),ConvertUtil.getString(map.get(CherryConstants.BRANDINFOID)));
		if(config){
			form.setIsShow("1");
		}else{
			form.setIsShow("0");
		...
		// 支付方式
//		payTypeList = binOLPTRPS13_BL.getPayTypeList(map);
	...
	public void exportCheck() throws Exception {
		...
		int count = binOLPTRPS13_BL.getExportDetailCount(map);
		// Excel导出最大数据量
	...
	public String export() throws Exception {
		...
		try {
			...
			excelStream = new ByteArrayInputStream(binOLCM37_BL.fileCompression(binOLPTRPS13_BL.exportExcel(map), extName+".xls"));
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			this.addActionError(getText("EMO00022"));
			return CherryConstants.GLOBAL_ACCTION_RESULT;
		}
		return "BINOLPTRPS13_excel";
	}
    ...
    public String exportCsv() throws Exception {
    	...
    	try {
    		...
    		if(count > maxCount) {
    			...
    			String tempFilePath = binOLPTRPS13_BL.exportCSV(map);
    ...
    public String exportMain() throws Exception {
    	...
    	try {
    		...
    		int count = binOLPTRPS13_BL.getSaleRecordCount(map);
    		// 导出CSV最大数据量
    		int maxCount = CherryConstants.EXPORTCSV_MAXCOUNT;
    		...
    		if(count > maxCount) {
    			...
    			String tempFilePath = binOLPTRPS13_BL.exportMain(map);
        		if(tempFilePath != null) {
        			msgParam.put("exportStatus", "1");
        			msgParam.put("message", getText("ECM00096"));
        			msgParam.put("tempFilePath", tempFilePath);
    ...
    public String exportDetail() throws Exception {
    	...
    	try {
    		...
    		int count = binOLPTRPS13_BL.getExportDetailFilterKDCount(map);
    		// 导出CSV最大数据量
    		int maxCount = CherryConstants.EXPORTCSV_MAXCOUNT;
    		...
    		if(count > maxCount) {
    			...
    			String tempFilePath = binOLPTRPS13_BL.exportDetail(map);
        		if(tempFilePath != null) {
        			msgParam.put("exportStatus", "1");
        			msgParam.put("message", getText("ECM00096"));
        			msgParam.put("tempFilePath", tempFilePath);
...
//		count = binOLPTRPS13_BL.getSaleRecordCount(searchMap);
	...
	private void saleSum(Map<String,Object> searchMap ,List<Map<String,Object>> saleProPrmListPara){
		...
		//指定商品统计详细（指定商品的总成本价）
		//Map<String, Object> SaleProPrmCostPriceList = binOLPTRPS13_BL.getSaleProPrmCostPriceList(searchMap);
...
//				saleSumInfo.put("saleSumCostPrice", SaleProPrmCostPriceList.get("costPrice"));
//			}
		}
	}
...
//		Map<String, Object> searchSDMap = getSearchMapBySDArea(searchMap);
		sumInfoBySaleDateArea = binOLPTRPS13_BL.getSumInfo(searchMap);
	...
	public void indSearchPrmPrt() throws Exception{
		...
		// 取得促销品信息结果String
		String resultString = binOLPTRPS13_BL.indSearchPrmPrt(map);
		// 将数据传到页面
	...
	private void getProPrm(Map<String, Object> map){
		...
		List<String> saleProPrmListByPrt = new ArrayList<String>();
		for(String itemString : salePrtVendorIdList){
			...
			if("N".equals(prtType)){
				...
				saleProPrmListByPrt.add(prmPrtVendorId);
			} 
		...
		map.put("saleProPrmListByPrt", saleProPrmListByPrt);
		...
		List<String> jointProPrmListByPrt = new ArrayList<String>();
		for(String itemString : joinPrtVendorIdList){
			...
			if("N".equals(prtType)){
				prtList.add(prmPrtVendorId);
				jointProPrmListByPrt.add(prmPrtVendorId);
		...
		map.put("jointProPrmListByPrt", jointProPrmListByPrt);
...
//		}
	...
	public String invoiceFlagRevise() {
		try {
			...
			//发票类型修改
			binOLPTRPS13_BL.updateInvoiceFlag(list);
			return SUCCESS;
	...
	@Override
	public BINOLPTRPS13_Form getModel() {
		return form;
	}
}
