...
import com.cherry.ss.prm.bl.BINOLSSPRM84_BL;
...
public class BINOLSSPRM84_Action extends BaseAction implements ModelDriven<BINOLSSPRM14_Form>{

	private static final long serialVersionUID = 3350345953998512219L;
	
	private static final Logger logger = LoggerFactory.getLogger(BINOLSSPRM84_Action.class);
	...
	@Resource
	private BINOLSSPRM84_BL binOLSSPRM84_BL;
...
//		form.setSearchPrmEndDate(DateUtil.addDateByDays(CherryConstants.DATE_PATTERN, sysDateStr, Integer.parseInt(PromotionConstants.PRM_ACT_SPACE_OF_TIME)));
	...
	public String searchActive () throws Exception{
		...
		if(0 != form.getPrmCounterId()){
			Map<String,Object> cntMap = binOLSSPRM84_BL.getCounterInfo(form.getPrmCounterId());
			if(null !=cntMap){
				map.putAll(cntMap);
			}
		}
		// 取得分页件数
		int count = binOLSSPRM84_BL.getActiveCount(map);
		// 查询促销活动一览
		List activeList = binOLSSPRM84_BL.getActiveList(map);
		// 设定活动信息List
		form.setActiveInfoList(activeList);
		// form表单设置
		form.setITotalDisplayRecords(count);
		form.setITotalRecords(count);
		return "BINOLSSPRM84_1";
	}
...
//		String errMsg = binOLMOCOM01_BL.getResourceValue("BINOLSSPRM14", language, "publishMsg_0");
		...
		if(form.getCheck() == 1){// 审核促销
			try {
				binOLSSPRM84_BL.tran_check(map);
			} catch (Exception e) {
...
//				errMsg = binOLMOCOM01_BL.getResourceValue("BINOLSSPRM14", language, "publishMsg_102");
			...
			if("0".equals(errCode)){
				try {
					...
					Map<String,Object> actInfo = binOLSSPRM84_BL.getActivityInfo(form.getActiveID());
					...
					if(null != result){
						...
						if(!"0".equals(errCode)){
							...
							int rst = binOLSSPRM84_BL.sendMqNotice(map,mainData);
...
//					errMsg = binOLMOCOM01_BL.getResourceValue("BINOLSSPRM14", language, "publishMsg_101");
	...
	public void indSearchPrmPrt () throws Exception{
		// 取得session信息
		Map<String, Object> map  = this.getSessionInfo();
		List resultList = binOLSSPRM84_BL.indSearchPrmPrt(map);
		// 取得促销品信息结果String
		String resultString = binOLSSPRM84_BL.parseStringByPrmPrt(resultList);
		// 将数据传到页面
	...
	public void indSearchPrmLocation () throws Exception{
		...
		// 取得session信息
		Map<String, Object> map  = this.getSessionInfo();
		List resultList = binOLSSPRM84_BL.indSearchPrmLocation(map);
		String resultString = binOLSSPRM84_BL.parseStringByLocation(resultList);
		// 将数据传到页面
		ConvertUtil.setResponseByAjax(response, resultString);
	}
	...
	public void indSearchPrmName () throws Exception{
		...
		// 取得session信息
		Map<String, Object> map  = this.getSessionInfo();
		List resultList = binOLSSPRM84_BL.indSearchPrmName(map);
		String resultString = binOLSSPRM84_BL.parseStringByPrmName(resultList);
		// 将数据传到页面
		ConvertUtil.setResponseByAjax(response, resultString);
	}
	...
	public void indSearchPrmGrpName () throws Exception{
		...
		// 取得session信息
		Map<String, Object> map  = this.getSessionInfo();
		List resultList = binOLSSPRM84_BL.indSearchPrmGrpName(map);
		String resultString = binOLSSPRM84_BL.parseStringByPrmGrpName(resultList);
		// 将数据传到页面
		ConvertUtil.setResponseByAjax(response, resultString);
	}
