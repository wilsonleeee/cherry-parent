/*  
 * @(#)ProFlowRA_YT_FN.java    1.0 2013.03.21
 *      
 ...
 */
public class ProFlowRA_YT_FN implements FunctionProvider{
    ...
    private void startFlow(Map arg0, Map arg1, PropertySet propertyset){
        ...
        //所属组织ID
        propertyset.setString("BIN_OrganizationInfoID", ConvertUtil.getString(mainData.get("BIN_OrganizationInfoID")));
        //流程模型所属品牌
        propertyset.setString("BIN_BrandInfoID", ConvertUtil.getString(mainData.get("BIN_BrandInfoID")));
        ...
        paramMap.put("CurrentUnit", "ProFlowRA_YT_FN");
    ...
    private void updateRADetail(Map arg0, Map arg1, PropertySet propertyset) {
        ...
        if(null != form){
            ...
            for(int i=0;i<productVendorIDArr.length;i++){
                ...
                detailMap.put("BIN_InventoryInfoID", CherryUtil.obj2int(form.getInventoryInfoID()));
                detailMap.put("BIN_LogicInventoryInfoID", CherryUtil.obj2int(form.getLogicInventoryInfoID()));
...
//        mainData.put("TotalAmount", mainDataRA.get("TotalAmount"));
    ...
    private void changeCounterStock(Map arg0, Map arg1, PropertySet propertyset) {
        //判断PropertySet里是否有BIN_ProductReturnID，有是柜台退库，没有是后台退库，需要生成退库单。
        Collection<String> col = propertyset.getKeys("BIN_ProductReturnID");
        if(col.size() == 0){
