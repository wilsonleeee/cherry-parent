/*  
 * @(#)BINOLSTBIL13_Action.java     1.0 2012/7/24      
 *      
...
import com.cherry.st.bil.form.BINOLSTBIL19_Form;
...
public class BINOLSTBIL19_Action extends BaseAction implements
ModelDriven<BINOLSTBIL19_Form>{

	private static final long serialVersionUID = 6995451060006803090L;
	/** 异常处理 */
	private static final Logger logger = LoggerFactory
			.getLogger(BINOLSTBIL19_Action.class);
    /** 参数FORM */
    private BINOLSTBIL19_Form form = new BINOLSTBIL19_Form();
    ...
    @Resource(name="binOLSTBIL19_BL")
    private BINOLSTBIL19_IF binOLSTBIL19_BL;

    @Override
    public BINOLSTBIL19_Form getModel() {
        return form;
    }
    ...
    public String search() throws Exception {
        ...
        // 取得退库申请单总数
        int count = binOLSTBIL19_BL.searchSaleReturnRequestCount(searchMap);
        if (count > 0) {
            // 取得退库申请单List
            form.setSaleReturnRequestList(binOLSTBIL19_BL.searchSaleReturnRequestList(searchMap));
        }
        form.setSumInfo(binOLSTBIL19_BL.getSumInfo(searchMap));
        // form表单设置
        form.setITotalDisplayRecords(count);
        form.setITotalRecords(count);
        // AJAX返回至dataTable结果页面
        return "BINOLSTBIL19_1";
    }
	...
	public String export() {
        ...
        try {
        	Map<String, Object> searchMap = getSearchMap();
    		// 设置排序ID（必须）
        	searchMap.put("SORT_ID", "tradeDate desc");
            String language = ConvertUtil.getString(searchMap.get(CherryConstants.SESSION_LANGUAGE));
            String fileName = binOLMOCOM01_BL.getResourceValue("BINOLSTBIL19", language, "downloadFileName");
            downloadFileName = fileName + ".zip";
            excelStream=new ByteArrayInputStream(binOLCM37_BL.fileCompression(binOLSTBIL19_BL.exportExcel(searchMap), fileName+".xls"));
            return SUCCESS;
        } catch (Exception e) {
    ...
    private Map<String, Object> getSearchMap() throws JSONException {
        ...
        // 产品名称
        map.put("nameTotal", ConvertUtil.getString(form.getNameTotal()).trim());
