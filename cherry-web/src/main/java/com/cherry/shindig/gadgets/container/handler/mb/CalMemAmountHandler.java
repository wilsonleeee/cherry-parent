 ...
 */
@Service(name = "calMemAmount")
public class CalMemAmountHandler {
	...
	public Future<DataCollection> getRuleCalState(RequestItem request) throws Exception {
		...
		// 取得总会员销售额和新会员销售额
		Map<String, Object> memAmountMap = ruleCalStateService.getMemAmount(map);
		
		for(int i = 0; i < dateList.size(); i++) {
			String key = (String)dateList.get(i).get("key");
			map.putAll(dateList.get(i));
			// 取得活跃会员数
			Map<String, Object> activeMemAmountMap = ruleCalStateService.getActiveMemAmount(map);
			memAmountMap.put("activeMemAmount_"+key, activeMemAmountMap.get("activeMemAmount"));
		}
		
		// 返回结果
		Map resultData = new HashMap();
		resultData.putAll(memAmountMap);
