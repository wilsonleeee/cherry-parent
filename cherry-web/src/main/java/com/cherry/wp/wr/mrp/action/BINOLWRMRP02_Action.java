...
import com.cherry.wp.wr.mrp.form.BINOLWRMRP02_Form;
 ...
 */
public class BINOLWRMRP02_Action extends BaseAction implements ModelDriven<BINOLWRMRP02_Form> {

	private static final long serialVersionUID = -7528623652025092226L;
	
	private static Logger logger = LoggerFactory.getLogger(BINOLWRMRP02_Action.class.getName());
	
	@Resource
	private BINOLWRMRP02_IF binOLWRMRP02_BL;
	...
	public String search() throws Exception {
		...
		binOLWRMRP02_BL.setCondition(map);
		
		int count = binOLWRMRP02_BL.getMemberCount(map);
		form.setITotalDisplayRecords(count);
		form.setITotalRecords(count);
		if(count != 0) {
			memberInfoList = binOLWRMRP02_BL.getMemberList(map);
    ...
    public String exportExcel() throws Exception {
        ...
        try {
    		...
    		// 获取导出参数
    		Map<String, Object> exportMap = binOLWRMRP02_BL.getExportParam(map);
    		
    		String zipName = CherryUtil.getResourceValue("wp", "BINOLWRMRP02", language, "downloadFileName");
    		downloadFileName = zipName+".zip";
            
    		// 导出excel处理
        	byte[] byteArray = binOLCM37_BL.exportExcel(exportMap, binOLWRMRP02_BL);
    ...
    public String exportCsv() throws Exception {
    	...
    	try {
			...
			if(count > maxCount) {
				...
				String tempFilePath = binOLWRMRP02_BL.exportCSV(map);
	...
	/** 新会员入会统计Form **/
	private BINOLWRMRP02_Form form = new BINOLWRMRP02_Form();

	@Override
	public BINOLWRMRP02_Form getModel() {
		return form;
	}
