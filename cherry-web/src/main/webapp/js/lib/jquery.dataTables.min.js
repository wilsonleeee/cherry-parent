...
_fnAjustColumnSizing(oSettings);_fnDraw(oSettings);_fnSaveState(oSettings);_fnCalculateColumnWidths(oSettings);};this.fnPageChange=function(sAction,bRedraw)
...
{_fnProcessingDisplay(oSettings,true);oSettings.fnServerData.call(oSettings.oInstance,oSettings.sAjaxSource,[],function(json){var json=_htmlToJson(json,oSettings);for(i=0;i<json.aaData.length;i++)
...
function _htmlToJson(msg,oSettings){var callbackFun=oSettings.callbackFun;if(callbackFun!=null&&$.isFunction(callbackFun)){callbackFun(msg);}
var divObject=document.createElement("div");if($("#ajaxResult").length!=0){divObject="#ajaxResult";}
$(divObject).html(msg);$("#errorMessage").empty();if($("#sEcho",divObject).length==0){if(msg.indexOf('sEcho')>-1){return eval('('+msg+')');}
if($("#actionResultDiv").length>0){$("#errorMessage").html(msg);}else{if(msg.indexOf('pageid=login')>-1){$("#timeoutform").submit();}else{$('body').html(msg);}}
_fnProcessingDisplay(oSettings,false);return{iTotalRecords:0,iTotalDisplayRecords:0,aaData:[]};}
var json='{"sEcho":"'+$("#sEcho",divObject).text()+'","iTotalRecords":"'+$("#iTotalRecords",divObject).text()+'","iTotalDisplayRecords":"'+$("#iTotalDisplayRecords",divObject).text()+'","aaData":[';var $ul=$("#aaData",divObject).find("ul");$ul.each(function(i){json+='[';var $li=$(this).find("li");$li.each(function(j){json+="'"+$.trim($(this).html()).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+"'";if(j+1!=$li.length){json+=",";}});json+=']';if(i+1!=$ul.length){json+=",";}});json+=']}';$(divObject).empty();json=json.replace(/\r\n/g,'\\r\\n').replace(/\n/g,'\\n');return eval('('+json+')');}
...
oSettings.fnServerData.call(oSettings.oInstance,oSettings.sAjaxSource,aoData,function(json){var json=_htmlToJson(json,oSettings);_fnAjaxUpdateDraw(oSettings,json);});return false;}
else
{return true;}}
...
nScroller.className=oClasses.sScrollWrapper;nScrollHead.className=oClasses.sScrollHead;nScrollHeadInner.className=oClasses.sScrollHeadInner;nScrollBody.className=oClasses.sScrollBody;nScrollFoot.className=oClasses.sScrollFoot;nScrollFootInner.className=oClasses.sScrollFootInner;nScroller.style.overflow="auto";nScroller.style.width="100%";nScrollFoot.style.overflow="hidden";nScrollHead.style.border="0";nScrollFoot.style.border="0";nScrollHeadInner.style.width="150%";nScrollHeadTable.removeAttribute('id');nScrollHeadTable.style.marginLeft="0";oSettings.nTable.style.marginLeft="0";if(nTfoot!==null)
...
{o.nTable.style.width=_fnStringToCss(iSanityWidth);if($(o.nTable).outerWidth()>iSanityWidth-o.oScroll.iBarWidth)
...
nScrollHeadTable.style.width=_fnStringToCss($(o.nTable).outerWidth());nScrollHeadInner.style.width=_fnStringToCss($(o.nTable).outerWidth());if(o.nTFoot!==null)
{var
...
{return;}
var
iMax=oSettings.fnRecordsTotal(),iTotal=oSettings.fnRecordsDisplay(),iPages=Math.ceil((oSettings.fnRecordsDisplay())/oSettings._iDisplayLength),sMax=oSettings.fnFormatNumber(iMax),sTotal=oSettings.fnFormatNumber(iTotal),sPages=oSettings.fnFormatNumber(iPages),sOut;if(oSettings.oScroll.bInfinite)
...
{sOut=oSettings.oLanguage.sInfo.replace('_PAGES_',sPages).replace('_TOTAL_',sTotal)+
oSettings.oLanguage.sInfoPostFix;}
else
{sOut=oSettings.oLanguage.sInfo.replace('_PAGES_',sPages).replace('_TOTAL_',sTotal)+' '+
