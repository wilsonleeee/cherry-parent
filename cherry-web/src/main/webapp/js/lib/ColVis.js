...
ColVis = function( oDTSettings )
{
	...
	/** huzude 2010-01-11 修改 多个datatable 列选择div重叠 **/
	if (this.s.dt.oInit.colVisFlag==undefined){
		this._fnConstruct();
	}
	return this;
};
...
//			"ColVis_Button TableTools_Button ui-button ui-state-default";
		...
		$(nButton).click( function (e) {
			var showHide = $('input',this).prop('checked')===true ? false : true;
			if ( e.target.nodeName.toLowerCase() == "input" )
			{
				showHide = $('input',this).prop('checked');
			}
	...
	{
		var $tableMulti = $("#"+this.s.dt.sTableId+"_colSetting");
		var	that = this,
//		  nButton = document.createElement('a'),
//		  nSpan = document.createElement('span'),
		nButton = $tableMulti.length > 0 ? $tableMulti : $("a.setting");
...
//		nButton.innerHTML = text;
	...
	"_fnDomCollection": function ()
	{
		...
		nHidden.innerHTML = '<div class="sidemenu2-header clearfix"><strong class="left">选择显示的列</strong><span class="close right"><span class="ui-icon ui-icon-close"></span></span></div>';
		
		$(nHidden).children('.sidemenu2-header').children('span.close').click( function () {
	...
	"_fnCollectionShow": function ()
	{
		...
		var iDivX = parseInt(oPos.left, 10) - 90;
...
//		nBackground.style.width = ((iWinWidth>iDocWidth)? iWinWidth : iDocWidth) +"px";
		...
		nBackground.style.width = "100%";
		$(nBackground).css("min-width","992px");
		//=================================背景遮盖修改结束=========================================== //
		...
		{
			/** 2011/02/10 by 胡祖德 **/
			// 添加nButton的判断,防止窗口过小点击按钮造成异常
			if (!(typeof nButton == "undefined")){
		...
		setTimeout( function () {
			...
			$(nBackground).animate({"opacity": 0.5}, 100);
		}, 10 );
	...
	"_fnCollectionHide": function (  )
	{
		...
		if ( !this.s.hidden && this.dom.collection !== null )
		{
			...
			$(this.dom.collection).animate({"opacity": 0}, 100, function (e) {
				this.style.display = "none";
			} );
			
			$(this.dom.background).animate({"opacity": 0}, 100, function (e) {
