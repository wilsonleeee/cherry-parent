...
//							"This is most likely to be caused by a JSON formatting error." );
		...
		function _fnInitalise ( oSettings )
		{
			...
			if ( oSettings.sAjaxSource !== null && !oSettings.oFeatures.bServerSide )
			{
				...
				oSettings.fnServerData.call( oSettings.oInstance, oSettings.sAjaxSource, [], function(json) {
					// 2010/10/19 修改Start
					var json = _htmlToJson(json, oSettings);
					// 2010/10/19 修改End
		...
		// 2010/10/19 修改Start
		function _htmlToJson(msg, oSettings) {
			
			var callbackFun = oSettings.callbackFun;
			// 回调函数
			 if(callbackFun != null && $.isFunction(callbackFun)){
				 callbackFun(msg);
			 }
			var divObject = document.createElement("div");
			if($("#ajaxResult").length != 0) {
				divObject = "#ajaxResult";
			}
			$(divObject).html(msg);
			$("#errorMessage").empty();
			if($("#sEcho",divObject).length == 0) {
				if(msg.indexOf('sEcho') > -1) {
					return eval('('+msg+')');
				}
				if($("#actionResultDiv").length > 0) {
			...
			var json = '{"sEcho":"'+$("#sEcho",divObject).text()+'","iTotalRecords":"'+$("#iTotalRecords",divObject).text()+'","iTotalDisplayRecords":"'+$("#iTotalDisplayRecords",divObject).text()+'","aaData":[';
			var $ul = $("#aaData",divObject).find("ul");
			$ul.each(function(i){
				json += '[';
				var $li = $(this).find("li");
				$li.each(function(j){
					json += "'"+$.trim($(this).html()).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+"'";
					if(j+1 != $li.length) {
						json += ",";
					}
				});
		...
		function _fnFeatureHtmlTable ( oSettings )
		{
			...
			// ie8样式调整修改 2011-7-6 start
			nScroller.style.overflow = "auto";
			nScroller.style.width = "100%";
			//nScrollHead.style.overflow = "hidden";
			nScrollFoot.style.overflow = "hidden";
			//nScrollBody.style.overflow = "auto";
			// ie8样式调整修改 2011-7-6 end
		...
		function _fnScrollDraw ( o )
		{
			...
			else
			{
				   ...
				   $(nScrollBody).height() < $(o.nTable).height() )
				{
					...
					// ================== 2010-10-28 IE7 表格翻页宽度窄 问题 开始 ================== //
					// o.nTable.style.width = _fnStringToCss( iSanityWidth-o.oScroll.iBarWidth );
					o.nTable.style.width = _fnStringToCss( iSanityWidth );
					// ================== 2010-10-28 IE7 表格翻页宽度窄 问题 结束 ================== //
...
//				sStart = oSettings.fnFormatNumber( iStart ), sEnd = oSettings.fnFormatNumber( iEnd ),
				...
				sPages = oSettings.fnFormatNumber( iPages ),
				sOut;
...
//					alert( sAlert );
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "colVisFlag");
				_fnMap( oSettings, oInit, "callbackFun" );
