...
$("<span>"+o.togglerContent_closed+"</span>").data("layoutRole","togglerContent").data("layoutEdge",pane).addClass("content content-closed").css("display","none").appendTo($T).hover(addHover,removeHover);enableClosable(pane);}
...
$Frames.each(function(){$("#"+$(this).data("layoutMaskID")).css($(this).position()).css({width:this.offsetWidth+"px",height:this.offsetHeight+"px"});});newSize=resizerPos-sC["inset"+c.side];manualSizePane(pane,newSize);}});};var destroy=function(){$(window).unbind("."+sID);$(document).unbind("."+sID);var
...
o=options[pane],$P=$Ps[pane],$R=$Rs[pane],trigger=o.slideTrigger_open;if(!$R||(enable&&!o.slidable))return;if(trigger.match(/mouseover/))
