...
* @return
*/
function SSPRM22_openPromotionSearchBox (thisObj){
	SSPRM22_clearActionMsg();
 ...
 * 
 * */
function SSPRM22_getHtmlFun(info){
	if(!info){
		return ;
	}
	var html = [];
	...
	//数量
	html.push('<td id="dataTd6"><span><input name="quantityuArr" id="quantityuArr" class="text-number" size="10" maxlength="9" onchange="SSPRM22_changeCount(this);" ondragstart="return false;" ondragover="return false;" ondragend="return false;" ondrop="return false;"></input></span></td>');
	//盘差数量
	html.push('<td id="gainCount" style="text-align:right;"></td>');
	//盘差金额
	html.push('<td id="gainMoney" style="text-align:right;"></td>');
	//备注
	...
	html.push('<input type="hidden" id="prmVendorId" name="prmVendorId" value="'+info.proId+'"/>');
 ...
 * @param thisObj
 */
function SSPRM22_changeCount(thisObj){
	...
	$tr_obj.find('#gainMoney').html(money.toFixed(2));
 ...
 * @param thisObj
 */
function SSPRM22_chooseDepart(thisObj){
	SSPRM22_clearActionMsg();
	//清除现有明细
	$("#databody > tr").remove();
	$('#allSelect').prop("checked",false);
	...
	ajaxRequest(url,param,SSPRM22_changeDepotDropDownList);	
 ...
 * @return
 */
function SSPRM22_changeDepotDropDownList(data){	
	//data为json格式的文本字符串	
	...
	ajaxRequest(url,param,SSPRM22_changeLogicDepotDropDownList);
}
 ...
 * @return
 */
function SSPRM22_changeLogicDepotDropDownList(data){
	//data为json格式的文本字符串	
	var member = eval("("+data+")");    //包数据解析为json 格式  
	$("#logicDepot ").empty();
	if (null == member){
 ...
 * 点击确定按钮
 */
function SSPRM22_submitFormSave(){	
	SSPRM22_clearActionMsg();
	if(!SSPRM22_checkIsStock()){
		return;
	}
	if(!SSPRM22_checkData()){
		return;
	}
	...
	cherryAjaxRequest({
		...
		callback:SSPRM22_submitSuccess
	});	
}
 ...
 * @param msg
 */
function SSPRM22_submitSuccess(msg){
	submitflag=true;
 ...
 * @return
 */
function SSPRM22_clearActionMsg(){
