function BINOLSSPRM88_4(){};
BINOLSSPRM88_4.prototype={
		"index":1000000,
		// 添加组合框
...
//				}
			...
			$popInputs.click(function() {
				...
				if ($input.is(":checked")) {
					...
					if('ALL' == info.id){
						...
						html += '<a href="#" onclick="PRM88_4.popSetRangeDialog(this,\'' + info.id + '\');return false;">选择</a>';
					}
	...
	//产品排除页面弹出框
	"popRangeExceptDialog":function(_this){
		var dialogId = 'productExpectDivDialog';
		var $dialog = $("#" + dialogId);
		if($dialog.length == 0) {
		...
		var param="execLoadType=2&filterType=2";
		//var url;
		//var param;
		cherryAjaxRequest({
			url: url,
			...
			callback: function(msg){
				...
				var dialogSetting = {
					...
					confirmEvent:function(){
						PRM88_4.productExceptConfirm(_this);
	...
	"productExceptConfirm":function(_this){
		var excelProductExcept=$("#excelProduct_b").val();
		if(excelProductExcept!=""){
			var excelProductExceptList=eval("("+excelProductExcept+")");
			//var excelProductExceptList=excelProductExcept;
			var html='';
            var upMode=$("#productUpMode").val();
			$.each(excelProductExceptList.productJson, function (i, item) {
				html += '<div class="tag">'
					+ '<input class="NOSUBMIT" type="hidden" name="key" value="PRODUCT"/>'
					+ '<input class="NOSUBMIT" type="hidden" name="name" value="特定产品"/>'
					+ '<div class="tag_l">特定产品</div>'
					+ '<div class="tag_r">';
				html += '<div><input class="NOSUBMIT" type="hidden" name="val" value="' + item.unitCode + '+' + item.barCode +'"/>'
					+ '<input class="NOSUBMIT" type="hidden" name="valText" value="' + item.productName + '"/>' + item.productName + '</div>';;
				html += '</div><span class="icon_del" onclick="$(this).parent().remove();return false;"></span></div>';
			});
			//for(var i=0;i<excelProductExceptList.length;i++){
			//	html += '<div class="tag">'
			//		+ '<input class="NOSUBMIT" type="hidden" name="key" value="PRODUCT"/>'
			//		+ '<input class="NOSUBMIT" type="hidden" name="name" value="特定产品"/>'
			//		+ '<div class="tag_l">'+ excelProductExceptList[i].productName + '</div>'
			//		+ '<div class="tag_r">';
			//	html += '<div><input class="NOSUBMIT" type="hidden" name="val" value="' + excelProductExceptList[i].unitCode + '+' + excelProductExceptList[i].barCode +'"/>'
			//		+ '<input class="NOSUBMIT" type="hidden" name="valText" value="' + excelProductExceptList[i].productName + '"/>' + excelProductExceptList[i].productName + '</div>';;
			//	html += '</div><span class="icon_del" onclick="$(this).parent().remove();return false;"></span></div>';
			//}
			var $target = $(_this).parents("td").find("span:first");
 			if (upMode=='2'){
				$target.html("");
			}
			$target.append(html);
	...
	//购买条件页面弹出框
	"popShopProductDialog":function(_this){
		var dialogId = 'popShopProductDialog';
		var $dialog = $("#" + dialogId);
		if($dialog.length == 0) {
			$("body").append('<div style="display:none" id="'+dialogId+'"></div>');
		} else {
		...
		var url = '/Cherry/ss/BINOLSSPRM88_execlLoadInit';
		var param="filterType=1&execLoadType=2";
		cherryAjaxRequest({
			url: url,
			param:param,
			callback: function(msg){
				...
				var dialogSetting = {
					...
					confirmEvent:function(){
						PRM88_4.popShopProductConfirm(_this);
						removeDialog("#" + dialogId);
					}
				};
				openDialog(dialogSetting);
	...
	"popShopProductConfirm":function(_this){
		var excelProductShop=$("#excelProduct_w").val();
		if (excelProductShop!=""){
			...
			for(var i=0;i<excelProductShopList.length;i++){
				this.index ++;
				var rangeTypeClass = excelProductShopList[i].rangeType;
				var htmlCondition;
				var productList = excelProductShopList[i].productList;
				if (rangeTypeClass == 'PRODUCT'){
					htmlCondition ='<span id="rangeVal_'+this.index+'" onclick="PRM88_4.popConDialog(this);" class="RANGEVAL">';
					htmlCondition+='<div class="tag"><input type="hidden" name="prtVendorId" value="'+productList[0].prtVendorId+'"/>';
					htmlCondition+='<span style="margin:0 5px;">'+productList[0].nameTotal+'(UC:'+productList[0].unitCode +',BC:'+ productList[0].barCode+')</span>';
					htmlCondition+='<input type="hidden" value="'+productList[0].nameTotal+'(UC:'+productList[0].unitCode +',BC:'+ productList[0].barCode+')" name="rangeText">';
					htmlCondition+='<input type="hidden" value="'+ productList[0].unitCode + '+' + productList[0].barCode +'" name="rangeVal">';
				...
				}else if (rangeTypeClass == 'RANGE'){
					htmlCondition='<span id="rangeVal_'+this.index+'" onclick="PRM88_4.popConDialog(this);" class="RANGEVAL"></span>';
					htmlCondition+='<span id="ranges_1" class="RANGES" onclick="">';
					for (var j=0;j<productList.length;j++){
						htmlCondition += '<div class="tag">';
						htmlCondition += '<input class="NOSUBMIT" type="hidden" name="key" value="PRODUCT"/>';
						htmlCondition += '<input class="NOSUBMIT" type="hidden" name="name" value="特定产品"/>';
						htmlCondition += '<div class="tag_l">特定产品</div>';
						htmlCondition += '<div class="tag_r">';
						htmlCondition += '<div><input class="NOSUBMIT" type="hidden" name="val" value="' + productList[j].unitCode + '+' + productList[j].barCode +'"/>'
						htmlCondition += '<input class="NOSUBMIT" type="hidden" name="valText" value="' + productList[j].nameTotal + '"/>' + productList[j].nameTotal + '</div>';;
						htmlCondition += '</div><span class="icon_del" onclick="$(this).parent().remove();return false;"></span></div>';
					}
					htmlCondition +='</span><a class="hide" href="#" onclick="PRM88_4.popRangeDialog(this);return false;" style="display: inline;">添加</a>';
					htmlCondition +='<input name="ranges" value="" type="hidden">';
				}
				html+= '<li class="sortsubbox" style="margin-bottom:10px;">';
				html+= '<select name="rangeType" style="margin:0px;width:95px;"  value="'+excelProductShopList[i].rangeType+'" onchange="PRM88_4.changeRangeType(this);">'+
					'<option  value="ALL" >任意产品</option>' ;
				if("PRODUCT"==excelProductShopList[i].rangeType){
					html+='<option  value="PRODUCT"  selected="selected">特定产品</option><option  value="RANGE">产品范围</option>';
				}else if ("RANGE"==excelProductShopList[i].rangeType){
					html+='<option  value="PRODUCT"  >特定产品</option><option  value="RANGE" selected="selected">产品范围</option>';
				}
				...
				html+= '<select name="rangeOpt" style="margin:0px;width:60px;" disabled="true" onchange="PRM88_4.changeRangeOpt(this);"> ' +
...
var PRM88_4 = new BINOLSSPRM88_4();
$(document).ready(function() {
//	PRM88_4.index = $('li.sortsubbox').length;
	PRM88_4.setRewardType();
});
