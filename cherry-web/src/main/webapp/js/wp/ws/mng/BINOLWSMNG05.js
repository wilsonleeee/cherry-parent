var BINOLWSMNG05 = function () {
    
};

BINOLWSMNG05.prototype = {
	...
	"search":function(){
		...
		$("#mainForm").find(":input").each(function() {
		 ...
		 // 显示结果一览
		 $("#section").show();
		 var outputStockState=$("#outputStockState").val();
		 // 表格设置
		 if(outputStockState == "Y"){
			 var tableSetting = {
					 // 表格ID
					 tableId : '#dataTable',
					 // 数据URL
					 ...
					 aoColumns : [
									...
									{ "sName": "allocationNo","sWidth": "20%"},
									{ "sName": "allocationDate","sWidth": "10%"},
								    { "sName": "bin_OrganizationIDIn","bVisible": false,"sWidth": "10%"},
	 ...
	 * 弹出调出一览窗口
	 */
	"popAllocationInit":function(){
		var param = "";
		binOLWSMNG05_05.popDataTableOfProBG(param);
	},
	...
	"exportExcel" : function(url){
		...
		if($(".dataTables_empty:visible").length == 0){
		    ...
		    // 查询参数序列化
		    var params= $("#mainForm").find("div.column").find(":input").serialize();
			params = params + "&allocationType="+$("#allocationType").val();
			params = params + "&csrftoken=" +$("#csrftoken").val();
			params = params + "&currentMenuID="+$("#currentMenuID").val();
			params = params + "&params="+$("#params").val();
		    //选中导出的单据，不选导出全部
	...
	"exportCsv" : function() {
		if($(".dataTables_empty:visible").length==0) {
		    ...
		    var params = binOLWSMNG05.getSearchParams();
			exportReport({
...
var binOLWSMNG05 = new BINOLWSMNG05();
