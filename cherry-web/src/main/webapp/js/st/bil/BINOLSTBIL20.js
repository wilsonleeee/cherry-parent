var binOLSTBIL20_global = {};
//是否需要解锁
binOLSTBIL20_global.needUnlock = true;
...
window.onbeforeunload = function(){
	if(OS_BILL_Jump_needUnlock){
		if(binOLSTBIL20_global.needUnlock){
			if (window.opener) {
...
var BINOLSTBIL20 = function () {};
BINOLSTBIL20.prototype = {
	//产品弹出框
	"openProPopup":function(_this){
		...
		var option = {
			...
			getHtmlFun:function(info){// 目标区追加数据行function
				...
				var html = '<tr id="dataRow'+index+'"><td id="dataTd0"><input id="chkbox" type="checkbox" value="'+index+'" onclick="BINOLSTBIL20.changechkbox(this)"/></td>';
				...
				html += '<td id="dataTd5" class="alignRight">'+BINOLSTBIL20.formateMoney(price,2)+'</td>';
				if($("#th_stockQuantity").is(":visible")){
					html += '<td id="dataTd6" class="alignRight"></td>';
				}
				html += '<td id="nowCount" class="alignRight"><input name="quantityArr" id="quantityArr" class="text-number" size="10" maxlength="9" onchange="BINOLSTBIL20.changeCount(this);"/></td>';
			...
			click:function(){
				...
				BINOLSTBIL20.changeOddEvenColor();
				BINOLSTBIL20.calcTotal();
				...
				if(len>0){
					...
					cherryAjaxRequest({
						...
						reloadFlg:true,
						param:param,
						callback:BINOLSTBIL20.getStockSuccess
					});
				}
			}
		};
	...
	"changeDepotDropDownList":function(data){	
		...
		BINOLSTBIL20.setHideInventoryValue();
	},
	...
	"back":function(){
		binOLSTBIL20_global.needUnlock=false;
		var tokenVal = $('#csrftoken',window.opener.document).val();
		$('#saleReturnReqDetailUrl').find("input[name='csrftoken']").val(tokenVal);
		$('#saleReturnReqDetailUrl').submit();
	},
...
var BINOLSTBIL20 = new BINOLSTBIL20();