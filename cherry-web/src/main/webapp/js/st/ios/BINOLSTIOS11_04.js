function BINOLSTIOS11_04(){
	this.needUnlock = true;
};

BINOLSTIOS11_04.prototype={
		...
		"popDepartBox" : function(thisObj,flag) {
			...
			var callback = function() {
				...
				if($selected.length > 0) {
					...
					html += '<span class="text" style="line-height:19px;">' + departNameAccept + '</span>';
					html += '<span class="close" style="margin: -1px 0 0 6px;" onclick="BINOLSTIOS11_04.delPrmLabel(this);return false;"><span class="ui-icon ui-icon-close"></span></span>';
		 			if(flag=='in'){
		 				html += '<input type="hidden" name="organizationIdTo" value="' + departId + '"/>';
		 				html += '</span></td></tr>';
			 			$("#organization_ID").html("");
			 			$("#organization_ID").append(html);
		 			} else if(flag == 'out') {
		 				html += '<input type="hidden" name="organizationIdFrom" value="' + departId + '"/>';
		 				html += '</span></td></tr>';
		...
		"search" : function() {
			...
			var url = $("#searchDetail_Url").attr("href");
			...
			// 显示结果一览
			$("#proReturnReqExcelInfo").removeClass("hide");
			// 表格设置
			var tableSetting = {
					 // 表格ID
					 tableId : '#proReturnReqExcelInfoDataTable',
					 // 数据URL
					 url : url,
					 // 表格默认排序
					 aaSorting : [[ 1, "desc" ]],
					 ...
					 aoColumns : [  { "sName": "number", "bSortable": false},
					                ...
					                { "sName": "inventoryNameFrom", "bVisible": false },
					                { "sName": "logicInventoryNameFrom", "bVisible": false },
					                { "sName": "departNameTo" },
					                { "sName": "inventoryNameTo", "bVisible": false },
					                { "sName": "logicInventoryNameTo", "bVisible": false },
...
var BINOLSTIOS11_04 = new BINOLSTIOS11_04();
