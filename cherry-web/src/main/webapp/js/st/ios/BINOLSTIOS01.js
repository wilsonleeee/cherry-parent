var BINOLSTIOS01 = function () {};

BINOLSTIOS01.prototype = {
	...
	"changeDepotDropDownList":function(data){	
		...
		BINOLSTIOS01.getLogicDepotInfo();
	},
	...
	"getHtmlFun":function(info,negativeFlag){
		...
		var html = '<tr id="dataRow'+nextIndex+'">';
			html += '<td class="center"><input type="checkbox" id="chkbox" onclick="BINOLSTIOS01.changechkbox(this);"/></td>';
			html += '<td>' + unitCode + '</td>';
			html += '<td>' + barCode + '</td>';
			...
			html += '<td style="text-align:right;">' +BINOLSTIOS01.formateMoney(price,2) + '</td>';
			html += '<td style="text-align:right;"><input type="text" class="price" id="priceArr" name="priceArr" maxlength="17" value="'+ discountPrice + '" onchange="BINOLSTIOS01.setPrice(this);"/>';
			html += '<span id="spanCalPrice" class="hide"><span class="calculator" onclick="BINOLSTIOS01.initRateDiv(this);" title="'+$("#spanCalTitle").text()+'"></span></span></td>';
			html += '<td style="text-align:right;"><input value="'+quantity+'" name="quantityuArr"style="text-align:right;" id="quantityuArr" cssClass="text-number" size="10" maxlength="9" onchange="BINOLSTIOS01.calcuAmount(this);" cssStyle="width:98%"/></td>';
			html += '<td id="tdAmount" class="center" style="text-align:right;">' + BINOLSTIOS01.formateMoney(amount,2) + '</td>';
	...
	"openProPopup":function (){
	    ...
	    getHtmlFun:that.getHtmlFun,// 目标区追加数据行function
     	click:function(){//点击确定按钮之后的处理
			// 改变奇偶行的样式
			BINOLSTIOS01.changeOddEvenColor();
			//计算总金额总数量
			BINOLSTIOS01.calcuTatol();
			// 设置全选状态
	...
	"calcuAmount":function(obj){
		...
		if(isNaN($this.val().toString())){
			...
			$thisTd.parent().find("#tdAmount").text(BINOLSTIOS01.formateMoney(amount,2));
		}
		BINOLSTIOS01.calcuTatol();
	},
	...
	"calcuTatol":function(){
		...
		if(rows.length > 0){
			rows.each(function(i){
				...
				var $priceArr=$tds.find("#priceArr").val()
				if(isNaN($inputVal)){
					$inputVal=0;
				}
				...
				totalAmount += Number($inputVal) * parseFloat($priceArr);
			});
		}
		$("#totalQuantity").html(BINOLSTIOS01.formateMoney(totalQuantity,0));
		$("#totalAmount").html(BINOLSTIOS01.formateMoney(totalAmount,2));
	},
	...
	"copyInDepot":function(){
		...
		var callback = function() {
			...
			if($selected.length>0){
				...
				//清空原先入库单
				BINOLSTIOS01.clearDetailData();
				BINOLSTIOS01.getProInDepotDetail(negativeFlag,inDepotID);
			}
		};
	...
	"setPrice":function(obj){
		...
		//画面显示的金额
		$tr_obj.find("#tdAmount").html(BINOLSTIOS01.formateMoney(count*discountPrice,2));
		BINOLSTIOS01.calcuTatol();
	},
	...
	"pbAfterSelectFun":function(info){
		...
		if(flag){
			...
			$('#'+info.elementId).parent().parent().find("#spanPrice").html(BINOLSTIOS01.formateMoney(price,2));
			...
			//计算金额（防止先输入数量后输入产品）
			BINOLSTIOS01.calcuAmount($quantityInputText);
	...
	"addNewLine":function(){
		...
		if(addNewLineFlag){
			...
			html += '<td style="text-align:right;"><input type="text" class="price" id="priceArr" name="priceArr" maxlength="17" value="" onchange="BINOLSTIOS01.setPrice(this);"/>';
			html += '<span id="spanCalPrice" class="hide"><span class="calculator" onclick="BINOLSTIOS01.initRateDiv(this);" title="'+$("#spanCalTitle").text()+'"></span></span></td>';
			html += '<td style="text-align:right;"><input value="" name="quantityuArr"style="text-align:right;" id="quantityuArr" cssClass="text-number" size="10" maxlength="9" onchange="BINOLSTIOS01.calcuAmount(this);" cssStyle="width:98%"/></td>';
	...
	"setProductBinding":function(id){
		productBinding({elementId:id,showNum:20,targetDetail:true,afterSelectFun:BINOLSTIOS01.pbAfterSelectFun});
	},
	...
	"bindInput":function(){
		...
		$("#"+tableName+" #databody tr").each(function(i) {
			...
			BINOLSTIOS01.bindInputTR(trParam);
		});
	},
	...
	"bindInputTR":function(trParam){
		...
		$inp.bind('keydown', function (e) {
			if($("#currentMenuID").val() == "BINOLSTIOS01"){
				var key = e.which;
				if (key == 13) {
					//回车键
					e.preventDefault();
...
var BINOLSTIOS01 = new BINOLSTIOS01();

$(function(){
	// 产品popup初始化
//	BINOLSTIOS01.STIOS01_dialogBody = $('#productDialog').html();
