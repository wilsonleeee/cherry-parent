...
//						$("#spanBtnStart").bind("click",function(){
//							BINOLSTSFH06.STSFH06_addblankbill();
//							return false;
//						});
...
//			$("#spanBtnAddNewLine").attr('disabled',true);
		...
		"ShowProductResult":function (msg){
			...
			BINOLSTSFH06.changeOddEvenColor();
			BINOLSTSFH06.bindInput();
			$("#blankTable #databody").sortable({
				update:function (){//拖动后改变行序号
...
//		$("#spanBtnStart").unbind("click");	
		...
		if(addNewLineFlag){
			...
			html += '<td class="center"><input  value="" name="quantityuArr" id="quantityuArr" cssClass="text-number" style="width:120px;text-align:right;" size="10" maxlength="9" onchange="STSFH06_calcuAmount(this)" onkeyup="STSFH06_calcuAmount(this)"  cssStyle="width:98%"/></td>';
	...
	"setProductBinding":function(id){
		productBinding({elementId:id,showNum:20,targetDetail:true,afterSelectFun:BINOLSTSFH06.pbAfterSelectFun});
	},
	...
	"bindInput":function(){
		...
		$("#"+tableName+" #databody tr").each(function(i) {
			...
			BINOLSTSFH06.bindInputTR(trParam);
		})
	},
...
//		$tr_obj.find("[name=priceUnitArr]").val($discountPrice.val());
	...
	"submitConfirm":function() {
		var dialogSetting = {
			...
			confirmEvent: function(){BINOLSTSFH06.submitSend();removeDialog("#dialogInit");},
...
//		return false;
//	});
//	$("#spanBtnCancel").bind("click",function(){
//		BINOLSTSFH06.canceProductResult(true);
//		return false;
//	});
});
