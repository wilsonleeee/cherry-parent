var BINOLSTSFH23_02 = function () {};

BINOLSTSFH23_02.prototype = {
...
//			productBinding({elementId:id,showNum:20,targetDetail:true,afterSelectFun:BINOLSTSFH22.pbAfterSelectFun});
		...
		"bindInputTR":function(trParam){
			...
			$inp.bind('keydown', function (e) {
				if($("#currentMenuID").val() == "BINOLSTSFH22"){
					var key = e.which;
					if (key == 13) {
						//回车键
						e.preventDefault();
...
var BINOLSTSFH23_02 = new BINOLSTSFH23_02();
...
$(document).ready(function(){
	// 产品popup初始化
//	BINOLSTSFH23_02.STIOS01_dialogBody = $('#productDialog').html();
	if($("#inOrganizationId").val() != null && $("#inOrganizationId").val() !=""){
    ...
    + ((date.getMonth() + 1) > 9 ? (date.getMonth() + 1) : "0"
            + (date.getMonth() + 1))
    + "-"// "月"
    + (date.getDate() < 9 ? "0" + date.getDate() : date
            .getDate());
...
//	sugggestDayPopBody = $('#sugggestDayPop').html();
	//计算总金额总数量
	BINOLSTSFH23_02.calcuTatol();
	BINOLSTSFH23_02.bindInput();
//	BINOLSTSFH23_02.addNewLine();
});
