function BINOLSTSFH18(){
	this.needUnlock = true;
};

BINOLSTSFH18.prototype={
		...
		"popDepartBox" : function(thisObj,flag) {
			...
			var callback = function() {
				...
				if($selected.length > 0) {
					...
					html += '<span class="close" style="margin: -1px 0 0 6px;" onclick="BINOLSTSFH18.delPrmLabel(this);return false;"><span class="ui-icon ui-icon-close"></span></span>';
		 			if(flag=='in'){
		 				...
		 				html += '<input type="hidden" name="organizationIdAccept" value="' + departId + '"/>';
		 				html += '</span></td></tr>';
		...
		"search" : function() {
			...
			// 显示结果一览
			$("#prtOrderExcelInfo").removeClass("hide");
			// 表格设置
			var tableSetting = {
					 // 表格ID
					 tableId : '#prtOrderExcelInfoDataTable',
					 // 数据URL
					 url : url,
					 // 表格默认排序
					 aaSorting : [[ 1, "desc" ]],
					 ...
					 aoColumns : [  { "sName": "number", "bSortable": false},
					                { "sName": "billNo"},
					                { "sName": "departNameOrder" },
					                { "sName": "inventoryName", "bVisible": false },
					                { "sName": "logicInventoryName", "bVisible": false },
					                { "sName": "departNameAccept" },
					                { "sName": "inventoryNameAccept", "bVisible": false },
					                { "sName": "logicInventoryNameAccept", "bVisible": false },
					                { "sName": "totalQuantity" },
									{ "sName": "totalAmount" },
									{ "sName": "importDate" },
...
var BINOLSTSFH18 = new BINOLSTSFH18();
