function BINOLMBSVC01_1() {

};

BINOLMBSVC01_1.prototype = {
	/* 是否刷新一览画面 */
	"doRefresh" : false,
	/* 是否打开父页面锁定 */
	"needUnlock" : true,
	...
	"confirm" : function() {
		...
		var addRuleUrl = $('#addRuleUrl').attr('href');
		cherryAjaxRequest({
			url : addRuleUrl,
			param : param,
			callback : function(data) {
				BINOLMBSVC01_1.doRefresh=true;
			}
		});
	},
...
window.onbeforeunload = function() {
	if (window.opener) {
		if (BINOLMBSVC01_1.doRefresh) {
			// 刷新父页面
			window.opener.BINOLMBSVC01.searchList();
		}
		if (BINOLMBSVC01_1.needUnlock) {
			// 解除父页面锁定
...
var BINOLMBSVC01_1 = new BINOLMBSVC01_1();

var BINOLMBSVC01_1_tree;
...
$(function(){
	...
	$("#position_left_0").bind("keydown", function(event) {
		if (event.keyCode == 13) {
			BINOLMBSVC01_1.locationCutPosition(this, "keydown");
		}
	});
	...
	cherryValidate({
		formId : "ruleForm",
		rules : {
			"rechargeRule.ruleName" : {
				required : true,
				maxlength : 20
			},
			"rechargeRule.discountBeginDate" : {
				required : true
			},
			"rechargeRule.discountEndDate" : {
				required : true
			},
			"rechargeRule.rechargeValueActual" : {
				pointValid : [ 10, 2 ],
				required : true
			},// 应大于0的整数
			"rechargeRule.giftAmount" : {
				pointValid : [ 10, 2 ],
				required : true
			}
		// 可为0
	...
	BINOLMBSVC01_1.needUnlock = true;
});
