	...
	if(oTableArr[index] == null){
		...
		oTableArr[index] = $(tableSetting.tableId).dataTable( {
		   // 加载数据时显示正在加载信息
		    ...
		    "aaData":tableSetting.aaData,
		    "bFilter":tableSetting.bFilter,
			// 表格默认排序
			"aaSorting": aaSorting,
			// 表格及控件布局
 ...
 * @param tableIndex
 */
function datatableFilter2 (thisObj,tableIndex){
	var index;
	if (tableIndex!=null && tableIndex!=""){
		index = tableIndex;
	}else{
	...
	var smallBrandCode=$("#smallBrandCode").val();
	var productList=$("#productList").val();
	var productDialogSearch=$("#productDialogSearch").val();
	var newProductFlag=$("#newProductFlag").val();
//	var questionBankType=$("#questionBankType1").val();
		var question = new Array();			
		var q1 = {};//品牌
		q1.key = "smallBrandCode";
		q1.value = smallBrandCode;
		var q2 = {};//分类
		q2.key = "productList";
		q2.value = productList;
		var q3 = {};//产品名称
		q3.key = "productDialogSearch";
		q3.value = productDialogSearch;
		var q4 = {};//新品
		q4.key = "newProductFlag";
		q4.value = newProductFlag;
	...
	// 数据过滤
	oTableArr[index].fnFilter(queStr);
 }