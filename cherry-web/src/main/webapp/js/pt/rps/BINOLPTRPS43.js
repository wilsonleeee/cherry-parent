function BINOLPTRPS43() {
	
};
BINOLPTRPS43.prototype={
	    ...
	    "openProDialog" :function(flag) {
			...
			var option = {
	 	            ...
	 	            ignorePrtPrmVendorID : prtVendorIDArr, // 剔除销售商品与连带商品彼此已选择的商品
	 		       	getHtmlFun:function(info){
	     				...
	     				html += '<td class="center" style="width:15%;"><a href="javascript:void(0);" onClick="binolptrps43.deleteHtml(this);return false;">'+$("#deleteButton").text()+'</a></td>';
	     				html += '</tr>';
				   	...
				   	click : function() {
				   		if($("#"+proShowDiv).find("tr").length > 0) {
				   			...
				   			// 销售商品为空时，不显示连带销售
				   			if(flag != '1'){
				   				binolptrps43.removeJointPrtPrm();
				   			}
				   		}
				   	}
	 	    };
		 ...
		 "openProDialogOne" :function(flag) {
	    	var proTableOne = "proTableOne";
	    	var proShowDivOne = "proShowDivOne";
	    	var prtVendorId = "salePrtVendorId";
	    	var proPrmName = "saleProPrmName";
	    	if(flag == '1') {
				proTableOne = "joinProTableOne";
				proShowDivOne = "joinProShowDivOne";
				prtVendorId = "joinPrtVendorId";
				proPrmName = "joinProPrmName";
	    	}
			...
			var option = {
	 	            ...
	 	            ignorePrtPrmVendorID : prtVendorIDArr, // 剔除销售商品与连带商品彼此已选择的商品
	 		       	getHtmlFun:function(info){
	     		       	var html = '<tr>'; 
	     				html += '<td class="hide">' + '<input type="hidden" name="prtBarCode" value="' + info.barCode + '"/><input type="hidden" name="'+prtVendorId+'" value="' + info.barCode + '_N"/><input type="hidden" name="prtType" value="N"/><input type="hidden" name="'+proPrmName+'" value="' + info.barCode + '_spt_n' + info.nameTotal + '"/></td>';
	     				html += '<td style="width:25%;">' + info.barCode + '</td>';
	     				html += '<td style="width:35%;">' + info.nameTotal + '</td>';
	     				html += '<td class="center" style="width:15%;"><a href="javascript:void(0);" onClick="binolptrps43.deleteHtmlOne(this);return false;">'+$("#deleteButton").text()+'</a></td>';
	     				html += '</tr>';	     				
	     				return html;
				   	},
				   	click : function() {
				   		if($("#"+proShowDivOne).find("tr").length > 0) {
				   			$("#"+proTableOne).show();
				   			if(flag != '1'){
				   				$("#joinPrmProductTR1One").show();
				   			}
				   		} else {
				   			$("#"+proTableOne).hide();
				   			// 销售商品为空时，不显示连带销售
				   			if(flag != '1'){
				   				binolptrps43.removeJointPrtPrmOne();
				   			}
				   		}
				   	}
	 	    };
		 ...
		 * @param flag
		 */
		"openPrmDialogOne" :function(flag) {
			var proTableOne = "proTableOne";
			var proShowDivOne = "proShowDivOne";
			var prtVendorId = "salePrtVendorId";
			var proPrmName = "saleProPrmName";
			...
			var option = {
					...
					getHtmlFun:function(info){
						var html = '<tr>'; 
						html += '<td class="hide">' + '<input type="hidden" name="prmVendorId" value="' + info.barCode + '"/><input type="hidden" name="'+prtVendorId+'" value="' + info.barCode + '_P"/><input type="hidden" name="prtType" value="P"/><input type="hidden" name="'+proPrmName+'" value="' + info.barCode + '_spt_n' + info.nameTotal + '"/></td>';
						html += '<td style="width:25%;">' + info.barCode + '</td>';
						html += '<td style="width:35%;">' + info.nameTotal + '</td>';
						html += '<td class="center" style="width:15%;"><a href="javascript:void(0);" onClick="binolptrps43.deleteHtmlOne(this);return false;">'+$("#deleteButton").text()+'</a></td>';
						html += '</tr>';
		...
		"deleteHtmlOne": function(_this) {
			...
			// 销售商品为空时，不显示连带销售
			if($tbody.attr("id") == "proShowDivOne"){
				if($tbody.html().length == 0){
					binolptrps43.removeJointPrtPrmOne();
				}
			}			
		 ...
		 */
		"changeMode": function(){
			var mode_select = $("#mode_select").val();
			if(mode_select == '0'){
				$("#searchDivOne").hide();
				$("#searchDiv").show();
				$("#openProDialogOne").hide();
				$("#openProDialog").show();
				$("#proTableOne").hide();
				$("#proShowDivOne").empty();
				$("#joinProShowDivOne").empty();
				binolptrps43.removeJointPrtPrmOne();				
			}
			if(mode_select == '1'){
				$("#searchDivOne").show();
				$("#searchDiv").hide();				
				$("#openProDialogOne").show();
				$("#openProDialog").hide();
				$("#proTable").hide();
				$("#proShowDiv").empty();
		 ...
		 * 销售商品为空时，不显示连带销售(合并)
		 */
		"removeJointPrtPrmOne": function(){
			$("#joinPrmProductTR1One").hide();
			$("#joinProTableOne").hide();
			$("#joinProShowDivOne").empty();
		},
		// 选择活动弹出框
		"popCampaignList": function(url) {
			var callback = function(tableId) {
				...
				if($checkedRadio.length > 0) {
					...
					html += '<span class="close" style="margin: 0 10px 2px 5px;" onclick="binolptrps43.delCampaignHtml(this);"><span class="ui-icon ui-icon-close" style="background-position: -80px -129px;"></span></span>';
		...
		// 导出信息(薇诺娜，合并)
		"exportCsvOne" : function() {
			if($(".dataTables_empty:visible").length==0) {
				if (!$('#mainForm').valid()) {
					return false;
				};
				var params = getSearchParamsOne();
				exportReport({
					exportUrl:$("#exportCsvOneUrl").attr("href"),
					exportParam:params
				});
			}
		},
		...
		"popEditDialogOne":function(obj){
			...
			var dialogSetting = {
					...
					buttons : [ {
						text : comfirmTitle,
						click : function() {
							var url = $("#invoiceFlagReviseOneUrl").attr("href");
							...
							var callback = function() {
								...
								//var searchUrl = $("#searchOne").html();
								this.binOLPTRSP43_searchOne();
							};
...
var binolptrps43 =  new BINOLPTRPS43();
 ...
 * @returns
 */
function getSearchParamsOne(){
	var $form = $("#mainForm");
	var params= $form.find("#rps13SearchId").find(":input").serialize(); 
	var prtVendorIdLength = $("input[name='prtBarCode']").length + $("input[name='prmVendorId']").length;
	params = params + "&prtVendorIdLength=" +prtVendorIdLength;
	params = params + "&" +getRangeParams();
	params = params + "&csrftoken=" +$("#csrftoken").val();
	return params;
 ...
 * 导出销售记录查询结果
 */
function binOLPTRSP43_exportExcel(url){
	//无数据不导出
	if($(".dataTables_empty:visible").length == 0){
	    if (!$('#mainForm').valid()) {
	        return false;
 ...
 * 导出销售记录查询结果
 */
function binOLPTRSP43_exportExcelOne(url){
	//无数据不导出
	if($(".dataTables_empty:visible").length == 0){
		if (!$('#mainForm').valid()) {
			return false;
		...
		exportExcelReport({
			url: $("#exporChecktOneUrl").attr("href"),
			param: params,
			callback: callback
		});
	}
 ...
 * 商品详细
 */
function openSaleProPrmDialogOne(){
	var url = $("#getSaleProPrmSumOne_url").html();
 ...
 * @param url
 */
function binOLPTRSP43_search(){
	var url1 = $("#search").html();
	var $form = $("#mainForm");
	var index = 1;
...
//	paramsBak = "&paramsBak=" + paramsBak;
	...
	var tableSetting = {
			...
			aaSorting : [[ 10, "desc" ]],
			// 表格列属性设置
			aoColumns : [{ "sName": "saleRecordId","sWidth": "1%","bSortable":false}, 	// 0
 ...
 * @param url
 */
function binOLPTRSP43_searchOne(){
	var url1 = $("#searchOne").html();
	var $form = $("#mainForm");
	var index = 2;
