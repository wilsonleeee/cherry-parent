//是否刷新父页面
var BINOLPTJCS24_00_doRefresh = false;
...
window.onbeforeunload = function() {
	if (window.opener) {
		...
		if (BINOLPTJCS24_00_doRefresh) {
			if (window.opener.BINOLPTJCS24.modeFlg == 'listMode') {
				// 刷新父页面
				window.opener.BINOLPTJCS24.search($("#search_url",
						window.opener.document).val());
			} else {
				// 初始化树
				window.opener.BINOLPTJCS24.initTree();
			}
		}
	}
};
var BINOLPTJCS24_00 = function() {
};
BINOLPTJCS24_00.prototype = {
	// 对象JSON化
	"toJSON" : function(obj) {
		var JSON = [];
		$(obj).find(':input').each(
...
//					$("#soluDetailForm").submit();
					...
					BINOLPTJCS24_00_doRefresh = true;
				}
			},
			coverId : "#pageButton"//,
	...
	"delBarCode" : function(_this) {
		...
		var dialogSetting = {
				...
				confirmEvent: function(){BINOLPTJCS24_00.delBarCodeHandle(_this);},
	...
	"openCateDialog" : function(propId,_this){
		...
		var option = {
 		       ...
 		       getHtmlFun:function(info){
		       			...
		       			html += '<span class="close" style="margin: 4px 0 0 6px;" onclick="BINOLPTJCS24_00.removeCate(this);return false;"><span class="ui-icon ui-icon-close"></span></span>';
...
function checkboxBind() {
	$("#dataTableBody").find(":input").click(function() {
		...
		// BOM产品价格计算
//		BINOLPTJCS24_00.getBOMPrice();
	});
}

var BINOLPTJCS24_00 = new BINOLPTJCS24_00();
// 页面初期化方法
