...
function BINOLPTJCS22(){
	this.needUnlock = true;
};

BINOLPTJCS22.prototype={
	// 对象JSON化
	"toJSON" : function(obj) {
		var JSON = [];
		var propValArr = [];
...
//			});
	...
	"openChkPrtDialogInit" : function(){
		BINOLPTJCS22.cleanMsgDIV();
		if(BINOLPTJCS22.disabOptBtn()==false)return;
		var _this = this;
...
//		BINOLSTSFH01.clearActionMsg();
		...
		var option = {
			...
			click:function(){//点击确定按钮之后的处理
				...
				//改变奇偶行的样式
				BINOLPTJCS22.changeOddEvenColor();
				// 拖动排序
				$("#prtSaveDiv #mainTable #databody").sortable({
					update: function(event,ui){BINOLPTJCS22.changeOddEvenColor();}
				});
				// 设置全选状态
...
//		}
		...
		var html = '<tr>';
		html += '<td class="center"><input type="checkbox" id="chkbox" onclick="BINOLPTJCS22.changechkbox(this);"/></td>';
		...
		html += '<td>' + memPrice + '</td>';
		/*
		html += '<td class="center"><input value="' + price +'" name="salePriceArr"  type="text" id="salePriceArr" class="price" maxlength="17" cssStyle="width:98%" onchange="BINOLPTJCS22.setPrice(this);" /></td>';
		html += '<td class="center"><input value="' + memPrice +'" name="memPriceArr"  type="text" id="memPriceArr" maxlength="17"  cssStyle="width:98%" onchange="BINOLPTJCS22.setPrice(this)" /></td>';
		*/
	...
	"openCateDialog" : function(propId,_this){
		...
		var option = {
 		       ...
 		       getHtmlFun:function(info){
		       			...
		       			html += '<span class="close" style="margin: 4px 0 0 6px;" onclick="BINOLPTJCS22.removeCate(this);return false;"><span class="ui-icon ui-icon-close"></span></span>';
 ...
 * @returns
 */
function BINOLPTJCS22_cate(){
};
BINOLPTJCS22_cate.prototype={
	/**
	 * 方案明细添加产品分类
	 */
	"addCate" : function(){
   		// 产品分类
   		var param  = "cateInfo=" + BINOLPTJCS22.toJSONArr($("#cateInfo").find(".detail").children().children());
//   		alert("分类值: "+param);
...
var BINOLPTJCS22 = new BINOLPTJCS22();
var JCS22_cate = new BINOLPTJCS22_cate();
