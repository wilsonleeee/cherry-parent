//是否刷新父页面
var BINOLPTJCS00_doRefresh = false;
var PTJCS00_dialogBody = "";
var PTJCS00_prmDialogBody = "";
//产品对话框 确定事件
...
function selectPromotion() {
	closeCherryDialog('promotionDialog', PTJCS00_prmDialogBody);
	oTableArr[1002]= null;
}
// 页面关闭
window.onbeforeunload = function() {
	if (window.opener) {
		...
		if (BINOLPTJCS00_doRefresh) {
			if (window.opener.BINOLPTJCS04.modeFlg == 'listMode') {
				// 刷新父页面
				window.opener.BINOLPTJCS04.search($("#search_url",
						window.opener.document).val());
			} else {
				// 初始化树
				window.opener.BINOLPTJCS04.initTree();
			}
		}
	}
};
var BINOLPTJCS00 = function() {
};
BINOLPTJCS00.prototype = {
	// 对象JSON化
	"toJSON" : function(obj) {
		var JSON = [];
		$(obj).find(':input').each(
...
//		}
	...
	"delBarCode" : function(_this) {
		...
		var dialogSetting = {
				...
				confirmEvent: function(){BINOLPTJCS00.delBarCodeHandle(_this);},
...
//		this.getBOMPrice();
	...
	"addPrmBOMs" : function() {
		var proPopParam = {
				...
				isBOM : '1',
				index : 1002,
				dialogBody : PTJCS00_prmDialogBody,
				callback:function(){
					// 选中已选BOM组件
					prmSetCheckbox();
					// 产品pop框checkbox事件绑定
		...
		popDataTableOfPrmInfo(proPopParam);
	},
	// BOM产品价格计算
	...
	"openCateDialog" : function(propId,_this){
		...
		var option = {
 		       ...
 		       getHtmlFun:function(info){
		       			...
		       			html += '<span class="close" style="margin: 4px 0 0 6px;" onclick="BINOLPTJCS00.removeCate(this);return false;"><span class="ui-icon ui-icon-close"></span></span>';
...
function checkboxBind() {
	$("#dataTableBody").find(":input").click(function() {
		...
		if (this.checked) {
			var html = '<tr>'
					+'<td>' + '产品' 
					+ '<input type="hidden" name="subProdouctType" value="' + 'N' + '"/>'
					+ '</td>' 
					+'<td>'
		...
		// BOM产品价格计算
//		BINOLPTJCS00.getBOMPrice();
	});
}
//选中已选产品
function prmSetCheckbox(){
	var $bomBody = $("#bomBody");
	var $prtVendorId = $("#barCode").find("input[name='prtVendorId']"); 
	var prtVendorId = null;
	if($prtVendorId.length == 1){
	...
	var $prtSelected = $bomBody.find(':input[name=subPrtVendorId]');
	$("#prm_dataTableBody").find(":input").each(function(){
		var $checkbox = $(this);
		var selectedObj = window.JSON2.parse($checkbox.val());
		var selectedPrtVendorId = selectedObj.promotionProductVendorId;
		// 不能把自己作为自己的BOM组件
		if(prtVendorId == selectedPrtVendorId){
			$checkbox.prop("disabled",true);
		}
...
// 产品pop框checkbox事件绑定
function prmCheckboxBind() {
	$("#prm_dataTableBody").find(":input").click(function() {
		var $thisCheckbox = $(this);
		var $bomBody = $("#bomBody");
		var $delButton = $("#delButton");
		var selectedObj = window.JSON2.parse($thisCheckbox.val());
		if (this.checked) {
			var html = '<tr>'
				+'<td>' + '促销品' 
				+ '<input type="hidden" name="subProdouctType" value="' + 'P' + '"/>'
				+ '</td>' 
				+'<td>'
				+ selectedObj.unitCode
				+ '<input type="hidden" name="subPrtVendorId" value="'
				+ selectedObj.promotionProductVendorId
				+ '"/>'
				+ '</td><td>'
				+ selectedObj.barCode
				+ '</td><td>'
				+ selectedObj.nameTotal
				+ '</td><td><span><input name="bomPrice" class="price" value="'
				+ parseFloat(selectedObj.standardCost).toFixed("2")
				+ '"/></span></td><td><span><input name="bomQuantity" class="number" value="1"/></span></td><td class="center">'
				+ $delButton.html() + '</td></tr>';
			$bomBody.append(html);
		} else {
			$bomBody.find('tr').each(
				function() {
					if (selectedObj.promotionProductVendorId == $(this).find(
					':input[name=subPrtVendorId]').val()) {
						$(this).remove();
						return false;
					}
...
var BINOLPTJCS00 = new BINOLPTJCS00();
// 页面初期化方法
...
//	});
	// 产品对话框初始化
		...
		// 促销产品对话框初始化
		PTJCS00_prmDialogBody = $('#promotionDialog').html();
		// 标准销售价格初期化
		BINOLPTJCS00.initSalePrice();
		// price
