var BINOLMOCIO22 = function () {};

BINOLMOCIO22.prototype = {
	...
	"loadTree":function(nodes){
		...
		BINOLMOCIO22_tree = null;
		BINOLMOCIO22_tree = $("#treeDemo").zTree(treeSetting, treeNodes);
	...
	"loadChannelRegion" : function(msg) {
		...
		if(regionIdInfo && regionIdInfo.length > 0) {
			...
			$('#channelRegionDiv').find("#channelRegionId").change(function() {
				...
				var channelParam = "brandInfoId="+$("#brandInfoId").val()+"&departmentMessageId="+$("#departmentMessageId").val();
	...
	"ajaxGetNodes" : function() {
		...
		var param="brandInfoId="+$("#brandInfoId").val()+"&departmentMessageId="+$("#departmentMessageId").val();
	...
	"publish":function(){
		...
		var allowNodes = BINOLMOCIO22_tree.getCheckedNodes(flagArrow?true:false);
		var forbiddenNodes = BINOLMOCIO22_tree.getCheckedNodes(flagForbidden?true:false);
	...
	"locationCutPosition" : function (obj,flag){
		...
		var inputNode = BINOLMOCIO22_tree.getNodeByParam("name",$input.val());
		BINOLMOCIO22_tree.expandNode(inputNode,true,false);
		BINOLMOCIO22_tree.selectNode(inputNode);
	},
	...
	"ajaxFileUpload" : function(url) {
		...
		$.ajaxFileUpload({
			...
			data : {
				...
				'privilegeFlag' : "1",
				'departmentMessageId' : $("#departmentMessageId").val(),
...
var BINOLMOCIO22 = new BINOLMOCIO22();

var BINOLMOCIO22_tree;
...
$(function(){
	...
	$("#position_left_0").bind("keydown",function(event){
		if(event.keyCode==13){
			BINOLMOCIO22.locationCutPosition(this,"keydown");
        }
	});
});