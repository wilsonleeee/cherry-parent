	  	...
	  	<tr>
	       ...
	       <td style="width: 85%">
	         ...
	         <s:if test='%{#systemConfigMap2.type == "1"}'>
	         	<select name="bsCfInfoList[${index}].configValue">
	         ...
	         <s:if test='%{#systemConfigMap2.type == "2"}'>
	        		<s:iterator value="%{#systemConfigMap2.list}" id="configValueMap">
	        			<input type="checkbox" name="bsCfInfoList[${index}].configValue" value='<s:property value="#configValueMap.configValue"/>' <s:if test='%{#configValueMap.configEfficient == "1"}'>checked</s:if>/><s:property value="#configValueMap.commentsChinese"/>
	        		</s:iterator>
	         </s:if>
	         <s:if test='%{#systemConfigMap2.type == "3"}'>
	        		<s:iterator value="%{#systemConfigMap2.list}" id="configValueMap">
	        			<input type="text" name="bsCfInfoList[${index}].configValue" value='<s:property value="#configValueMap.configValue"/>' />
	        		</s:iterator>
	         </s:if>
	         </span>
	         <s:hidden name="configCode" value="%{#systemConfigMap2.configCode}"></s:hidden>
...
<div class="center">
<s:url action="BINOLPLSCF18_saveBsCf" id="saveBsCfUrl"></s:url>
	<div id="edit">
