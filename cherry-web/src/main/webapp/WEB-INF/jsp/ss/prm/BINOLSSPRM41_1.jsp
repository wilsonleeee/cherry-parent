...
<div id="aaData">
	<s:iterator value="proReturnList">
		<s:url id="detailsUrl" value="BINOLSSPRM42_init">
			<s:param name="proReturnId"><s:property value="promotionReturnId" /></s:param>
		</s:url>
		<ul>
			<%-- 选择 --%>
			<li><input type="checkbox" id="checkbill" value='<s:property value="promotionReturnId" />' onclick="checkBill(this);"/></li>
			<%-- 退库单号 --%>
			<li>
				<a href="${detailsUrl}" class="popup" onclick="javascript:openWin(this);return false;">
					<s:property value="returnNo"/>
				</a>
			</li>
			<%-- 业务类型   --%>
			<li><span><s:property value='#application.CodeTable.getVal("1037", tradeType)'/></span></li>
			<%-- 退库部门 --%>
			...
			<%-- 退库日期 --%>
			<li><span><s:property value="returnDate"/></span></li>
			<%-- 审核区分 --%>
			<li> 
			...
			<li>
				<s:if test="printStatus == @com.cherry.cm.core.CherryConstants@PRINT_STATUS_1">
					<s:url id="queryPrintLog" action="../common/BINOLCM99_queryPrintLog">
						<s:param name="billId">${promotionReturnId}</s:param>
						<s:param name="pageId">BINOLSSPRM42</s:param>
					</s:url>
