...
<script type="text/javascript" src="/Cherry/js/pt/jcs/BINOLPTJCS24.js"></script>
<script type="text/javascript" src="/Cherry/js/bs/common/BINOLBSCOM03.js"></script>
<%-- 产品添加URL --%>
<s:url id="add_url" action="BINOLPTJCS23_init"/>
      ...
      <div class="panel-header">
        <div class="clearfix">
			<span class="breadcrumb left">	    
       		...
       		<span class="right"> 
       		<%-- 产品下发按钮 --%>
       		<cherry:show domId="BINOLPTJCS2403">
       		...
       		<%-- 产品添加按钮 --%>
       		<cherry:show domId="BINOLPTJCS2401">
       		 	<a href="#" class="add" onclick="BINOLPTJCS24.addprt('${add_url}');return false;">
       		 		<span class="button-text"><s:text name="pro.add"/></span>
       		 		<span class="ui-icon icon-add"></span>
       		 	</a>
       		 </cherry:show>
       		 	<s:url action="BINOLPTJCS24_treeInit" id="treeModeInitUrl"/>
        		<s:url action="BINOLPTJCS24_listInit" id="listModeInitUrl"/>
        		...
        		<a id="treeMode" class="display display-tree display-tree-on" title='<s:text name="pro.treeMode"></s:text>' 
        			onclick="BINOLPTJCS24.changeTreeOrTable(this,'${treeModeInitUrl}');return false;"></a><a 
        		id="listMode" class="display display-table" title='<s:text name="pro.listMode"></s:text>' 
        			onclick="BINOLPTJCS24.changeTreeOrTable(this,'${listModeInitUrl}');return false;"></a>
       		</span>
       	</div>
        ...
        <!-- 查看当前启用的条码是否存在且有效 -->
		<s:url id="issuedPrtUrl" action="BINOLPTJCS24_issuePrt"/>
		<span class="hide" id="issuedPrtId">${issuedPrtUrl}</span>
		
      </div>
      <div id="treeOrtableId">
	  	<s:action name="BINOLPTJCS24_treeInit" executeResult="true"></s:action>
	  </div>
</s:i18n>	 