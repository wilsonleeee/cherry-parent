...
<script type="text/javascript" src="/Cherry/js/pt/jcs/BINOLPTJCS27.js"></script>
<!-- 语言 -->
<s:set id="language" value="#session.language" />
<%-- 保存 --%>
<s:url id="update_url" action="BINOLPTJCS27_update"/>
<%-- 详细 --%>
<s:url id="back_url" action="BINOLPTJCS26_init"/>
...
<div id="div_main" class="main container clearfix">
    ...
    <div class="panel-header">
	  ...
	  <form id="toDetailForm" action="BINOLPTJCS26_init" method="post">
      ...
      <cherry:form id="mainForm" method="post" class="inline" csrftoken="false">
          <%-- ======================= 基本属性开始  ============================= --%>
          <jsp:include page="/WEB-INF/jsp/pt/jcs/BINOLPTJCS27_2.jsp" flush="true"/>
          <%-- ======================= 基本属性结束  ============================= --%>
          <%-- ======================= BOM信息开始  ============================= --%>
              		...
              		<table class="detail" cellpadding="0" cellspacing="0">
		              	...
		              	<caption style="height:20px;">
			              	...
			              	<span class="delBtn <s:if test="%{compareDateFlag}"> hide </s:if>">
							   <a class="delete right" href="javascript:void(0)" onclick="BINOLPTJCS27.delDivRow(this);return false;">
			                  ...
			                  <td>
			                  	<span>
			                  		<s:if test="%{compareDateFlag}"> 
				                  		<s:textfield name="salePrice" cssClass="price" maxlength="17" value="%{salePrice}" onkeyup="BINOLPTJCS24_00.setMemPrice(this);return false;" disabled="true"/> <s:text name="JCS03_moneryUnit"/>
		                  		    	<%--  <s:hidden name="salePrice" value="%{salePrice}"></s:hidden>  --%>
			                  		</s:if>
			                  		<s:else>
				                  		<s:textfield name="salePrice" cssClass="price" maxlength="17" value="%{salePrice}" onkeyup="BINOLPTJCS24_00.setMemPrice(this);return false;" /> <s:text name="JCS03_moneryUnit"/>
			                  		</s:else>
	                  		    </span>
	                  		 </td>
			                  <th>
		  ...
		  <div class="section hide">
		  	<div class="section-header clearfix">
		  		...
		  		<a class="add right" onclick="BINOLPTJCS24_00.showExtInfo(this,'<s:text name="JCS03_showExt"/>','<s:text name="JCS03_hideExt"/>');return false;">
		  			<span class="ui-icon icon-add"></span>
		     		<span class="button-text"><s:text name="JCS03_showExt"/></span><%-- 显示扩展属性--%>
		    	</a>
		  	</div>
		    <div class="section-content hide" id="extInfo">
	      		<jsp:include page="/WEB-INF/jsp/pt/jcs/BINOLPTJCS27_1.jsp" flush="true"/>
	      	</div>
	      </div>		
          <%-- ======================= 扩展属性结束  ============================= --%>
      	</cherry:form>
        ...
        <div class="center clearfix" id="pageButton">
          <button class="save" type="button" onclick="BINOLPTJCS24_00.doSave('${update_url}');return false;">
          	<span class="ui-icon icon-save"></span>
          	<span class="button-text"><s:text name="global.page.save"/></span><%-- 保存 --%>
          </button>
          <button id="back" class="back" type="button" onclick="BINOLPTJCS27.doBack()">
