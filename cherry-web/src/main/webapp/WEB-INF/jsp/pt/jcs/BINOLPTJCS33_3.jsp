...
<%-- 产品扩展属性AJAX查询URL --%>
<s:url id="queryExt_url" action="BINOLPTJCS33_queryExt"/>
...
$(function() {
	...
	BINOLPTJCS33.originalBrandBinding({elementId:"originalBrandStr",codeType:"1299"}); // 子品牌查询 
  ...
  <div id="baseInfo" class="section-content">
	...
	<s:if test="productId == 0">
	    <table class="detail" cellpadding="0" cellspacing="0" >
	      <tr class="hide">
	          ...
	          <td>
	             <span>
	               <s:select id="brandSel" name="brandInfoId" list="brandInfoList" listKey="brandInfoId" listValue="brandName" onchange="BINOLPTJCS33.getSelOpts('%{queryCate_url}','%{queryExt_url}');" />
	             </span>
	         </td>
	      </tr>
	      <%-- 一品一码显示 --%>
		      ...
		      <tr>
		      	...
		      	<td id="barCode">
		      		<span class="vendorSpan" style="display:block; float:none;">		    
					    <span>
					    	<s:textfield cssClass="text textOth" maxlength="13" type="text" value="%{proMap.barCode}" name="barCode" tabindex="2" id="barCodeValId"
					    		onkeyup="this.value=this.value.toUpperCase();BINOLPTJCS33.getNewUnitCode()" 
					    		onchange="" 
					    		onblur="BINOLPTJCS33.getNewUnitCode();BINOLPTJCS33.chkExistBarCode()"/>
			    		</span>
		      	...
		      	<td>
		      		<span id="unitCodeSpanId" > <s:property value="proMap.unitCode"/></span>
		      		<span class="hide"><s:textfield name="unitCode" value="%{proMap.unitCode}" id="unitCode" cssClass="text textOth" maxlength="20" tabindex="1" onkeyup="this.value=this.value.toUpperCase()" /></span>
	      		</td>
		 	  </tr>     
	      </s:if>
	      <tr>
	      	<th><label><s:text name="JCS03_nameTotal"/><span class="highlight"><s:text name="global.page.required"/></span></label></th><%-- 中文名 --%>
	      	<td><span><s:textfield name="nameTotal" cssClass="text textOth" value="%{proMap.nameCN}" onkeyup="return isMaxByteLen(this)" tabindex="3"/></span></td>
	      ...
	      <tr class="hide">
	      	<th><label><s:text name="JCS03.measureUnit"/></label></th><%-- 产品规格 --%>
	   		<td>
	   			<span><s:textfield name="spec" cssClass="text textOth" maxlength="8" cssStyle="width:80px;" value="%{proMap.spec}"/></span><%-- 规格 --%>
	   			<s:select name="moduleCode" list='#application.CodeTable.getCodes("1190")'
	   			listKey="CodeKey" listValue="Value" cssStyle="width:90px;" value="%{proMap.moduleCode}"/><%-- 计量单位 --%>
	   		</td>
	        <th><label><s:text name="JCS03_styleCode"/></label></th><%-- 样式 --%>
	       	<td>
	       		<span>
	       			<s:select name="styleCode" list='#application.CodeTable.getCodes("1002")' 
	       				headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.styleCode}"/>
	       		</span>
	       	</td>
	      </tr> 
	      <tr>
			...
			<td>
				<span>
                  <s:textfield name="originalBrandStr" cssClass="text textOth" maxlength="50" value="%{#application.CodeTable.getVal('1299',proMap.originalBrand)}" onblur="BINOLPTJCS33.clearVal()" onkeyup="BINOLPTJCS33.clear()"/>
                  <s:hidden name="originalBrand" value="%{proMap.originalBrand}"></s:hidden>
                  <s:hidden name="originalBrandStrOld" value="%{#application.CodeTable.getVal('1299',proMap.originalBrand)}"></s:hidden>
	...
	<s:else>
	    <table class="detail" cellpadding="0" cellspacing="0" >
		      		...
		      		<span class="vendorSpan" style="display:block; float:none;">		    
					    <span class="hide">
					    	<s:textfield cssClass="text textOth" maxlength="13" type="text" value="%{proMap.barCode}" name="barCode" tabindex="2" id="barCodeValId"
					    		onkeyup="this.value=this.value.toUpperCase();BINOLPTJCS33.getNewUnitCode()" 
					    		onchange="" 
					    		onblur="BINOLPTJCS33.chkExistBarCode()"/>
			    		</span>
						<span id="unitCodeSpanId" > <s:property value="proMap.unitCode"/></span>
			 		</span>
		      	</td>
	      ...
	      <tr>
	      	...
	      	<th><label><s:text name="JCS03_prtDescription"/></label></th><%-- 别名 --%>
	       	<td>
	       		<span class="hide"><s:textfield name="nameAlias" cssClass="text textOth" maxlength="50" value="%{proMap.nameAlias}" /></span>
	       		<span  > <s:property value="proMap.nameAlias"/></span>
       		</td>
	      ...
	      <tr>
			...
			<td>
				...
				<span><s:property value='#application.CodeTable.getVal("1299",proMap.originalBrand)'/></span>
			</td>
			<th></th>
			<td></td>
	      </tr>
	      <tr class="hide">
	      	<th><label><s:text name="JCS03.measureUnit"/></label></th><%-- 产品规格 --%>
	   		<td>
	   			<span class="hide">
	   				<s:textfield name="spec" cssClass="text textOth" maxlength="8" cssStyle="width:80px;" value="%{proMap.spec}"/><%-- 规格 --%>
	   				<s:select name="moduleCode" list='#application.CodeTable.getCodes("1190")'
	   				listKey="CodeKey" listValue="Value" cssStyle="width:90px;" value="%{proMap.moduleCode}"/><%-- 计量单位 --%>
	   			</span>
	   			<span>
	   				<s:property value="proMap.spec"/>
	   				<s:property value='#application.CodeTable.getVal("1190",proMap.moduleCode)'/>
   				</span>
	   		</td>
	        <th><label><s:text name="JCS03_styleCode"/></label></th><%-- 样式 --%>
	       	<td>
	       		...
	       		<span><s:property value='#application.CodeTable.getVal("1002",proMap.styleCode)'/></span>
	       	</td>
	      </tr> 
	    </table>
	</s:else>
    ...
    <table class="detail hide" cellpadding="0" cellspacing="0" style="margin:0">
      ...
      <tr>
        ...
        <td><span><s:textfield name="nameShortForeign" cssClass="text textOth" maxlength="50" value="%{proMap.stNameEN}"/></span></td>
      </tr>
      <tr>
       	<th><s:text name="JCS03_mode"/></th><%-- 产品类型 --%>
       	<td><span><s:select name="mode" list='#application.CodeTable.getCodes("1136")' 
         			listKey="CodeKey" listValue="Value" onchange="BINOLPTJCS00.changeMode(this);return false;" value="%{proMap.mode}"/></span>
       	</td>
      	<th><label><s:text name="JCS03_nameForeign"/></label></th><%-- 英文名 --%>
        <td><span><s:textfield name="nameForeign" cssClass="text textOth" maxlength="100" onkeyup="return isMaxByteLen(this)" tabindex="4" value="%{proMap.nameEN}"/></span></td>
      </tr>
      <tr>
       	...
       	<td>
       		<span>
       			<s:select name="status" list='#application.CodeTable.getCodes("1016")' 
       				headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.status}"/>
       		</span>
       	</td>
	    ...
	    <td>
	    	<span>
	    		<s:select name="saleStyle" list='#application.CodeTable.getCodes("1253")' 
	    			 listKey="CodeKey" listValue="Value" onchange="BINOLPTJCS00.showExchangeRel(this);return false;" value="%{proMap.saleStyle}"/>
	    	</span>
	    	<span class="hide" id="exchangeRelationID">
	    		<s:textfield name="exchangeRelation" cssClass="text textOth" cssStyle="width:70px;height:14px;" value="%{proMap.exchangeRelation}"/><s:text name="g/件"/>
	    	</span>
      ...
      <tr>
	 	...
	 	<td><span><s:select name="starProduct" list='#application.CodeTable.getCodes("1148")' listKey="CodeKey" listValue="Value" value="%{proMap.starProduct}"/></span> </td>
      ...
      <tr>
        <th><s:text name="JCS03_isBOM"/></th><%-- 可否作为BOM的组成 --%>
      	...
      	<th><label><s:text name="JCS03_isExchanged"/></label></th><%-- 可否用于积分兑换 --%>
   		<td><span><s:select name="isExchanged" list='#application.CodeTable.getCodes("1220")' listKey="CodeKey" listValue="Value" value="%{proMap.isExchanged}"/></span> </td>
      </tr>
      <tr>
       	...
       	<td>
       		<span>
       			<s:select name="isReplenish" list='#application.CodeTable.getCodes("1004")' 
        			headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.isReplenish}"/>
        	</span>
        </td>
        ...
        <td>
        	<span>
        		<s:select name="lackFlag" list='#application.CodeTable.getCodes("1056")'
        			headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.lackFlag}"/>
       		</span>
       	</td>
      </tr>
      <tr>
      	<th><s:text name="JCS03_discontinueReseller"/></th><%-- 经销商供货状态 --%>
       	<td>
       		<span>
       			...
       			headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.disReseller}"/>
       		</span>
       	</td>
      	...
      	<td>
      		<span>
      			...
      			headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.disCounter}"/>
      		</span>
      	</td>
      </tr>
      <tr>
		...
		<td>
       		<span>
       			<s:select name="isStock" list='#application.CodeTable.getCodes("1140")'
       			 listKey="CodeKey" listValue="Value" value="%{proMap.isStock}"/>
       		</span>
		</td>
      </tr>
       	...
       	<td>
       		<span>
       			<s:select name="operationStyle" list='#application.CodeTable.getCodes("1003")' 
       				headerKey="" headerValue="%{JCS03_selectFirst}" listKey="CodeKey" listValue="Value" value="%{proMap.optStyle}"/>
       		</span>
       	</td>
     	<th> </th>
     	<td> </td>
