...
<s:i18n name="i18n.wp.BINOLWSMNG05">
    ...
    <div id="aaData">
    <s:iterator value="productAllocationOutList" id="productAllocationOutList">
        <s:url id="detailsUrl" action="BINOLWSMNG05_initDetail">
            <%-- 调出单ID --%>
            <s:param name="productAllocationOutID">${proAllocationId}</s:param>
        </s:url>
    <ul>
        <%-- 选择 --%>
        ...
        <li>
            <%-- 调出号--%>
            <a href="${detailsUrl}" class="left" onclick="javascript:openWin(this);return false;"><s:property value="allocationNo"/></a>
        </li>
        <li>
            <%-- 业务日期 --%>
            <s:if test='allocationDate != null && !"".equals(allocationDate)'>
                <s:property value="allocationDate"/>
            </s:if>
            <s:else>
                &nbsp;
            </s:else>
        ...
        <li>
            <%-- 调入部门 --%>
            <s:if test='bin_OrganizationIDIn != null && !"".equals(bin_OrganizationIDIn)'>
                (<s:property value="bin_OrganizationIDIn"/>)<s:property value="receiveDepart"/>
            </s:if>
            <s:else>
                &nbsp;
            </s:else>
