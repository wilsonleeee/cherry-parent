		...
		<ul>
			...
			<li><%-- 实收金额 --%>
				<s:if test="payAmount != null">
					...
					<s:else><span class="highlight">0.00</span></s:else>
				</s:if>
				<s:else>&nbsp;</s:else>
			</li>
			<li>
				<button id="dgBtnGetBillDetail" class="wp_search_s" type="button" style="float:left; margin:0 5px 0 0" onclick="BINOLWPSAL07.getBillDetail(this);return false;">
					<span class="icon_search"></span>
					<span class="wp_search_text"><s:text name="wpsal07.getBillDetail" /></span>
				</button>
				<s:if test="quantity != null && quantity > 0">
					<button id="dgBtnReturnsGoods" class="wp_search_s" type="button" style="float:left; margin:0 5px 0 0" onclick="BINOLWPSAL07.returnsBill(this);return false;">
						<span class="icon_search"></span>
						<span class="wp_search_text"><s:text name="wpsal07.returnsGoods" /></span>
					</button>
				</s:if>
				<!-- 打印销售单需用于此参数 -->
				<input type="hidden" id="quantity" value="<s:property value='quantity'/>"/>
				<input type="hidden" id="saleIdForPrint" value="<s:property value='saleId'/>"/>
				...
				<input type="hidden" id="dgSaleType" name="dgSaleType" value="<s:property value='saleType'/>"/>
