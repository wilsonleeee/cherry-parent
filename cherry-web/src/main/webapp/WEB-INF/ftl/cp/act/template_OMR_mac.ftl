...
<#if tempMap.campList?exists && tempMap.campList?size gt 0>
	<#list tempMap.campList as camp>
		<@getLYHDCampGiftList camp=camp index=camp_index virtPrmFlag=tempMap.virtPrmFlag groupFlag=tempMap.groupFlag! needBuyFlag=tempMap.needBuyFlag!/><#-- 老会员领用 -->
	</#list>
</#if>
</#macro>
<#-- =========活动奖励确认=========== -->
...
<div class="box4" style="margin-top:0px;">
	...
	<div class="box4-content clearfix">
		<@getLYHDCampGiftList_CFM campMap=campMap /><#-- 老会员领用-->
	</div>
</div>
</#macro>
<#-- *********活动对象类型******** -->
