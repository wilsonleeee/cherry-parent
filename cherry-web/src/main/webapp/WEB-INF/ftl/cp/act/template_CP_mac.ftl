		...
		<table style="margin-bottom: 0px;" class="detail">
          <tbody>
            <tr>
			  ...
			  <td><span><@s.property value="#application.CodeTable.getVal('1228','${(campMap.subCampType)!?html}')"/></span></td>
		    </tr>
		     <tr>
...
<#if tempMap.campList?exists && tempMap.campList?size gt 0>
	<#list tempMap.campList as camp>
		<@getLYHDCampGiftList camp=camp index=camp_index virtPrmFlag=tempMap.virtPrmFlag groupFlag=tempMap.groupFlag! needBuyFlag=tempMap.needBuyFlag! showCoupon=1/><#-- coupon优惠券 -->
	</#list>
</#if>
</#macro>
<#-- =========活动奖励确认=========== -->
