...
$(document).ready(function() {
	var clubMod = '${clubMod!}';
	if (clubMod == "1") {
		CHERRYTREE.loadTree('${clubInfo.placeJson!}',0);
	}
});
</script>
		...
		<div class="panel ui-corner-all">
			<div class="panel-header">
				<div class="clearfix">
					<span class="breadcrumb left"> <span
						class="ui-icon icon-breadcrumb"></span> <@s.text name="cp.memclub" /> &gt; <@s.text name="cp.editmemclub" />
					</span>
				</div>
			</div>
			<div class="panel-content">
				...
				<div class="section">
					...
					<@cherry.form id="mainForm" method="post" class="inline"
							csrftoken="false">
							<input type="hidden" name="memberClubId" value="${(clubInfo.memberClubId)!}" id="memberClubId"/>
							<input type="hidden" name="upTime" value="${(clubInfo.upTime)!}" id="upTime"/>
							<input type="hidden" name="modCount" value="${(clubInfo.modCount)!}" id="modCount"/>
							...
							<table id="baseInfo" class="detail" cellpadding="0"
								...
								<tr>
									...
									<#if (brandInfoList?? && brandInfoList?size > 0) >
					         		...
					         		<input type="hidden" name="brandInfoId" value="${(clubInfo.brandInfoId)!}" id="brandSel"/>
									</span></td>
									<th>
										<label><@s.text name="cp.clubName" /></label>
									</th>
									<td><span> <@s.textfield name="clubName" cssClass = "text" maxlength="50" value="${(clubInfo.clubName)!}"></@s.textfield>
									</span></td>
								</tr>
								<tr>
									<th>
									...
									<td><span> <input type="text" name="clubCode" class = "text" maxlength="50" value="${(clubInfo.clubCode)!}" <#if (clubInfo.sdflag)! != "0" > disabled="true" </#if>></input>
									<#if (clubInfo.sdflag)! != "0" > <input type="hidden" name="clubCode" value="${(clubInfo.clubCode)!}"/>  </#if>
									</span></td>
									<th><label><@s.text name="cp.clubNameEn" /></label></th>
									<td><span> 
											<@s.textfield name="clubNameForeign" cssClass = "text" maxlength="50" value="${(clubInfo.clubNameForeign)!}"></@s.textfield>
									</span></td>
								</tr>
								<tr>
									<th><label><@s.text name="cp.clubDesp" /></label></th>
									<td><span> 
											<textarea  name="descriptionDtl" style="height:50px;">${(clubInfo.descriptionDtl)!}</textarea>
									</span></td>
								</tr>
            ...
            <tbody id="origBrand_tbody">
            	<#if (origBrandList?? && origBrandList?size>0)>
	 				...
	 				<td class="center"><input type="hidden" name="origBrandCode" value="${(origBrandInfo.originalBrand)!}" />
			            ...
			            <div class="box2-header clearfix">
			            	...
			            	<span style="margin:3px 15px 0px 15px;" class="left">                      
				            	...
				            	<select id="locationType_0" name="locationType" onchange="CHERRYTREE.changeType(this,0);" style="width:120px;">
									...
									<@getOptionList list=application.CodeTable.getCodes("1156") val= clubInfo.locationType! />
								</select>
			            	</span>
